...
static qboolean Q3_SetNavGoal( int entID, const char *name )
{
	...
	/*if ( Q_stricmp( "worldspawn", name ) == 0 )
	{
		G_DebugPrint( WL_ERROR, "Q3_SetNavGoal: Target is WorldSpawn: \"%s\"\n", ent->script_targetname );
		return qfalse;
	}*/
...
#ifdef _DEBUG
				if ( targ && targ->client )
					Com_Printf("Q3_SetNavGoal to \"player\", called '%s'\n",targ->client->pers.netname);
#endif
			}
