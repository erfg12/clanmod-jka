...
int InspectCvar (char *cvarname)
{
	char *buf, *name;
	char *q, *r;
	

	if ( (buf = BG_TempAlloc(256)) == 0 )
	{ //assume the worst
		return 1;
	}
	if ( (name = BG_TempAlloc(256)) == 0 )
	{ //assume the worst
		return 1;
	}
    ...
    // we don't actually want to recurse here (like in the file case),
	// since the execution of vstr is not recursive!
...
qboolean UI_ConsoleCommand( int realTime ) {
	...
	{ //lmo process binds and scan for macros
		int i,j, macro;
		char buf[256];
...
//		Com_Printf("starting macro scan\n");
		for ( i=0 ; i < MAX_KEYS ; i++)
		{
			...
			{ // keys is bound to exec scripts in files
			...
			{ // key is bound to an evaluation of a variable
				// OK, lets look at the variable
				char cname[255];
				char *q, *r;
	...
	if ( Q_stricmp (cmd, "ui_macroenable") == 0 )
	{ //lmo process binds and scan for macros, the re-enable them
		int i, j;
		char buf[256];
