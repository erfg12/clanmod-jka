...
//lmo for extended voting
mod_votes_t mod_votes[MAX_VOTES];
int mod_votes_count;
...
vmCvar_t	cm_DuelForceRegenTime;
...
//============================grapplemod===================
vmCvar_t	m_grapple_shoot_speed; //grapple speed 
...
vmCvar_t	cm_DISRUPTOR_MAIN_DAMAGE;
vmCvar_t	cm_DISRUPTOR_ALT_DAMAGE;
vmCvar_t	cm_BOWCASTER_DAMAGE;
vmCvar_t	cm_REPEATER_DAMAGE;
vmCvar_t	cm_REPEATER_ALT_DAMAGE;
vmCvar_t	cm_REPEATER_SPLASH_DAMAGE;
vmCvar_t	cm_REPEATER_VELOCITY;
vmCvar_t	cm_BOWCASTER_VELOCITY;
vmCvar_t	cm_DEMP2_DAMAGE;
vmCvar_t	cm_DEMP2_ALT_DAMAGE;
vmCvar_t	cm_DEMP2_VELOCITY;
vmCvar_t	cm_FLECHETTE_DAMAGE;
vmCvar_t	cm_FLECHETTE_ALT_DAMAGE;
vmCvar_t	cm_FLECHETTE_VELOCITY;
vmCvar_t	cm_ROCKET_DAMAGE;
vmCvar_t	cm_ROCKET_SPLASH_DAMAGE;
vmCvar_t	cm_ROCKET_VELOCITY;
vmCvar_t	cm_CONCUSION_DAMAGE;
vmCvar_t	cm_CONCUSION_SPLASH_DAMAGE;
vmCvar_t	cm_CONCUSION_ALT_DAMAGE;
vmCvar_t	cm_CONCUSION_VELOCITY;
vmCvar_t	cm_STUN_BATON_DAMAGE;
vmCvar_t	cm_MELEE_SWING1_DAMAGE;
vmCvar_t	cm_MELEE_SWING2_DAMAGE;
vmCvar_t	cm_THERMAL_DETONATOR_DAMAGE;
vmCvar_t	cm_THERMAL_DETONATOR_ALT_DAMAGE;
vmCvar_t	cm_THERMAL_DETONATOR_VELOCITY;
vmCvar_t	cm_TRIP_MINE_DAMAGE;
vmCvar_t	cm_DET_PACK_DAMAGE;
vmCvar_t	cm_DET_PACK_SPLASH_DAMAGE;

vmCvar_t	cm_BRYAR_PISTOL_DAMAGE;
vmCvar_t	cm_BRYAR_PISTOL_VELOCITY;

vmCvar_t	g_NPCmaxFollowers;

vmCvar_t		g_clanPassword;

vmCvar_t		roar_BotFakePings;

vmCvar_t		roar_sayings_display;
...
vmCvar_t		cm_allow_report_command;
...
vmCvar_t		cm_adminlogin4_saying;
vmCvar_t		cm_adminlogin5_saying;

vmCvar_t		cm_adminlogout1_saying;
vmCvar_t		cm_adminlogout2_saying;
vmCvar_t		cm_adminlogout3_saying;
vmCvar_t		cm_adminlogout4_saying;
vmCvar_t		cm_adminlogout5_saying;

vmCvar_t		cm_adminControl1;
vmCvar_t		cm_adminControl2;
vmCvar_t		cm_adminControl3;
vmCvar_t		cm_adminControl4;
vmCvar_t		cm_adminControl5;


vmCvar_t		cm_customSaberDmgs;

vmCvar_t		roar_allow_KnockMeDown_command;

vmCvar_t		cm_spillpassword;
...
//RoAR mod CUSTOM SAYINGS BEGIN
	...
	{ &roar_sayings_display,			"cm_sayings_display",			"2",		CVAR_ARCHIVE,					0, qfalse },
	...
	{ &roar_rename_saying,				"cm_rename_saying",			"'s name was forcefully changed by an admin.",		CVAR_ARCHIVE,					0, qfalse },
	...
	{ &roar_display_ending_duel_status,			"cm_duelstatus",			"1",		CVAR_ARCHIVE,					0, qtrue },

	{ &roar_elevator_1_hit_kill,			"cm_elevator",			"1",		CVAR_ARCHIVE,					0, qtrue },
	{ &cm_duelshield,			"cm_duelshield",			"25",		CVAR_ARCHIVE,					0, qtrue },

	{ &roar_remove_duel_radius,			"cm_duelradius",			"1",		CVAR_ARCHIVE,					0, qtrue },
	...
	{ &roar_allow_auto_saber_switching,			"cm_saberswitching",			"1",		CVAR_ARCHIVE,			0, qtrue },

	{ &cm_empower_on_saying,			"cm_empower_on_saying",			"has become empowered",		CVAR_ARCHIVE,					0, qfalse },
	...
	{ &cm_adminlogin4_saying,	"cm_adminlogin4_saying",		"has logged in as admin level 4!",	CVAR_ARCHIVE,					0, qfalse },
	{ &cm_adminlogin5_saying,	"cm_adminlogin5_saying",		"has logged in as admin level 5!",	CVAR_ARCHIVE,					0, qfalse },

	{ &sv_maxConnections,  "sv_maxConnections",  "0", CVAR_INTERNAL, 0, qfalse },

	{ &cm_adminlogout1_saying,	"cm_adminlogout1_saying",		"has logged out of admin level 1!",	CVAR_ARCHIVE,					0, qfalse },
	{ &cm_adminlogout2_saying,	"cm_adminlogout2_saying",		"has logged out of admin level 2!",	CVAR_ARCHIVE,					0, qfalse },
	{ &cm_adminlogout3_saying,	"cm_adminlogout3_saying",		"has logged out of admin level 3!",	CVAR_ARCHIVE,					0, qfalse },
	{ &cm_adminlogout4_saying,	"cm_adminlogout4_saying",		"has logged out of admin level 4!",	CVAR_ARCHIVE,					0, qfalse },
	{ &cm_adminlogout5_saying,	"cm_adminlogout5_saying",		"has logged out of admin level 5!",	CVAR_ARCHIVE,					0, qfalse },

	{ &cm_adminControl1,	"cm_adminControl1",		"0",	CVAR_INTERNAL,					0, qfalse },
	{ &cm_adminControl2,	"cm_adminControl2",		"0",	CVAR_INTERNAL,					0, qfalse },
	{ &cm_adminControl3,	"cm_adminControl3",		"0",	CVAR_INTERNAL,					0, qfalse },
	{ &cm_adminControl4,	"cm_adminControl4",		"0",	CVAR_INTERNAL,					0, qfalse },
	{ &cm_adminControl5,	"cm_adminControl5",		"0",	CVAR_INTERNAL,					0, qfalse },
	...
	{ &roar_spectate_keep_score,			"cm_keepscore",			"0",		CVAR_ARCHIVE,					0, qtrue },
...
void G_RegisterCvars( void ) {
	...
	*/
	...
	//lmo initialise vote strings

	i = 0;
	// this is gonna be brutal!
	if ( strlen(m_v1.string) )
	{
		Q_strncpyz(buf, m_v1.string, sizeof(buf));
		if ( (p = strchr(buf,':')) != 0 )
		{
			*p = 0;
			Q_strncpyz(mod_votes[i].votestring, buf, MAX_NAME_LENGTH);
			Q_strncpyz(mod_votes[i].voteexec, p+1, MAX_CVAR_VALUE_STRING);
			i++;
		}
		else
		{ // bad vote string
			G_Printf("m_v1 contains a mal-formed vote string\n");
		}
	}
	if ( strlen(m_v2.string) )
	{
		Q_strncpyz(buf, m_v2.string, sizeof(buf));
		if ( (p = strchr(buf,':')) != 0 )
		{
			*p = 0;
			Q_strncpyz(mod_votes[i].votestring, buf, MAX_NAME_LENGTH);
		...
		{ // bad vote string
			G_Printf("m_v2 contains a mal-formed vote string\n");
		}
	}
	if ( strlen(m_v3.string) )
	{
		Q_strncpyz(buf, m_v3.string, sizeof(buf));
		if ( (p = strchr(buf,':')) != 0 )
		{
			*p = 0;
			Q_strncpyz(mod_votes[i].votestring, buf, MAX_NAME_LENGTH);
		...
		{ // bad vote string
			G_Printf("m_v3 contains a mal-formed vote string\n");
		}
	}
	if ( strlen(m_v4.string) )
	{
		Q_strncpyz(buf, m_v4.string, sizeof(buf));
		if ( (p = strchr(buf,':')) != 0 )
		{
			*p = 0;
			Q_strncpyz(mod_votes[i].votestring, buf, MAX_NAME_LENGTH);
		...
		{ // bad vote string
			G_Printf("m_v4 contains a mal-formed vote string\n");
		}
	}
	if ( strlen(m_v5.string) )
	{
		Q_strncpyz(buf, m_v5.string, sizeof(buf));
		if ( (p = strchr(buf,':')) != 0 )
		{
			*p = 0;
			Q_strncpyz(mod_votes[i].votestring, buf, MAX_NAME_LENGTH);
		...
		{ // bad vote string
			G_Printf("m_v5 contains a mal-formed vote string\n");
		}
	}
	if ( strlen(m_v6.string) )
	{
		Q_strncpyz(buf, m_v6.string, sizeof(buf));
		if ( (p = strchr(buf,':')) != 0 )
		{
			*p = 0;
			Q_strncpyz(mod_votes[i].votestring, buf, MAX_NAME_LENGTH);
		...
		{ // bad vote string
			G_Printf("m_v6 contains a mal-formed vote string\n");
		}
	}
	mod_votes_count = i;
	//lmo PHEW!
...
void G_UpdateCvars( void ) {
	...
	for ( i = 0, cv = gameCvarTable ; i < gameCvarTableSize ; i++, cv++ ) {
		if ( cv->vmCvar ) {
			...
			if ( cv->modificationCount != cv->vmCvar->modificationCount ) {
				...
				// lmo consider special actions when some cvars are modified

				//mod vote strings
				if ( !Q_stricmp(cv->cvarName, "m_v1") ||
					 !Q_stricmp(cv->cvarName, "m_v2") ||
					 !Q_stricmp(cv->cvarName, "m_v3") ||
					 !Q_stricmp(cv->cvarName, "m_v4") ||
					 !Q_stricmp(cv->cvarName, "m_v5") ||
					 !Q_stricmp(cv->cvarName, "m_v6") )
				{
					voteupdate = qtrue; // mark it for later processing
				}
	...
	if (voteupdate)
	{ // votestring(s) have been changed, we must process
		char buf[MAX_INFO_STRING];
		char *p;
...
//MOD FEATURE
//========
extern void SP_misc_npc_generator( gentity_t *self );
...
void G_ReadLevelFix()
{
	...
	Com_sprintf(levelname, sizeof(levelname), "maps/%s.npcs\0", mapname.string);

#ifdef _DEBUG
	Com_Printf( "levelname : %s\n", levelname );
#endif
	...
	if ( !f || len >= 16384 )		//Juste par précaution
	{
		return;
	}

	trap_FS_Read(fix_info, len, f);

	trap_FS_FCloseFile(f);

	fix_info[len] = 0;
	len = COM_Compress( fix_info );

	p = fix_info;
	COM_BeginParseSession("levelfix");
	...
	while ( 1 ) 
	{
		...
		if ( !Q_stricmp( token, "npcgen" ) ) 
		{
			gentity_t	*ent;

			/*if ( g_gametype.integer != GT_RPG )
			{
				//Com_Printf( "incorrect GAMETYPE, SKIPPING!\n" );
				SkipRestOfLine( &p );
				continue;
			}*/

			//origin
			if ( COM_ParseVec3( &p, &v ) ) 
			{
				continue;
			}

			ent = G_Spawn();
			ent->classname = "misc_npc_generator";
			...
			//goal
			NPCG_ParseGoals( &p, ent->npcgen ); //hmm..
			...
			ent->npcgen->interval = n;

			//types
			NPCG_ParseTypes( &p, ent->npcgen );

			if ( !NPCG_TypeCount( ent->npcgen ) )
			{
#ifdef _DEBUG
				Com_Printf( S_COLOR_RED"ERROR : no one NPC type defined for NPC Generator, skipping..\n" );
#endif
			...
			SP_misc_npc_generator( ent );

			continue;
		}

#ifdef _DEBUG
		Com_Printf( S_COLOR_YELLOW"WARNING : Unknown key '%s'\n", token );
#endif
...
void G_RunFrame( int levelTime ) {
	...
	if (gDoSlowMoDuel)
	{
		...
		else
		{
			...
			/*else if (timeDif < 1150) //RoAR mod NOTE: I removed this because it causes a Connection Interrupted message.
			{
