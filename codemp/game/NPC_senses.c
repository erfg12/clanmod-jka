...
qboolean IsLookingAround( void )
{
	if ( NPC->client->ps.torsoAnim == BOTH_GUARD_IDLE1 ||
		NPC->client->ps.torsoAnim == BOTH_GUARD_LOOKAROUND1 )
	{
		return qtrue;
	}
...
qboolean G_ClearLOS( gentity_t *self, const vec3_t start, const vec3_t end )
{
	...
	trap_Trace ( &tr, start, NULL, NULL, end, self->s.number/*ENTITYNUM_NONE*/, CONTENTS_OPAQUE/*CONTENTS_SOLID*//*(CONTENTS_SOLID|CONTENTS_MONSTERCLIP)*/ );
