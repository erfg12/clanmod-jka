...
typedef struct {
	...
	int				duelTeam;
	int				clanIcon;		//RoAR mod NOTE: Added for clan icons!
...
//	vec3_t			rawOrigin;
	...
	//NPCMod FIX : Boba ze come back
	//NPCMod FIXME : intialisé à qfalse?
	qboolean		bobaInit;
...
//	snapshot_t	activeSnapshots[2];
	...
	// buy list
	int			numBuyables;
	buyable_t	buyables[MAX_BUYABLES];
...
//[/RGBSabers]
extern vmCvar_t	c_Clanpassword;
extern vmCvar_t	c_Adminpassword;
...
//lmo re-route syscalls for duel non interference
#define		trap_S_StartSound(a,b,c,d) nox_trap_S_StartSound(a,b,c,d)
...
//lmo re-route syscall for nox
#define		trap_S_AddLoopingSound(a,b,c,d) nox_trap_S_AddLoopingSound(a,b,c,d)
void		nox_trap_S_AddLoopingSound( int entityNum, const vec3_t origin, const vec3_t velocity, sfxHandle_t sfx );
//lmo re-route syscall for nox
#define		trap_S_AddRealLoopingSound(a,b,c,d) nox_trap_S_AddRealLoopingSound(a,b,c,d)
