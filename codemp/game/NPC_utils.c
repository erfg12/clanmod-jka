...
{
	npcteam_t entTeam = NPCTEAM_FREE;
	...
	else if ( ent->client->sess.sessionTeam == TEAM_SPECTATOR || ent->client->pers.tzone || ent->client->ps.duelInProgress )
	...
	if ( ent->client->ps.isJediMaster && g_gametype.integer == GT_JEDIMASTER )
		return qtrue;
	...
	//	return qfalse;
	if ( ( entTeam == NPC->client->enemyTeam && NPC->client->enemyTeam != NPCTEAM_FREE )//simplest case: they're on my enemy team
		...
		|| ( (entTeam == NPCTEAM_FREE||entTeam == NPCTEAM_NEUTRAL)
			&& ( ( ent->enemy == NPC || ( ent->enemy && ent->enemy->client && ent->enemy->client->playerTeam == NPC->client->playerTeam ) ) )
			//||( ent->NPC && ( ent->NPC->goalEntity == NPC || ( ent->NPC->goalEntity && ent->enemy->client && ent->enemy->client->playerTeam == NPC->client->playerTeam ) )
	...
	if ( ent->client->NPC_class == CLASS_RANCOR && ent->activator && ent->activator->client && ent->activator->client->playerTeam == NPC->client->playerTeam )
		return qtrue;
...
qboolean NPC_TargetVisible2( gentity_t *ent )
{
	//Make sure we're in a valid range
	//if ( DistanceSquared( ent->r.currentOrigin, NPC->r.currentOrigin ) > ( NPCInfo->stats.visrange * NPCInfo->stats.visrange ) )
	//	return qfalse;
...
int NPC_FindNearestEnemy( gentity_t *ent )
{
	...
	for ( i = 0; i < numEnts; i++ )
	{
		...
		if ( radEnt->s.eType == ET_MISSILE && radEnt->s.owner != ENTITYNUM_NONE && NPC_TargetVisible2( radEnt ) )
		{
			if ( NPC_ValidEnemy( &g_entities[radEnt->s.owner] ) )
			{
				distance = DistanceSquared( ent->r.currentOrigin, g_entities[radEnt->s.owner].r.currentOrigin );

				//Found one closer to us
				if ( distance < nearestDist )
				{
					nearestEntID = g_entities[radEnt->s.owner].s.number;
		...
		if ( radEnt->client
			&& radEnt->client->playerTeam == ent->client->playerTeam
			&& NPC_ValidEnemy( radEnt->enemy )
			&& ( NPC_TargetVisible2( radEnt->enemy ) ) )
		{
			distance = DistanceSquared( ent->r.currentOrigin, radEnt->enemy->r.currentOrigin );

			//Found one closer to us
			if ( distance < nearestDist )
			{
				nearestEntID = radEnt->enemy->s.number;
				nearestDist = distance;
			}
		...
		{
			if ( NPC_TargetVisible2( radEnt ) == qfalse )
				continue;
		}
	...
	*/
		...
		NPC->client->ps.powerups[PW_CLOAKED] )
	{
		if ( NPC->client->leader &&
			...
			NPC->client->leader->client->ps.powerups[PW_CLOAKED] )
		{
			return NULL;
		}
		else
		{
			NPCInfo->stats.earshot *= 0.2;
			NPCInfo->stats.visrange *= 0.2;
		}
	}
	
	entID = NPC_FindNearestEnemy( NPC );

	//if ( NPC->client->NPC_class == CLASS_SHADOWTROOPER &&
	if ( /*(NPCInfo->special & NSP_CLOAKING) &&*/
		NPC->client->ps.powerups[PW_CLOAKED] )
	{
		NPCInfo->stats.earshot *= 5;
		NPCInfo->stats.visrange *= 5;
	}
	...
	if ( checkAlerts )
	{
		...
		if ( alertEvent >= 0 )
		{
			...
			if ( event->level >= AEL_DISCOVERED )
			{
				...
				if ( NPC_ValidEnemy( event->owner->enemy ) )
					return event->owner->enemy;
			}
		}
	}
...
//	}
	...
	if ( NPC->enemy && NPC->enemy->health > 0 && NPC->enemy->inuse )
	{
		return qtrue;
	}
	...
	if( newenemy/*NPC_ValidEnemy( newenemy )*/ )
	{
...
qboolean NPC_FacePosition( vec3_t position, qboolean doPitch )
{
	...
	else if ( NPC->client->NPC_class == CLASS_GALAKMECH )
	{
