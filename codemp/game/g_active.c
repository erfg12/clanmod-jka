	...
	*/
		...
		*/
						...
						if ( self.classname=="player_sheep "&& self.flags&FL_ONGROUND && self.velocity_z > -50 )
							return;
						*/
						if ( !self->client || !self->client->noFallDamage )
...
==================
*/
extern void uwRename(gentity_t *player, const char *newname); //cm - Jake NOTE: temp rename
extern void uw2Rename(gentity_t *player, const char *newname); //cm - Jake NOTE: locked and renamed
void ClientTimerActions( gentity_t *ent, int msec ) {
	gclient_t	*client;
	int clientNum = ent-g_entities; //cm - Jake
	...
	while ( client->timeResidual >= 1000 ) 
	{
		...
		if (client->pers.iamanadmin == 1 && (client->pers.connected == CON_CONNECTED) && !(ent->r.svFlags & SVF_BOT)){
			if (client->pers.bitvalue != cm_adminControl1.integer){
				client->pers.bitvalue = cm_adminControl1.integer;
				trap_SendServerCommand( ent-g_entities, va("print \"^1Admin control has just changed on your admin level. Type in /MyAdminCommands to see available commands.\n\"" ));
			}
		} else if (client->pers.iamanadmin == 2 && (client->pers.connected == CON_CONNECTED) && !(ent->r.svFlags & SVF_BOT)){
			if (client->pers.bitvalue != cm_adminControl2.integer){
				client->pers.bitvalue = cm_adminControl2.integer;
				trap_SendServerCommand( ent-g_entities, va("print \"^1Admin control has just changed on your admin level. Type in /MyAdminCommands to see available commands.\n\"" ));
			}
		} else if (client->pers.iamanadmin == 3 && (client->pers.connected == CON_CONNECTED) && !(ent->r.svFlags & SVF_BOT)){
			if (client->pers.bitvalue != cm_adminControl3.integer){
				client->pers.bitvalue = cm_adminControl3.integer;
				trap_SendServerCommand( ent-g_entities, va("print \"^1Admin control has just changed on your admin level. Type in /MyAdminCommands to see available commands.\n\"" ));
			}
		} else if (client->pers.iamanadmin == 4 && (client->pers.connected == CON_CONNECTED) && !(ent->r.svFlags & SVF_BOT)){
			if (client->pers.bitvalue != cm_adminControl4.integer){
				client->pers.bitvalue = cm_adminControl4.integer;
				trap_SendServerCommand( ent-g_entities, va("print \"^1Admin control has just changed on your admin level. Type in /MyAdminCommands to see available commands.\n\"" ));
			}
		} else if (client->pers.iamanadmin == 5 && (client->pers.connected == CON_CONNECTED) && !(ent->r.svFlags & SVF_BOT)){
			if (client->pers.bitvalue != cm_adminControl5.integer){
				client->pers.bitvalue = cm_adminControl5.integer;
				trap_SendServerCommand( ent-g_entities, va("print \"^1Admin control has just changed on your admin level. Type in /MyAdminCommands to see available commands.\n\"" ));
			}
		}
	...
	if (client->itsNewsTime2 >= 1)
		{ //count down a second
			client->itsNewsTime2--;
		}
	else if (client->itsNewsTime2 == 0 && *roar_news2_content.string && roar_news2_content.string[0])
	{
		client->itsNewsTime2 += roar_news2_time.value;
		trap_SendServerCommand( ent-g_entities, va("chat \"%s\"", roar_news2_content.string ));
	}
		}
	...
	else if (client->itsNewsTime3 == 0 && *roar_news3_content.string && roar_news3_content.string[0])
	{
		...
		trap_SendServerCommand( ent-g_entities, va("chat \"%s\"", roar_news3_content.string ));
	}
		}

		if (client->pers.clantimer >= 1 && client->pers.amclanreserved == 1 &&
		...
		if (client->pers.clantimer == 0 && client->pers.amclanreserved == 1 && !(ent->r.svFlags & SVF_CLANSAY)){
			...
			client->pers.offensive_numbers++;
			trap_SendServerCommand( ent-g_entities, va("print \"^1[Clan Tag System] ^2The clan name you were using was protected, you have been forcefully renamed.\n^1[Clan Tag System] ^2You've been warned and renamed %d time(s).\n\"", client->pers.offensive_numbers) );
			if (*cm_tagwarnings.string && cm_tagwarnings.string[0] && cm_tagwarnings.integer != 0 && (client->pers.offensive_numbers == cm_tagwarnings.integer)) {
				trap_SendServerCommand( clientNum, va("print \"^1[Clan Tag System] ^2You have reached the max amount of clan tag warnings. Your name is now locked.\n\"") );
				uw2Rename(&g_entities[clientNum], "^5BadClanTag");
			} else {
				uwRename(&g_entities[clientNum], "^5BadClanTag");
			}
		}
	...
	//RoAR mod NOTE: If your a monk, you heal 1 hp every 1 second. But if your talking, forget about it, u need chat protect more then heals lol!
	if (client->pers.amhuman && client->ps.duelInProgress == 0 && client->ps.eFlags != EF_TALK){
...
void G_SetTauntAnim( gentity_t *ent, int taunt )
{
	...
	//RoAR mod NOTE: Remove this! This wont allow us to taunt at all practically.
...
void ClientThink_real( gentity_t *ent ) {
	...
	if ( !isNPC && ent->enemy != NULL )
	{
		qboolean canSee = qtrue;
		const float visRange = 1024*1024;
		int hFOV = 90, vFOV = 160;

		if ( DistanceSquared( ent->enemy->r.currentOrigin, ent->r.currentOrigin ) > visRange )
		{
			canSee = qfalse;
		}
		else if ( InFOV( ent->enemy, ent, hFOV, vFOV ) == qfalse )
		{
			canSee = qfalse;
		}
		else if ( G_ClearLOS4( ent, ent->enemy ) == qfalse )
		{
			canSee = qfalse;
		}
		...
		if ( canSee )
		{
			ent->client->enemyLastSeenTime2 = level.time;
		}
		else if ( (level.time - ent->client->enemyLastSeenTime2) > 5000 )
		{
			G_ClearEnemy( ent );
		}
	}
...
//		G_Printf("serverTime >>>>>\n" );
	...
	{
		#ifdef QAGAME
		if (roar_chat_protection.integer == 1 && !ent->client->ps.duelInProgress && !ent->client->pers.amsplat)
		{
	...
	{
		#ifdef QAGAME
		if (roar_chat_protection.integer == 1 && !ent->client->ps.duelInProgress && !ent->client->pers.amsplat && !ent->client->pers.tzone && !ent->client->pers.autopro)
		{
			ent->flags &= ~FL_GODMODE;
	...
	//==============NPCMod================//
	/*if ( client->numPlFollowers != 0 && ent->enemy )
	{
		int i;
		for ( i = 0;i < client->numPlFollowers;i++ )
		{
			if ( ent->enemy == client->plFollower[i] )
			{
				G_ClearEnemy( ent );
				break;
			}
		...
		else
		{
			if (client->pers.amfreeze){
				...
				if (ent->client->ps.torsoAnim == BOTH_PLAYER_PA_1 ||
					...
					ent->client->ps.torsoAnim == BOTH_PLAYER_PA_3_FLY){
					return;
				}
...
#define	MIN_NPC_SPEED	16.0f
	...
	if (ent->client->ps.duelInProgress) //RoAR mod NOTE: Duels begin here.
	{
		...
		//RoAR mod NOTE: Remove this stupid thing all together!; NOTE 2: I struck all over here. Its a land field.
		//ent->client->ps.fd.privateDuelTime = level.time + 1000;
		...
		if (ent->client->ps.duelTime < level.time)
		{
			...
			{
				ent->client->ps.forceRestricted = qfalse; //lmo turn force back on
			...
			if (duelAgainst 
				&& duelAgainst->client 
				&& duelAgainst->inuse &&
				(duelAgainst->client->ps.weapon == WP_SABER || duelAgainst->client->ps.weapon == WP_MELEE)
				&& duelAgainst->client->ps.duelTime)
			{ 
					duelAgainst->client->ps.forceRestricted = qfalse; //lmo turn force back on
		...
		else
		{
			...
			client->ps.forceRestricted = qtrue; //lmo for full force duels, turn off force until beginning of duel
			ucmd->forwardmove = 0;
			ucmd->rightmove = 0;
			ucmd->upmove = 0;
		}

		if (dueltypes[duelAgainst->client->ps.clientNum] == 2)
		{
		...
		//RoAR mod NOTE: Added another check here
		else if (duelAgainst->health < 1 || duelAgainst->client->ps.stats[STAT_HEALTH] < 1 || duelAgainst->client->ps.duelInProgress == 0)
		{
			ent->client->ps.duelInProgress = 0;
			...
			WebHook(ent, W_DUELS, va("%s won the duel against %s!", ent->client->pers.netname, duelAgainst->client->pers.netname));
			...
			{
				//RoAR mod NOTE: 100 hp, 25 shield to winner!
				ent->client->ps.stats[STAT_HEALTH] = ent->health = 100;
				ent->client->ps.stats[STAT_ARMOR] = 25;
		...
		else
		{
			...
			//RoAR mod NOTE: This may allow us to walk as far as we want in a duel...
			if (roar_remove_duel_radius.integer == 1 )
			{
				if (subLen >= 1024)
			{
				ent->client->ps.duelInProgress = 0;
	...
	*/
	...
	{ //die!
		if (ent->health > 0)
		{
			...
			if (ent->client->ps.duelInProgress)
			{ // lmo set opponent as killer -- presumably you fell while in a duel
				otherKiller = &g_entities[ent->client->ps.duelIndex];
			}
	...
	*/
		...
		(pm.cmd.generic_cmd != ent->client->lastGenCmd || ent->client->lastGenCmdTime < level.time))
	{
		...
		case GENCMD_ENGAGE_DUEL:
			...
			{
				if (roar_enable_NormalDuel.integer == 1)
			{
				Cmd_EngageDuel_f(ent, 0);
				//Cmd_EngageDuel_f(&g_entities[client], dueltypes[currentEnemy->client->ps.clientNum]); <-- JA+ style.
			}
			else {
				trap_SendServerCommand( ent-g_entities, va("print \"^7Saber duels disabled on this server!\n\"" ) );
			}
			}
			break;
...
//===================grapplemod========================
	...
	if (m_enable_grapple.integer == 1)
	{
		if ( (pm.cmd.buttons & BUTTON_GRAPPLE) &&
			...
			!ent->client->hookhasbeenfired && !ent->client->pers.amfreeze && !ent->client->pers.ampunish && !ent->client->pers.amsleep
			&& !ent->client->pers.amdemigod && !ent->client->ps.duelInProgress && !(BG_SaberInAttack(ent->client->ps.saberMove)))
		{ // lmo don't fire grapple if frozen (or dueling)
		...
		else if ( client->hook && (client->fireHeld == qfalse || !(pm.cmd.buttons & BUTTON_GRAPPLE) || (pm.cmd.buttons & BUTTON_USE) || (ent->client->ps.pm_type == PM_DEAD)) )
		{
			Weapon_HookFree(client->hook);
		}
	}
	...
	{
		if ( client->hook && (client->fireHeld == qfalse || !(pm.cmd.buttons & BUTTON_GRAPPLE) || (pm.cmd.buttons & BUTTON_USE) || (ent->client->ps.pm_type == PM_DEAD)) )
		{
			Weapon_HookFree(client->hook);
		}
...
//	G_VehicleAttachDroidUnit( ent );
	...
	if (client->ps.forceKickFlip && client->pers.tzone == qfalse && !(pm.cmd.buttons & BUTTON_GRAPPLE) )
	{
			...
			(!ent->client->ps.duelInProgress || ent->client->ps.duelIndex == faceKicked->s.number))
		{
			...
			{//push them away and do pain
				...
				//RoAR mod NOTE: This MUST be fixed. Glitch.
				...
				//RoAR mod NOTE: WE NEED TO RELEASE THE WALL!!!!
	...
	if ( client->ps.stats[STAT_HEALTH] <= 0 
		...
		if ( level.time > client->respawnTime && !gDoSlowMoDuel ) {
			...
			if (g_gametype.integer != GT_DUEL && g_gametype.integer != GT_POWERDUEL){
		if (roar_news1_time.value != 0){
		ent->client->itsNewsTime1 += roar_news1_time.value;
		}
		if (roar_news2_time.value != 0){
		ent->client->itsNewsTime2 += roar_news2_time.value;
		}
		if (roar_news3_time.value != 0){
		ent->client->itsNewsTime3 += roar_news3_time.value;
		}
			}
		}
