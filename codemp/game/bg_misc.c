...
qboolean BG_LegalizedForcePowers(char *powerOut, int maxRank, qboolean freeSaber, int teamForce, int gametype, int fpDisabled)
{
	...
	while (i < NUM_FORCE_POWERS)
	{
		...
		//cm - Jake NOTE: http://lucasforums.com/showthread.php?t=175392
		if(countDown > 3)
		{
			return -1;
		}
...
#ifndef UI_EXPORTS // ui compile breaks cause it can't see the dueltypes array
					// however, this function is not used by the UI so ifdef it out
					//// lmo
#ifndef QAGAME
...
/*#ifndef QAGAME
extern int cg_Tforce[MAX_CLIENTS];
#endif*/
...
qboolean BG_CanUseFPNow(int gametype, playerState_t *ps, int time, forcePowers_t power)
{
	...
	if (ps->duelInProgress) // consider duel types.
	{
		...
		// obtain duel type -- this should be set by events in the client, directly in the server
#ifdef QAGAME
		...
		case 3: //melee duel
			...
			default: //wtf mate, what's this mean?
			break;
		}
	}
...
qboolean BG_CanItemBeGrabbed( int gametype, const entityState_t *ent, const playerState_t *ps ) {
	...
	if ( ps )
	{
		...
		//RoAR mod NOTE: Monks cant pick up weapons!
		if (ps->persistant[PERS_MONK] == 1 && item && (item->giType == IT_WEAPON || item->giType == IT_AMMO || item->giType == IT_POWERUP)) {
			return qfalse;
		}
		//
...
void BG_PlayerStateToEntityState( playerState_t *ps, entityState_t *s, qboolean snap ) {
	...
	if (ps->duelInProgress)
	{
		s->bolt1 = ps->duelIndex + 1; // use client num +1 as indicator
	}
	else
	{
		s->bolt1 = 0;
