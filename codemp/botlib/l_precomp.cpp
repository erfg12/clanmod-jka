...
int PC_Directive_include(source_t *source)
{
	...
	else if (token.type == TT_PUNCTUATION && *token.string == '<')
	{
		...
		while(PC_ReadSourceToken(source, &token))
		{
			...
			strncat(path, token.string, MAX_PATH2);
		} //end while
		if (*token.string != '>')
		{
	...
	if (!script)
	{
		...
		script = LoadScriptFile(path);
		if (script) strncpy(script->filename, path, MAX_PATH2);
	} //end if
#endif //QUAKE
	if (!script)
	{
...
define_t *PC_DefineFromString(char *string)
{
	...
	strncpy(src.filename, "*extern", MAX_PATH2);
...
{
	strncpy(source->includepath, path, MAX_PATH2);
...
source_t *LoadSourceFile(const char *filename)
{
	...
	strncpy(source->filename, filename, MAX_PATH2);
	source->scriptstack = script;
	source->tokens = NULL;
	source->defines = NULL;
...
source_t *LoadSourceMemory(char *ptr, int length, char *name)
{
	...
	strncpy(source->filename, name, MAX_PATH2);
	source->scriptstack = script;
	source->tokens = NULL;
	source->defines = NULL;
	source->indentstack = NULL;
