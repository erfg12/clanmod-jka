...
extern float RandFloatMissle(float min, float max);
void G_ReflectMissile( gentity_t *ent, gentity_t *missile, vec3_t forward ) 
{
	...
	for ( i = 0; i < 3; i++ )
	{
		...
		bounce_dir[i] += RandFloatMissle( -0.2f, 0.2f );
	}
...
void G_DeflectMissile( gentity_t *ent, gentity_t *missile, vec3_t forward ) 
{
	...
	for ( i = 0; i < 3; i++ )
	{
		...
		// MJN - Missle Fix
		bounce_dir[i] += RandFloatMissle( -1.0f, 1.0f );
	}
...
void G_MissileImpact( gentity_t *ent, trace_t *trace ) {
	...
	*/
	...
	{ //only block one projectile per 200ms (to prevent giant swarms of projectiles being blocked)
		...
		te->s.weapon = 0;//saberNum
		te->s.legsAnim = 0;//bladeNum
		te->s.otherEntityNum = ent->r.ownerNum; //lmo hack for sending owner info with sound event entity
...
//		if ( other->takedamage && other->client ) {
		...
		{ // lmo attach and mark enemy if client or mover (for tracking position)

			G_AddEvent( nent, EV_MISSILE_HIT, DirToByte( trace->plane.normal ) );
			nent->s.otherEntityNum2 = other->s.number;
// done when initialized			nent->s.otherEntityNum = ent->r.ownerNum; //lmo tag owner and other info for dNOX
