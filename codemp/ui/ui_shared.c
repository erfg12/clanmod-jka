...
//mp only
	...
	{"voicechat",		-1,					-1,		-1,	-1},
	//RoAR mod BEGIN
	{"engage_meleeduel",		-1,					-1,		-1,	-1},
	{"engage_forceduel",		-1,					-1,		-1,	-1},
	{"engage_trainingduel",		-1,					-1,		-1,	-1},
	{"jetpack",		-1,					-1,		-1,	-1},
	{"+button12",		-1,					-1,		-1,	-1},
	{"knockmedown",		-1,					-1,		-1,	-1},
	{"drop",		-1,					-1,		-1,	-1},
	{"ordernpc follow",		-1,					-1,		-1,	-1},
	{"ordernpc attack",		-1,					-1,		-1,	-1},
	{"ordernpc defend",		-1,					-1,		-1,	-1},
	//RoAR mod END
...
// Item_Buyable_Paint (-at-)
void Item_Buyable_Paint(itemDef_t *item) {
	char text[1024];
	const char *textPtr;
	int height, width, textWidth;
	vec4_t color;
	...
	if (*textPtr != '\0') {
		Item_TextColor(item, &color);
		DC->drawText( item->textRect.x + item->textalignx, item->textRect.y + item->textaligny, item->textscale, color, textPtr, 0, 0, item->textStyle, item->iMenuFont );
	}

	// this needs to go here as it sets extents for cvar types as well
	Item_SetTextExtents(item, &width, &height, textPtr);

	if (item->descText)	// is there a description line?
	{
		...
		Item_TextColor(item, &color);
		DC->drawText(item->textRect.x + item->textalignx, item->textRect.y + item->text2aligny, item->textscale * 0.75, color, textPtr, 0, 0, item->textStyle,item->iMenuFont);
	}

	if (item->text2)	// is there a price?
	{
		textPtr = item->text2;
		if (*textPtr == '@')	// string reference
		{
		...
		textWidth = DC->textWidth(textPtr, item->textscale, item->iMenuFont);

		DC->drawText(item->textRect.x + item->text2alignx - textWidth, item->textRect.y + item->textaligny, item->textscale, color, textPtr, 0, 0, item->textStyle,item->iMenuFont);
	}

	DC->drawHandlePic( item->textRect.x - item->special / 2, item->textRect.y, item->special, item->special, item->window.background );
...
//JLF end transition stuff for models
  ...
  switch (item->type) {
    ...
    case ITEM_TYPE_SLIDER:
      ...
      break;
  case ITEM_TYPE_BUYABLE:
	  Item_Buyable_Paint(item);
    default:
      break;
  }
