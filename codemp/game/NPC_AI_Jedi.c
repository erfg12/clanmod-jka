...
extern qboolean WP_ForcePowerInUse( gentity_t *self, forcePowers_t forcePower );
...
extern void ST_ResolveBlockedShot( int hit );
void Jedi_NoSaberAttackDecide( void )
{
	vec3_t	enemyDir, shootDir;
	vec3_t	enemyTarget, spotMuzzle;
	qboolean enemyLOS;
	qboolean enemyCS;
	qboolean enemyInFOV;
	qboolean hitAlly;
	...
	if ( NPCInfo->shotTime > level.time ||
		 !TIMER_Done( NPC, "gripping" ) ||
		 !TIMER_Done( NPC, "draining" ) ||
		 !TIMER_Done( NPC, "holdLightning" ) )
		 return;
	...
	CalcEntitySpot( NPC, SPOT_WEAPON, spotMuzzle );
	CalcEntitySpot( NPC->enemy, SPOT_CHEST, enemyTarget );

	VectorSubtract( enemyTarget/*NPC->enemy->r.currentOrigin*/, spotMuzzle/*NPC->r.currentOrigin*/, enemyDir );
...
{
	G_SoundIndex( "sound/chars/boba/bf_blast-off.mp3" );
	//G_SoundIndex( "sound/chars/boba/jethover.wav" );
	G_SoundIndex( "sound/chars/boba/bf_land.mp3" );
	G_SoundIndex( "sound/effects/fire_lp" );
	G_SoundIndex( "sound/effects/fireout.mp3" );
...
extern void BG_ContinueLegsAnim( playerState_t *ps, int anim );
...
void Boba_SetJetpackAnims( gentity_t *self )
{
	...
	//Pas pour les rockettroopers, les vrais
	if ( self->localAnimIndex != 0 )
		return;

	if ( self->client->ps.legsTimer > 0 /*|| ( BG_JetpackAnim(self->client->ps.torsoAnim) && self->client->ps.torsoTimer > 0 )*/ )
		return;

	//si vélocity est quasi nul, on skippe
	if ( VectorLengthSquared( NPC->client->ps.velocity ) < 2048/*4096*/ )
		return;

	vectoangles( NPC->client->ps.velocity, angs );
	angs[YAW] -= NPC->client->ps.viewangles[YAW];
	...
	/*if (TIMER_Done(self,"jetAnim"))
	{
		self->NPC->rightPriority = Q_irand(qfalse,qtrue);
		TIMER_Set(self,"jetAnim",Q_irand(4000,6500));
		//return;
	}*/
	...
	/*if ((self->client->ps.weaponstate != WEAPON_RAISING) &&
		...
		(self->client->ps.weaponstate != WEAPON_CHARGING) &&
		(self->client->ps.weaponstate != WEAPON_CHARGING_ALT) &&
		(self->client->ps.weaponTime <= 0) && !(ucmd.buttons & (BUTTON_ATTACK|BUTTON_ALT_ATTACK)))
	{
		if (!self->NPC->rightPriority)
		{
			if (forwardmove > 0)
			{
				BG_ContinueLegsAnim(&self->client->ps,BOTH_JETPACK_F);
				if ((self->client->ps.torsoAnim) != BOTH_JETPACK_F)
					NPC_SetAnim(self,SETANIM_TORSO,BOTH_JETPACK_F,SETANIM_FLAG_HOLD);
			}
			else if (forwardmove < 0)
			{
				BG_ContinueLegsAnim(&self->client->ps,BOTH_JETPACK_B);
				if ((self->client->ps.torsoAnim) != BOTH_JETPACK_B)
					NPC_SetAnim(self,SETANIM_TORSO,BOTH_JETPACK_B,SETANIM_FLAG_HOLD);
			}
			else if (rightmove > 0)
			{
				BG_ContinueLegsAnim(&self->client->ps,BOTH_FORCEJUMPRIGHT1);
				if ((self->client->ps.torsoAnim) != BOTH_FORCEJUMPRIGHT1)
					NPC_SetAnim(self,SETANIM_TORSO,BOTH_FORCEJUMPRIGHT1,SETANIM_FLAG_HOLD);
			}
			else if (rightmove < 0)
			{
				BG_ContinueLegsAnim(&self->client->ps,BOTH_FORCEJUMPLEFT1);
				if ((self->client->ps.torsoAnim) != BOTH_FORCEJUMPLEFT1)
					NPC_SetAnim(self,SETANIM_TORSO,BOTH_FORCEJUMPLEFT1,SETANIM_FLAG_HOLD);
			}
			else
			{
				if (BG_StandAnim(self->client->ps.torsoAnim))
				{
					BG_ContinueLegsAnim(&self->client->ps,BOTH_CEILING_CLING);
					if ((self->client->ps.torsoAnim) != BOTH_CEILING_CLING)
						NPC_SetAnim(self,SETANIM_TORSO,BOTH_CEILING_CLING,SETANIM_FLAG_NORMAL);
				}
			}
		}
		else
	...
	else if ((self->client->ps.weaponstate == WEAPON_FIRING) ||
			 (level.time < self->client->ps.weaponTime))
	{
		if (!self->NPC->rightPriority)
		{
			if (forwardmove > 0)
	...
	NPC->client->ps.legsTimer = 2500;*/
}

void Boba_FlyStart( gentity_t *self, int jetTime )
...
void Boba_FlyStop( gentity_t *self )
{
	...
	if ( self->NPC )
	{
		...
		TIMER_Set( self, "jetRecharge", Q_irand( 2500, 5000 ) );
		TIMER_Set( self, "jumpChaseDebounce", Q_irand( 500, 2000 ) );
	}

	G_Sound( self, CHAN_ITEM, G_SoundIndex("sound/chars/boba/bf_land.mp3") );
}
...
{
	int		damage	= Q_irand( 5, 8 );
	trace_t		tr;
	gentity_t	*traceEnt = NULL;
	mdxaBone_t	boltMatrix;
	vec3_t		start, end, dir, traceMins = {-64, -64, -64}, traceMaxs = {64, 64, 64};
	...
	G_PlayEffectID( G_EffectIndex("boba/fthrw"), start, dir);
	
	G_Sound( self, CHAN_WEAPON, G_SoundIndex("sound/effects/fire_lp") );
	VectorMA( start, 768, dir, end );
...
void Boba_StartFlameThrower( gentity_t *self )
{
	...
	flameTime = Q_irand( 3000, 4000 );

	self->client->ps.torsoTimer = flameTime;//+1000;
	if ( self->NPC )
	{
		TIMER_Set( self, "nextAttackDelay", flameTime );
		TIMER_Set( self, "nextFlameDelay", flameTime + Q_irand( 9000, 13000 ));
		TIMER_Set( self, "walking", 0 );
	}
	TIMER_Set( self, "flameTime", flameTime );

	G_SoundOnEnt( self, CHAN_WEAPON, "sound/effects/fireout.mp3" );
}
...
void Boba_DoFlameThrower( gentity_t *self )
{
	...
	if ( TIMER_Done( self, "nextAttackDelay" ) && TIMER_Done( self, "flameTime" ) && TIMER_Done( NPC, "nextFlameDelay" ) )
	{
...
float G_GroundDistance( gentity_t *self )
{
	...
	trap_Trace( &tr, self->r.currentOrigin, self->r.mins, self->r.maxs, down, self->s.clientNum, MASK_SOLID );

	VectorSubtract( self->r.currentOrigin, tr.endpos, down );

	if ( tr.entityNum != ENTITYNUM_NONE && g_entities[tr.entityNum].classname && !Q_stricmp(g_entities[tr.entityNum].classname,"trigger_hurt") )
		return Q3_INFINITE;
...
void Boba_JetpackDecideDefault( void )
{
	...
	gDist = G_GroundDistance( NPC );

	if (!NPC->client->playerLeader)
	{
		if (!NPC->enemy)
			Boba_FlyThink( NPC );
		return;
	...
	if ( NPC->client->playerLeader->client->jetPackOn && NPCInfo->defaultBehavior == BS_FOLLOW_LEADER )
	{
		if ( TIMER_Done(NPC,"jetpackGoGo") )
		{
			int r = Q_irand( 4000, 6500 );

			if ( NPC->NPC->shouldJetOn )
			{
				ucmd.rightmove = ucmd.forwardmove = 0;
				ucmd.upmove = 127;
				Boba_ForceFlyStart( NPC, r );
				NPC->NPC->shouldJetOn = qfalse;
			}
			else
			{
				NPC->client->jetPackTime = level.time + r;
			}

			TIMER_Set( NPC, "jetpackGoGo", r - (FRAMETIME*2) );
		}
	}
	else
	{
		if ( NPC->client->ps.groundEntityNum == ENTITYNUM_NONE 
			...
			&& !Boba_Flying( NPC ) )
		{//take off
			Boba_FlyStart( NPC, Q_irand( 2500, 5000 ) );
		}
		else if ((gDist > 512) && !Boba_Flying( NPC ))
		{
			Boba_ForceFlyStart( NPC, Q_irand( 2500, 5000 ) );
		}
...
void Boba_JetpackDecideCombat( void )
{
	...
	if ( NPC->client->ps.groundEntityNum == ENTITYNUM_NONE 
		...
		&& !Boba_Flying( NPC ) )
	{//take off
		Boba_FlyStart( NPC, Q_irand( 2000, 10000 )*NPCInfo->stats.move );
	}
	else if ((gDist > 512) && !Boba_Flying( NPC ))
	{
		Boba_ForceFlyStart( NPC, Q_irand( 2000, 10000 )*NPCInfo->stats.move );
	}
...
{
	if ( TIMER_Done( NPC, "nextWeaponDelay" ) )
	{
		if (Q_irand(0,1))
		...
		while(1)
		{
			//FIXME : signifie que si le NPC a WP_NONE plus dautres armes, il y a une chance quil prenne WP_NONE, c léger con
		...
		TIMER_Set( NPC, "nextWeaponDelay", Q_irand(5000,12000) );
	}
}
...
void Boba_FireDecide( void )
{
	...
	if ( !NPC->client->ps.stats[STAT_WEAPONS] || NPC->client->ps.weapon == WP_NONE )
	{
	...
	if ( ((enemyDist < (128*128)&&enemyInFOV) && TIMER_Done( NPC, "nextFlameDelay" )) || !TIMER_Done( NPC, "flameTime" ) )
	{//flamethrower
				...
				&& NPCInfo->enemyLastSeenTime > 0 )//we've seen the enemy
			{
				if ( level.time - NPCInfo->enemyLastSeenTime < 10000 )//we have seem the enemy in the last 10 seconds
				{
					if ( !Q_irand( 0, 10 ) )
					{
						...
						CalcEntitySpot( NPC, SPOT_WEAPON/*HEAD*/, muzzle );
						if ( VectorCompare( impactPos, vec3_origin ) )
						{//never checked ShotEntity this frame, so must do a trace...
							trace_t tr;
							//vec3_t	mins = {-2,-2,-2}, maxs = {2,2,2};
...
static void Jedi_Aggression( gentity_t *self, int change )
{
	...
	int		min_aggression = 1, max_aggression = 10;

	self->NPC->jediAggression += change;

	if ( self->NPC->jediAggression > max_aggression )
		self->NPC->jediAggression = max_aggression;
	else if ( self->NPC->jediAggression < min_aggression )
		self->NPC->jediAggression = min_aggression;

	return;

	//NPCMod : ça devrait fixer les crises de BlackHunter et autres, mais on perd en fun, il faudra voir
...
static qboolean Jedi_BattleTaunt( void )
{
		...
		&& jediSpeechDebounceTime[NPC->client->playerTeam] < level.time )
	{
		...
		if ( /*NPC->client->playerTeam == NPCTEAM_PLAYER 
			&& */NPC->enemy && NPC->enemy->client && NPC->client->NPC_class == CLASS_JEDI /*&& NPC->enemy->client->NPC_class == CLASS_JEDI*/ )
		...
		if ( event != -1 )
		{
			...
			if ( NPC->client->ps.weapon == WP_SABER )
				...
				TIMER_Set( NPC, "chatter", Q_irand( 9000, 17000 ) );
...
static qboolean Jedi_Hunt( void )
{
	...
	{//approach enemy
		...
		// c surtout pour les disrupteurs
		else if ( NPC->client->NPC_class != CLASS_COMMANDO )
		{
...
{	
	...
	if ( self->client->NPC_class == CLASS_TAVION && Q_irand(0,1) )
	{//special attacks
		self->client->ps.fd.saberAnimLevel = FORCE_LEVEL_5;
		return;
	}
	else if ( self->client->NPC_class == CLASS_DESANN && Q_irand(0,1) )
	{//special attacks
...
static void Jedi_CheckDecreaseSaberAnimLevel( void )
{
	...
	{//not attacking
		if ( TIMER_Done( NPC, "saberLevelDebounce" ) && !Q_irand( 0, 1 ) )
		{
...
static void Jedi_FootKnockdown( void )
{
	if ( NPC->client->ps.weapon != WP_SABER )
		return;

	if ( NPC->client->saber[0].type != SABER_STAFF 
		&& !(NPC->client->saber[0].saberFlags&SFL_NOT_THROWABLE) )
		return;
...
extern void NPC_BSSniper_Attack( void );
void Jedi_CombatDistance( int enemy_dist )
{//FIXME: for many of these checks, what we really want is horizontal distance to enemy
	...
	{//maintain distance
		...
		{
			if ( !( !NPC->client->ps.saberEntityNum && NPC->client->saberStoredIndex && NPC->client->ps.saberInFlight ) )
				Jedi_Advance();
		}
	...
	{
		if ( enemy_dist <= 64 && ( /*NPC->client->NPC_class != CLASS_ALORA || */NPC->health < ( NPC->client->pers.maxHealth / 4 ) ) )
	...
	*/
	...
	{//can't use saber and they're in striking range
		if ( !Q_irand( 0, 5 ) && InFront( NPC->enemy->r.currentOrigin, NPC->r.currentOrigin, NPC->client->ps.viewangles, 0.2f ) )
		{
			...
			{//drain
				...
				TIMER_Set( NPC, "attackDelay", 3100 );
				Jedi_Advance();
				return;
			}
			else
	...
	{//we're too damn close!
		...
		else if ( NPCInfo->stats.aggression < 2 )
		{//back off and defend
			Jedi_Retreat();
		}
		else if ( Q_irand( 0, 3 ) )
	...
	else if ( enemy_dist <= 200 && !IsMeleeWeapon( NPC->client->ps.weapon ) )
	{//we're within striking range
		if ( Q_irand(0,6) )
			Jedi_Retreat();
	}
	...
	*/
	...
	{//we're out of striking range and we are allowed to attack
		...
		{//They're being gripped, rush them!
			...
			if ( NPCInfo->rank >= RANK_LT_JG 
				...
				&& !(NPC->client->ps.fd.forcePowersActive&(1 << FP_SPEED))
				&& !(NPC->client->ps.saberEventFlags&SEF_INWATER)//saber not in water
				&& NPC->client->saber[0].type != SABER_STAFF )
			{//throw saber
				ucmd.buttons |= BUTTON_ALT_ATTACK;
			}
		}
		...
		{//They're throwing their saber, grip them!
			...
			TIMER_Set( NPC, "attackDelay", 3000 + 100 );
		}
		else
		{
			int chanceScale;
			...
			else if ( NPCInfo->rank == RANK_ENSIGN  || (NPC->client->ps.weapon == WP_MELEE || NPC->client->ps.weapon == WP_NONE))
			{
				chanceScale = 2;
			}
			...
			if ( chanceScale 
				&& (enemy_dist > Q_irand( 100, 200 ) || (NPCInfo->scriptFlags&SCF_DONT_FIRE) || (!Q_stricmp("Yoda",NPC->NPC_type)&&!Q_irand(0,3)) || (NPC->client->NPC_class == CLASS_REBORN && NPC->client->ps.weapon != WP_SABER) )
				&& enemy_dist < 500 
			...
			{//else, randomly try some kind of attack every now and then
				if ( ((NPCInfo->rank == RANK_ENSIGN || NPCInfo->rank > RANK_LT_JG) && !Q_irand( 0, 1 )) ||
					 (NPC->client->ps.weapon == WP_MELEE || NPC->client->ps.weapon == WP_NONE) )
				{
					if ( WP_ForcePowerAvailable( NPC, FP_PULL, 0 ) && !Q_irand( 0, 2 ) )
					{
						...
						//maybe strafe too?
						if ( Q_irand(0,1) )
							TIMER_Set( NPC, "duck", enemy_dist*Q_irand(1,3) );
						/*if ( Q_irand( 0, 1 ) )
						{
							ucmd.buttons |= BUTTON_ATTACK;
						}*/
					...
					else if ( WP_ForcePowerAvailable( NPC, FP_LIGHTNING, 0 ) && Q_irand( 0, 1 ) )
					{
						...
						TIMER_Set( NPC, "attackDelay", NPC->client->ps.weaponTime + 100 );
					}
			...
			else if ( !Q_irand( 0, 6 - NPCInfo->stats.aggression ) )
			{//approach enemy
				if ( TIMER_Done( NPC, "parryTime" ) || NPCInfo->rank > RANK_LT )
				{//not parrying
			...
			{//maintain this distance?
				...
				if ( Q_irand(0,10) )
				{
					Jedi_Strafe( 1500,3000,4000,5500, Q_irand(qfalse,qtrue));
				}
				else
				{
					Jedi_Advance();
	...
	{//we're not close enough to attack, but not far enough away to be safe
		if ( !IsMeleeWeapon( NPC->client->ps.weapon ) )
		{
			if ( Q_irand(0,1) || NPCInfo->stats.aggression < 2 )
				Jedi_Retreat();
		}
		//else if ( NPCInfo->stats.aggression < 2 && !Q_irand(0,3) )
		//{//back off and defend
		//	Jedi_Retreat();
		//}
		else if ( Q_irand( 0, 3 - NPCInfo->stats.aggression ) <= 0 )
		{//try to get closer
	...
	if ( NPCInfo->stats.intelligence/*aggression*/ > Q_irand( 2, 4 )
		&& NPC->health < NPC->client->pers.maxHealth*0.75f 
		&& !Q_irand( 0, 2 )
		&& TIMER_Done( NPC, "combatDistanceEndPower" ) )
	{
		//int	lucky = 9 * ( NPC->health / NPC->client->pers.maxHealth );
	
		/*if ( (NPC->client->ps.fd.forcePowersKnown&(1<<FP_SPEED)) != 0 
			&& (NPC->client->ps.fd.forcePowersActive&(1<<FP_SPEED)) == 0
			&& !Q_irand(0,3))*/
		if ( WP_ForcePowerAvailable( NPC, FP_SPEED, qfalse ) && 
			 WP_ForcePowerUsable( NPC, FP_SPEED) &&  
			 !WP_ForcePowerInUse( NPC, FP_RAGE) && 
			 !WP_ForcePowerInUse( NPC, FP_SPEED)
			 && !Q_irand(0,3))
		{
			ForceSpeed( NPC, 0 );
		}
		else if ( WP_ForcePowerAvailable( NPC, FP_PROTECT, qfalse ) && 
			 WP_ForcePowerUsable( NPC, FP_PROTECT) &&  
			 !WP_ForcePowerInUse( NPC, FP_RAGE) && 
			 !WP_ForcePowerInUse( NPC, FP_PROTECT)
			 && !Q_irand(0,3))
		{
			ForceProtect( NPC );
		}
		else if ( WP_ForcePowerAvailable( NPC, FP_RAGE, qfalse ) && 
			 WP_ForcePowerUsable( NPC, FP_RAGE) && 
			 !WP_ForcePowerInUse( NPC, FP_RAGE) && 
			 !WP_ForcePowerInUse( NPC, FP_SPEED) &&
			 !WP_ForcePowerInUse( NPC, FP_PROTECT) &&
			 !WP_ForcePowerInUse( NPC, FP_ABSORB)
			 && !Q_irand(0,3))
		{
			Jedi_Rage();
		}

		TIMER_Set( NPC, "combatDistanceEndPower", Q_irand( 3500, 6500 ) );
	}
}
...
evasionType_t Jedi_CheckFlipEvasions( gentity_t *self, float rightdot, float zdiff )
{
	...
	{//already running on a wall
		...
		//&& Q_irand( 0, 1 )
		&& Q_irand( 0, (5-self->NPC->stats.evasion+RANK_COMMANDER-self->NPC->rank) ) <= 0
...
{
	if ( ( self->client->NPC_class == CLASS_JEDI && self->NPC->rank == RANK_CAPTAIN ) ||
		self->client->NPC_class == CLASS_TAVION ||
		self->client->NPC_class == CLASS_LUKE/* ||
		( self->client->NPC_class == CLASS_ALORA && self->NPC->stats.reactions >= 3 )*/ ||
		self->NPC->stats.reactions >= 5 )
	{
		return qtrue;
	}
	return qfalse;
...
evasionType_t Jedi_SaberBlockGo( gentity_t *self, usercmd_t *cmd, vec3_t pHitloc, vec3_t phitDir, gentity_t *incoming, float dist ) //dist = 0.0f
{
	...
	{
		if ( incoming->s.weapon == WP_SABER && incoming->r.ownerNum != ENTITYNUM_NONE )
		{//flying lightsaber, face it!
			gentity_t *saberOwn = &g_entities[incoming->r.ownerNum];
			...
			//sabre désactivé, pas la peine de sautiller comme un bouley
			if ( !saberOwn || !saberOwn->client->ps.saberEntityNum )
			{
				return EVASION_NONE;
			}

			//FIXME: for this to actually work, we'd need to call update angles too?
			//NPC_FaceEntity( /*self, */incoming, qtrue );
		}
		VectorCopy( incoming->r.currentOrigin, hitloc );
		VectorNormalize2( incoming->s.pos.trDelta, hitdir );
	}
	if ( self->client && (self->client->NPC_class == CLASS_BOBAFETT||self->client->NPC_class == CLASS_COMMANDO) )
	{
		saberBusy = qtrue;
	}
	...
	{//either it will miss by a bit (and 25% chance) OR our saber is not in-hand OR saber is off
		if ( self->NPC && (/*self->NPC->rank == RANK_CREWMAN || */Q_irand(0,self->NPC->rank-RANK_CAPTAIN) >= 0 || Q_irand(0,9-self->NPC->stats.evasion) <= 0 ) )
	...
	if ( zdiff >= -5 )//was 0
	{
		if ( incoming || !saberBusy )
		{
			...
			{//coming from right
				if ( doDodge )
				{
					if ( (self->client->NPC_class == CLASS_BOBAFETT && !Q_irand( 0, 2 ))
						|| (self->client->NPC_class == CLASS_COMMANDO && !Q_irand( 0, 4 )) ) //les commandos préfèrent se la joueur Matrix
					{//roll!
			...
			{//coming from left
				...
				{
					if ( (self->client->NPC_class == CLASS_BOBAFETT && !Q_irand( 0, 2 ))
						|| (self->client->NPC_class == CLASS_COMMANDO && !Q_irand( 0, 4 )) )
					{//roll!
						TIMER_Start( self, "duck", Q_irand( 500, 1500 ) );
	...
	else if ( zdiff > -22 )//was-15 )
	{
		...
		if ( incoming || !saberBusy )
		{
			if ( rightdot > 8 || (rightdot > 3 && zdiff < -11) )//was normalized, 0.2
			{
				...
				{
					if ( (self->client->NPC_class == CLASS_BOBAFETT && !Q_irand( 0, 2 ))
						|| (self->client->NPC_class == CLASS_COMMANDO && !Q_irand( 0, 1 )) )	//tt le contraire, le manque de rolls est frappant sinon pour les commandos
					{//roll!
						TIMER_Start( self, "strafeLeft", Q_irand( 500, 1500 ) );
						TIMER_Set( self, "strafeRight", 0 );
					}
			...
			else if ( rightdot < -8 || (rightdot < -3 && zdiff < -11) )//was normalized, -0.2
			{
				...
				{
					if ( (self->client->NPC_class == CLASS_BOBAFETT && !Q_irand( 0, 2 ))
						|| (self->client->NPC_class == CLASS_COMMANDO && !Q_irand( 0, 1 )) )
					{//roll!
						TIMER_Start( self, "strafeLeft", Q_irand( 500, 1500 ) );
						TIMER_Set( self, "strafeRight", 0 );
					}
		...
		{//gotta jump!
			...
			{//normal jump
				...
				if ( self->NPC 
					...
					&& !(self->client->ps.fd.forcePowersActive&(1<<FP_RAGE)) )
				{
					if ( (self->client->NPC_class == CLASS_BOBAFETT || self->client->NPC_class == CLASS_COMMANDO) && !Q_irand( 0, 1 ) )
					{//roll!
						if ( rightdot > 0 )
						{
							TIMER_Start( self, "strafeLeft", Q_irand( 500, 1500 ) );
				...
				//else if ( self->client->NPC_class == CLASS_ALORA )
				//{
				//	if ( !incoming 
				//		&& self->client->ps.groundEntityNum < ENTITYNUM_NONE 
				//		&& Q_irand( 0, 2 ) )
				...
				//		{//do the special jump!
				//			int aloraJumpAnim;
				//			cmd->forwardmove = Q_irand(0,1)?127:-127;
				//			if ( cmd->forwardmove >= 0 )
				//			{
				//				aloraJumpAnim = BOTH_ALORA_FLIP_1 + Q_irand( 0, 2 );
				//			}
				//			else
				//			{
				//				aloraJumpAnim = BOTH_ALORA_FLIP_B;
				//			}
				//			evasionType = EVASION_CARTWHEEL;
				//			NPC_SetAnim( self, SETANIM_BOTH, aloraJumpAnim, SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD );
				//			self->client->ps.velocity[2] = 225;
				//			//self->client->ps.forceHandExtendTime = level.time + 3000;
				//			self->client->ps.fd.forceJumpZStart = self->r.currentOrigin[2];//so we don't take damage if we land at same height
				//			G_Sound( self, CHAN_BODY, G_SoundIndex("sound/weapons/force/jump.wav") );
				//			cmd->rightmove = cmd->upmove = 0;
				//			saberBusy = qtrue;
				//			evaded = qtrue;
				//		}
				//	}
	...
	else
	{
		if ( incoming || !saberBusy )
		{
			...
			{//thrown saber!
				gentity_t	*owner = &g_entities[incoming->s.owner];

				//Signifie que le sabre est tombé et inactif, pas besoin de lesquiver
				if ( !( owner->client->ps.saberInFlight && !owner->client->ps.saberEntityNum && owner->client->saberStoredIndex) )
				{
...
//	VectorCopy(saberTip, saberTipOld);
	...
	{//we did block (not dodge)
		...
		{
			if ( NPC->client->NPC_class == CLASS_TAVION /*|| NPC->client->NPC_class == CLASS_ALORA*/ )
			{
	...
	else
	{
		...
		if ( NPC->client->NPC_class == CLASS_COMMANDO )
		{
			dodgeTime -= 200;	//ils deviennent teigneux là
		}
...
void Jedi_EvasionSaber( vec3_t enemy_movedir, float enemy_dist, vec3_t enemy_dir )
{
	...
	{//check to see if he's coming at me
		...
		{//coming at/facing me!  
			int whichDefense = 0;
			if ( NPC->client->ps.weaponTime || NPC->client->ps.saberInFlight || NPC->client->NPC_class == CLASS_BOBAFETT || (NPC->client->NPC_class == CLASS_COMMANDO&&Q_irand(0,2)) )
			...
			case 3:
				...
				if ( (/*NPCInfo->rank == RANK_ENSIGN || */NPCInfo->rank >= RANK_COMMANDER || NPCInfo->stats.intelligence > Q_irand(3,4)) && TIMER_Done( NPC, "parryTime" ) )
				{//FIXME: check forcePushRadius[NPC->client->ps.fd.forcePowerLevel[FP_PUSH]]
					ForceThrow( NPC, qfalse );
				}
				break;
			...
			default:
				...
				{//strafed
					...
					{//FIXME: make this a function call?
						...
						if ( NPC->client->NPC_class == CLASS_COMMANDO )
						{
							NPC->client->ps.fd.forceJumpCharge = NPC->client->ps.fd.forcePowerLevel[FP_LEVITATION] * 80;//FIXME: calc this intelligently?
						}
...
//	for ( e = 0 ; e < 3 ; e++ ) 
//	{
//		mins[e] = self->r.currentOrigin[e] - /*1024*/NPCInfo->stats.visrange * ( 2 / (6 - NPCInfo->stats.vigilance) );
//		maxs[e] = self->r.currentOrigin[e] + /*1024*/NPCInfo->stats.visrange * ( 2 / (6 - NPCInfo->stats.vigilance) );
//	}
...
void Jedi_FaceEnemy( qboolean doPitch )
{
	...
	CalcEntitySpot( NPC, SPOT_HEAD/*SPOT_CHEST*/, eyes );

	CalcEntitySpot( NPC->enemy, SPOT_HEAD/*SPOT_CHEST*/, enemy_eyes );

	if ( ( (NPC->client->NPC_class == CLASS_BOBAFETT && TIMER_Done( NPC, "flameTime" )) || (NPC->client->NPC_class == CLASS_COMMANDO && NPCInfo->stats.aim >= 3) )
	...
	{
		CalcEntitySpot( NPC, SPOT_ORIGIN, eyes );
	}
...
static void Jedi_CombatTimersUpdate( int enemy_dist )
{
	...
	if ( TIMER_Done( NPC, "noStrafe" )
		&& TIMER_Done( NPC, "strafeLeft" )
		&& TIMER_Done( NPC, "strafeRight" )
		&& !( NPC->client->ps.weapon == WP_DISRUPTOR && (NPCInfo->scriptFlags&SCF_ALT_FIRE) && NPC->client->ps.zoomMode )
		)
	{
		//FIXME: Maybe more likely to do this if aggression higher?  Or some other stat?
		if ( !Q_irand( 0, NPCInfo->stats.aggression ) )
		{//start a strafe
	...
	if ( NPC->client->ps.saberEventFlags && ( NPC->client->ps.weapon == WP_SABER ) )
...
void Jedi_CombatIdle( int enemy_dist )
{
	if ( !TIMER_Done( NPC, "parryTime" ) )
	{
		return;
			...
			{//FIXME: add more taunt behaviors
				...
				{//taunt even more, turn off the saber
					...
					if ( /*NPC->client->playerTeam != NPCTEAM_PLAYER && */!Q_irand( 0, 1 ))
					{
						...
						{
							NPC_SetAnim( NPC, SETANIM_BOTH, BOTH_ALORA_TAUNT, SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD );
							TIMER_Set( NPC, "chatter", Q_irand( 4500, 8000 ) );
							TIMER_Set( NPC, "taunting", 5000 );
							WP_ActivateSaber( NPC );
							NPC->client->ps.forceHandExtend = HANDEXTEND_JEDITAUNT;
							NPC->client->ps.forceHandExtendTime = level.time + 3000;
						}
						else
						{*/
					...
					else
					{
						...
						//if ( NPC->client->ps.weapon == WP_SABER )
							...
							TIMER_Set( NPC, "taunting", Q_irand(9000,15000) );*/
					}
				}
...
qboolean Jedi_AttackDecide( int enemy_dist )
{
	if ( NPC->enemy->client 
		&& NPC->enemy->s.weapon == WP_SABER 
		&& NPC->enemy->client->ps.saberLockTime > level.time 
	...
	{//we won a saber lock, press the advantage with an attack!
		...
		{//based on skill with some randomness
			...
			//FIXME: check enemy_dist? //NPCMod FIXME?
	...
	if ( NPC->client->NPC_class == CLASS_TAVION ||
		...
		Q_irand(0,5-NPCInfo->stats.evasion) <= 0 )
	...
	if ( NPC->client->NPC_class == CLASS_KYLE && enemy_dist <= 64 && InFOV( NPC->enemy, NPC, 30,30 ) )
	{
		if ( TIMER_Done( NPC, "rushHour" ) )
		{
			//TryGrapple( NPC );
			//NPCInfo->grappleType = Q_irand(0,2);
			TIMER_Set( NPC, "rushHour", Q_irand( 8000, 13000 ) );
			return qtrue;
		}
	}
	...
	/*if ( TIMER_Done(NPC,"danceDanceBaby") && (ucmd.buttons & BUTTON_ATTACK) && !Jedi_Jumping( NPC ) )
	{
		int rmax;
		...
		default:
			rmax = 100; //ouai on leur laisse une chance
		}

		if ( !Q_irand( 0, rmax ) )
		{
			NPCInfo->scriptFlags &= ~SCF_ALT_FIRE;	//hack..
			ucmd.buttons = BUTTON_ATTACK|BUTTON_ALT_ATTACK;
			ucmd.forwardmove = ucmd.rightmove = ucmd.upmove = 0;
			TIMER_Set( NPC, "danceDanceBaby", Q_irand(8500,13500) / (max(1,NPCInfo->rank - RANK_LT_COMM)) );
			TIMER_Set( NPC, "saberKata", 500 );

			return qtrue;
		}
		else
		{
			TIMER_Set( NPC, "danceDanceBaby", Q_irand(3000,6000) );
		}
	}
	else */if ( !Q_irand( 0, 15-NPCInfo->stats.aggression ) && WP_ForcePowerAvailable( NPC, FP_PUSH, 0 ) )
	{
		ForceThrow( NPC, qfalse );
	}
	...
	if ( ucmd.buttons&BUTTON_ATTACK && NPC->client->ps.weapon == WP_SABER)	//plus de prob avec la melee normalement
	{//attacking
		/*
		...
		*/
		...
		{//not already strafing
			if ( !TIMER_Done( NPC, "jediStraffeAttack") )
			{
				ucmd.rightmove = NPCInfo->lastRightMove;
			}
			else if ( !Q_irand( 0, 3 ) && TIMER_Done( NPC, "jediStraffeAttack") )
			{//25% chance of doing this
				...
				{//he's to my right, strafe left
					NPCInfo->lastRightMove = ucmd.rightmove = -127;
					VectorClear( NPC->client->ps.moveDir );
				}
				else
				...
				TIMER_Set( NPC, "jediStraffeAttack", Q_irand(400,2500));
			}
		}
		return qtrue;
	}
...
{//FIXME: never does a simple short, regular jump...
	...
	if ( NPC->client->NPC_class == CLASS_COMMANDO && goal != NPC->client->leader )
	{
		return qfalse;
	}
	if ( NPC->client->ps.weapon == WP_DISRUPTOR && (NPCInfo->scriptFlags&SCF_ALT_FIRE) )
	...
	if ( TIMER_Done( NPC, "jumpChaseDebounce" ) )
	{
		if ( (!goal->client || goal->client->ps.groundEntityNum != ENTITYNUM_NONE) )
		{
			...
			{//enemy is on terra firma
				...
				//FIXME?
				/*if ( NPCInfo->behaviorState == BS_FOLLOW_LEADER && goal == NPC->client->leader )
				{
					TIMER_Set( NPC, "jumpChaseDebounce", Q_irand( 2000, 5000 ) );
					ucmd.forwardmove = 127;
					if ( goal_z_diff < 32 && goal_xy_dist < 200 )
						ucmd.upmove = 127;
					VectorClear( NPC->client->ps.moveDir );
					TIMER_Set( NPC, "duck", -level.time );
					return qtrue;
				}
				else */if ( goal_xy_dist < 550 && goal_z_diff > -400/*was -256*/ )//for now, jedi don't take falling damage && (NPC->health > 20 || goal_z_diff > 0 ) && (NPC->health >= 100 || goal_z_diff > -128 ))//closer than @512
				{
					...
					else
					{
						...
						*/
						if ( (goal_z_diff > 0 || goal_xy_dist > 128)||( NPCInfo->behaviorState == BS_FOLLOW_LEADER && goal == NPC->client->leader ) )
						{//Fake a force-jump
							...
							if ( Jedi_Jump( dest, goal->s.number ) )
							{
								...
								*/
								{//FIXME: make this a function call
									...
									{
										jumpAnim = Q_irand(0,1)?BOTH_FLIP_F:BOTH_FORCEJUMP1;
										/*if (NPC->client->NPC_class == CLASS_ALORA && Q_irand(0,2))
											jumpAnim = BOTH_ALORA_FLIP_1 + Q_irand(0,2);*/
									}
...
static void Jedi_Combat( void )
{
	...
	if ( NPC->client->ps.weapon == WP_DISRUPTOR && ( NPCInfo->scriptFlags&SCF_ALT_FIRE) && NPC->client->ps.zoomMode )
	{
		NPC_BSSniper_Attack();
		return;
	}
	...
	{//I'm in the middle of a jump, so just see if I should attack
		if ( IsMeleeWeapon( NPC->client->ps.weapon )/*NPC->client->ps.weapon == WP_SABER*/ )
			Jedi_AttackDecide( enemy_dist );
		else
			Jedi_NoSaberAttackDecide();
		return;
	}
	...
	*/		else
	...
	if ( !enemy_lost && NPC->client->ps.weapon == WP_SABER )
	{
	...
	if ( !TIMER_Done( NPC, "saberKata")/* &&
		 BG_SaberInKata( NPC->client->ps.torsoAnim )*/ )
	{
		//hack, mais là c plutot bon pour le sys
		NPCInfo->scriptFlags &= ~SCF_ALT_FIRE;
		ucmd.buttons |= (BUTTON_ATTACK|BUTTON_ALT_ATTACK);
		return;
	}
	...
	if ( /*( NPC->client->ps.weapon == WP_SABER )*/IsMeleeWeapon( NPC->client->ps.weapon ) && !NPC->client->ps.saberInFlight && (!(NPC->client->ps.fd.forcePowersActive&(1<<FP_GRIP))||NPC->client->ps.fd.forcePowerLevel[FP_GRIP] < FORCE_LEVEL_2) )
	...
	/*else if ( NPC->client->ps.weapon == WP_DISRUPTOR && ( NPCInfo->scriptFlags&SCF_ALT_FIRE) )
	{
		NPC_BSSniper_Attack();
	}*/
...
void NPC_Jedi_Pain(gentity_t *self, gentity_t *attacker, int damage)
{
	...
	if ( other->client && self->NPC && TIMER_Done( self, "painForceDecide" ) && !WP_ForcePowerInUse( self, FP_ABSORB ) )
	{
		if ( ( ( WP_ForcePowerInUse( other, FP_PUSH ) || WP_ForcePowerInUse( other, FP_PULL ) ) && !Q_irand(0,10 - self->NPC->stats.intelligence) ) ||
			( ( WP_ForcePowerInUse( other, FP_DRAIN ) || WP_ForcePowerInUse( other, FP_LIGHTNING ) ) && !Q_irand(0,7 - self->NPC->stats.intelligence) ) )
			ForceAbsorb( NPC );

		TIMER_Set( self, "painForceDecide", Q_irand( 1200, 3000 ) );
	}
...
static void Jedi_Patrol( void )
{
	...
	{
		if ( TIMER_Done( NPC, "imsuffering" ) )
		{
			ForceHeal( NPC );
			TIMER_Set( NPC, "imsuffering", Q_irand( 1000 * ( 8 - NPCInfo->stats.intelligence ), 2000 * ( 8 - NPCInfo->stats.intelligence ) ) );
		}
	}
	...
	{//look for enemies
		...
		//float	best_enemy_dist = NPCInfo->stats.visrange * NPCInfo->stats.visrange;
		//int i;
		...
		//				//if ( /*enemy_dist < (NPCInfo->stats.visrange*220) || *//*( NPCInfo->investigateCount>= 3 && !NPC->client->ps.saberHolstered )*/ )
		//				//{
		...
		//				//	break;
		//				//}
		//				//if ( enemy->client->ps.saberInFlight && !enemy->client->ps.saberHolstered && enemy->client->ps.saberEntityNum )
		...
		//	}
		//}
		gentity_t *best_enemy;
		float	best_enemy_dist = 65480464;

		best_enemy = NPC_PickEnemyExt( qtrue );
		if ( best_enemy )
			best_enemy_dist = DistanceSquared( NPC->r.currentOrigin, best_enemy->r.currentOrigin );
		...
		{//still not mad
			...
			{//have one to consider
				...
				{//we have a clear (of architecture) LOS to him
					...
					if ( DistanceSquared( best_enemy->r.currentOrigin, NPC->r.currentOrigin ) < 16384 /*128*128*/
						|| NPC->client->ps.weapon != WP_SABER
...
void NPC_BSJedi_FollowLeader( void )
{
	...
	if ( NPC->client->leader && NPC->client->leader->client )
	{
		if ( WP_ForcePowerInUse( NPC->client->leader, FP_SPEED ) && !Q_irand(0,8) && WP_ForcePowerUsable( NPC, FP_SPEED ) )
		{
			ForceSpeed( NPC, 0 );
		}
	}
			...
			//	if ( Jedi_CanPullBackSaber( NPC ) )
				if (1) //no matter
				{
					...
					{//get our saber back NOW!
						...
						//RoAR mod NOTE: "Unknown symbol: max" found here

						/*if (InFOV( NPC->enemy, NPC, 90, 90 ))
						{
							int	pw2use = Q_irand(0,9);

							if (TIMER_Done( NPC, "dropNextPwd"))
							{

								switch(pw2use)
								{
								case 0:
									if ( WP_ForcePowerAvailable( NPC, FP_PUSH, qfalse ) && 
										 WP_ForcePowerUsable( NPC, FP_PUSH) && 
										 !WP_ForcePowerInUse( NPC, FP_PUSH) )
										ForceThrow( NPC, qfalse );
									break;
								case 1:
									if ( WP_ForcePowerAvailable( NPC, FP_HEAL, qfalse ) && 
										 WP_ForcePowerUsable( NPC, FP_HEAL) && 
										 !WP_ForcePowerInUse( NPC, FP_HEAL) )
										ForceHeal( NPC );
									break;
								case 2:
									if ( WP_ForcePowerAvailable( NPC, FP_LIGHTNING, qfalse ) && 
										 WP_ForcePowerUsable( NPC, FP_LIGHTNING) && 
										 !WP_ForcePowerInUse( NPC, FP_LIGHTNING) )
										TIMER_Set( NPC, "holdLightning", Q_irand(3500,5000));
									break;
								case 3:
									if ( WP_ForcePowerAvailable( NPC, FP_DRAIN, qfalse ) && 
										 WP_ForcePowerUsable( NPC, FP_DRAIN) && 
										 !WP_ForcePowerInUse( NPC, FP_DRAIN) )
										TIMER_Set(NPC, "draining", Q_irand(4000,6500));
									break;
								case 4:
									if ( WP_ForcePowerAvailable( NPC, FP_GRIP, qfalse ) && 
										 WP_ForcePowerUsable( NPC, FP_GRIP) && 
										 !WP_ForcePowerInUse( NPC, FP_GRIP) )
										TIMER_Set(NPC, "gripping", Q_irand(4000,6500));
									break;
								case 5:
									if ( WP_ForcePowerAvailable( NPC, FP_SPEED, qfalse ) && 
										 WP_ForcePowerUsable( NPC, FP_SPEED) && 
									...
									break;
								case 6:
									if ( WP_ForcePowerAvailable( NPC, FP_PROTECT, qfalse ) && 
										 WP_ForcePowerUsable( NPC, FP_PROTECT) && 
										 !WP_ForcePowerInUse( NPC, FP_PROTECT) )
										ForceProtect( NPC );
									break;
								case 7:
									if ( WP_ForcePowerAvailable( NPC, FP_ABSORB, qfalse ) && 
										 WP_ForcePowerUsable( NPC, FP_ABSORB) && 
										 !WP_ForcePowerInUse( NPC, FP_ABSORB) )
										ForceAbsorb( NPC );
									break;
								case 8:
									if ( WP_ForcePowerAvailable( NPC, FP_TELEPATHY, qfalse ) && 
										 WP_ForcePowerUsable( NPC, FP_TELEPATHY) && 
										 !WP_ForcePowerInUse( NPC, FP_TELEPATHY) )
										ForceTelepathy( NPC );
									break;
								default:
									break;
								...
								TIMER_Set( NPC, "dropNextPwd", Q_irand(10000,15000) / max(1,NPCInfo->rank - RANK_LT_COMM) );
							}

							if ( TIMER_Done( NPC, "noStrafe" ) && TIMER_Done( NPC, "strafeLeft" ) && TIMER_Done( NPC, "strafeRight" ) )
							{
								int	strafeTime = Q_irand( 1500, 3500 );
								TIMER_Set( NPC, Q_irand(0,1)?"strafeLeft":"strafeRight", strafeTime );
								TIMER_Set( NPC, "noStrafe", strafeTime + Q_irand( 1000, 2000 ) );
								TIMER_Set( NPC, "walking", Q_irand(0,1)?0:(strafeTime / 2) );
							}
						}*/
	...
	if ( NPC->enemy )
	{
		...
		{//my enemy is dead and I killed him
			...
			else
			{
				...
				{//keep walking
					if ( NPC->client->NPC_class != CLASS_COMMANDO && !(NPC->client->ps.weapon == WP_DISRUPTOR&&(NPCInfo->scriptFlags&SCF_ALT_FIRE)) )
					{
						if ( DistanceHorizontalSquared( NPC->client->renderInfo.eyePoint, NPC->enemy->r.currentOrigin ) > 4096 && (NPCInfo->scriptFlags&SCF_CHASE_ENEMIES) )//64 squared
						{
							NPCInfo->goalEntity = NPC->enemy;
...
void NPC_BSJedi_Default( void )
{
	...
	{//have an enemy
		...
		if ( NPC->client->NPC_class == CLASS_BOBAFETT )
		{
			...
			if ( (NPC->enemy != NULL) && (NPC->enemy->health >= 0) && (NPC->client->ps.weapon == WP_DISRUPTOR) && (NPCInfo->scriptFlags & SCF_ALT_FIRE) )
				NPC_BSSniper_Default();
		}
		Jedi_Attack();
		//if ( NPC->enemy && (NPC->enemy->health < 1 || !NPC->enemy->inuse) )
		//{
		//	G_ClearEnemy( NPC );
		//}
		...
		//if ( ((!ucmd.buttons&&!NPC->client->ps.fd.forcePowersActive)||(!NPC->enemy/*||(NPC->enemy->health < 1)*/)) && NPCInfo->enemyCheckDebounceTime < level.time )
		...
		{//not doing anything (or walking toward a vanquished enemy - fixme: always taunt the player?), not using force powers and it's time to look again
			...
			gentity_t *newEnemy = NPC_PickEnemyExt( qtrue );
			if ( newEnemy != NPC->enemy )
			{
				G_SetEnemy( NPC, newEnemy );
			}
			NPCInfo->enemyCheckDebounceTime = level.time + Q_irand( 4000, 10000 );
		}
	}
}
