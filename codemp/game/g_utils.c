...
gentity_t *G_TempEntity( vec3_t origin, int event ) {
	...
	e->s.otherEntityNum = ENTITYNUM_NONE; // lmo hack for conveying owner info of sound events
...
gentity_t *G_SoundTempEntity( vec3_t origin, int event, int channel ) {
	...
	e->s.eType = ET_EVENTS + event;
	e->inuse = qtrue;
	e->s.otherEntityNum = ENTITYNUM_NONE; //lmo hack to convey owner info with sound events
...
=============
*/
gentity_t *G_PlayEffect(int fxID, vec3_t org, vec3_t ang, int ownerNum)
{
	gentity_t	*te;
	...
	te->s.eventParm = fxID;
	te->s.otherEntityNum = ownerNum; // lmo mark tempentity with owner info for duel NoX
...
void G_SoundAtLoc( gentity_t *ent, vec3_t loc, int channel, int soundIndex ) {
	...
	te->s.eventParm = soundIndex;
	te->s.saberEntityNum = channel;
	te->s.otherEntityNum = ent->s.number; //lmo mark with owner info for duel NoX
}
...
void G_EntitySound( gentity_t *ent, int channel, int soundIndex ) {
	...
	te->s.otherEntityNum = ent->s.number; //lmo mark owner for duel nox
	te->s.trickedentindex = channel;
}
