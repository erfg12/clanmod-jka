...
#ifndef QAGAME
extern int cg_dueltypes[MAX_CLIENTS]; // lmo from cg_main.c
#endif

/*#ifndef QAGAME
extern int cg_Tforce[MAX_CLIENTS]; // lmo from cg_main.c
#endif*/
...
static qboolean PM_CheckEnemyPresence( int dir, float radius, float tSize/* = 12.0f*/ )
{ //anyone in this dir?
	vec3_t angles;
	vec3_t checkDir;
	vec3_t interDir;
...
#ifdef QAGAME
//Ne supporte que DIR_ALL
static int PM_CheckEnemyPresence2( int dir, float radius )
{
	int			iradiusEnts[ 128 ];
	bgEntity_t	*radEnt;
	vec3_t		mins, maxs;
	...
	for ( i = 0; i < 3; i++ )
	{
		mins[i] = pm->ps->origin[i] - radius;
		maxs[i] = pm->ps->origin[i] + radius;
	}

	numEnts = trap_EntitiesInBox( mins, maxs, iradiusEnts, 128 );

	for ( i = 0; i < numEnts; i++ )
	{
		//Don't consider self
		if ( iradiusEnts[i] == pm->ps->clientNum )
			continue;

		radEnt = PM_BGEntForNum(iradiusEnts[i]);

		if ( radEnt->s.eType != ET_PLAYER && radEnt->s.eType != ET_NPC )
			continue;

		if ( radEnt->playerState
			&& ( ( radEnt->playerState->pm_flags & PM_DEAD ) || radEnt->playerState->stats[STAT_HEALTH] <= 0 )
			)
			continue;

		numChecks++;

//		//Optimisation pour les kicks
//#if 1
//		if ( numChecks >= 12 )
//			break;
//#endif
...
saberMoveName_t PM_SaberAttackForMovement(saberMoveName_t curmove)
{
	...
	if (pm->ps->fd.saberAnimLevel == SS_DUAL)
	{
		if ( ( newmove == LS_A_R2L || newmove == LS_S_R2L
			...
			&& PM_CheckEnemyPresence( DIR_RIGHT, 100.0f, 12.0f )
			&& PM_CheckEnemyPresence( DIR_LEFT, 100.0f, 12.0f ) )
		{//enemy both on left and right
			...
			&& PM_CanDoDualDoubleAttacks()
			&& PM_CheckEnemyPresence( DIR_FRONT, 100.0f, 12.0f )
			&& PM_CheckEnemyPresence( DIR_BACK, 100.0f, 12.0f ) )
...
int PM_KickMoveForConditions(void)
{
	...
	else
	{
		...
		{ //ok, let's try some fancy kicks
			...
			int front = (int)PM_CheckEnemyPresence( DIR_FRONT, 50.0f, 12.0f );
			int back = (int)PM_CheckEnemyPresence( DIR_BACK, 50.0f, 12.0f );
			int right = (int)PM_CheckEnemyPresence( DIR_RIGHT, 50.0f, 10.0f );
			int left = (int)PM_CheckEnemyPresence( DIR_LEFT, 50.0f, 10.0f  );
			//int lf = (int)PM_CheckEnemyPresence( DIR_LF, 50.0f );
			//int lb = (int)PM_CheckEnemyPresence( DIR_LB, 50.0f );
			//int rf = (int)PM_CheckEnemyPresence( DIR_RF, 50.0f );
			//int rb = (int)PM_CheckEnemyPresence( DIR_RB, 50.0f );
#ifdef QAGAME
			int all = PM_CheckEnemyPresence2( DIR_ALL, 50.0f );
			int numEnemy = front+back+right+left/*+lf+lb+rf+rb*/;
#endif	

#ifdef QAGAME
			if ( ( all >= 3 )
				|| ( numEnemy == 2 && ( (front&&left)||(front&&right)||(back&&left)||(back&&right) ) )
				)
			{
                kickMove = LS_KICK_S;
				pm->cmd.upmove = 0;
		...
		if ( BG_SaberInAttack( newMove ) || BG_SaberInSpecialAttack( anim ) )
		{
			...
			{//wasn't playing that attack before
				if ( newMove != LS_KICK_F
					...
					&& newMove != LS_KICK_L_AIR*/ ) //RoAR mod NOTE: REMOVED
				{
