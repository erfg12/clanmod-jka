...
stringID_table_t ClassTable[] =
{
	...
	//NPCMod FIX : petite faute d'ortho, c GALAK_MECH et non GALAKMECH
	"CLASS_GALAK_MECH",CLASS_GALAKMECH,		// droid
	...
	//NPCMod FIX : réajout des classes PLAYER(pour les cinématiques)
...
stringID_table_t WeaponFlagTable[] =
{
	//ENUM2STRING(NWF_JANGO_BLASTER),
	"",	-1
};
...
//
#define MAX_NPC_DATA_SIZE 0xFFFFF	//un peu tro?
char	NPCParms[MAX_NPC_DATA_SIZE];
char	NPCFile[MAX_QPATH] = "NPC";

const char *RandomJedi()
{
#if 0
	int	totalCount = 0;
	const char *p;
	...
	while ( p ) 
	{
		...
		if ( strlen( token ) >= strlen("jedi") &&
			 !Q_stricmpn( token , "jedi", strlen("jedi") ) )
			totalCount++;

		//Com_Printf("token : %s, totalCount : %d\n",token,totalCount);

		SkipBracedSection( &p );
	}

	if (!totalCount)
		return Q_irand(0,1)?"Jedi":"Jedi2";

	totalCount = Q_irand(0,totalCount - 1);
...
//NPCMod FIX : feu-NPC_WeaponsForTeam n'étant plus, on s'en remet au membre weapon
//NPCMod NOTE : la modification porte jusquaux arguments de NPC_PrecacheWeapons
...
void NPC_PrecacheWeapons( gentity_t *ent )
{
	int weapons;
	int curWeap;
	...
	int weapons = ent->client->ps.weapon;
	gitem_t	*item;
...
qboolean NPC_ParseParms( char **NPC_Name, gentity_t *NPC/*, qboolean printErrors*/ ) 
{
	const char	*NPCName = *NPC_Name;
	const char	*token;
	const char	*value;
	...
	saber_colors_t	sc1[8],sc2[8];

	memset( &sc1[0], 0, sizeof(sc1) );
	memset( &sc2[0], 0, sizeof(sc2) );
...
//#define IFPR if ( printErrors == qtrue )
#define IFPR

	if ( !Q_stricmp( "random", NPCName ) )
	{//Randomly assemble a starfleet guy
		//NPC_BuildRandom( NPC );		
		IFPR Com_Printf("RANDOM NPC NOT SUPPORTED IN MP\n");
		return qfalse;
	}
	else
	{
		...
		NPC_SeekNPCParms( NPC_Name, &p, &token );
		NPCName = *NPC_Name;
		//// look for the right NPC
		//while ( p ) 
		//{
		...
		while ( 1 ) 
		{
			...
			if ( !token[0] ) 
			{
				IFPR Com_Printf( S_COLOR_RED"ERROR: unexpected EOF while parsing '%s'\n", NPCName );
				return qfalse;
			}
	...
	//===MODEL PROPERTIES===========================================================
			...
			if ( !Q_stricmp( token, "headYawRangeLeft" ) ) 
			{
				...
				if ( n < 0 ) 
				{
					IFPR Com_Printf( S_COLOR_YELLOW"WARNING: bad %s in NPC '%s'\n", token, NPCName );
					continue;
				}
...
#if 0
#define CHECK_AI_STAT if ( n < 0 || n > 5 )\
					{\
						Com_Printf( S_COLOR_YELLOW"WARNING: bad %s in NPC '%s'\n", token, NPCName );\
						continue;\
					}
#else
#define CHECK_AI_STAT
#endif
		...
		//		}
			...
			if ( !Q_stricmp( token, "playerTeam" ) ) 
			{
				...
				Com_sprintf(tk, sizeof(tk), "NPC%s", token);
				NPC->client->playerTeam = NPC->alliedTeam = NPC->s.teamowner = (npcteam_t)GetIDForString( TeamTable, tk );//TranslateTeamName(value);
				continue;
			}
			...
			/*if ( !Q_stricmp( token, "wflag" ) ) {
				if ( COM_ParseString( &p, &value ) ) 
				{
					continue;
				}

				n = GetIDForString( WeaponFlagTable, value );

				if ( NPC->NPC && n != -1 )
				{
					NPC->NPC->weaponFlags |= n;
				}
				continue;
			}*/
	...
	//===MOVEMENT STATS============================================================
			...
			if ( !parsingPlayer )
			{
				if ( !Q_stricmp( token, "movetype" ) ) 
				{
					...
					else if ( Q_stricmp( "walk", value ) == 0 )
					{
						if ( NPC->NPC )
...
//===MISC===============================================================================
			...
			if ( !Q_stricmp( token, "weapon" ) ) 
			{
				...
				if ( weap >= WP_NONE && weap <= WP_NUM_WEAPONS )///*WP_BLASTER_PISTOL*/WP_SABER ) //?!
				{
					...
					//	RegisterItem( FindItemForWeapon( (weapon_t)(NPC->client->ps.weapon) ) );	//precache the weapon
						NPC->client->ps.ammo[weaponData[NPC->client->ps.weapon].ammoIndex] = ammoData[weaponData[NPC->client->ps.weapon].ammoIndex].max / 2;//FIXME: max ammo!
					}
				}
				continue;
			}

			/*if ( !Q_stricmp( token, "dualweapons" ) ) 
			{
				if ( COM_ParseInt( &p, &n ) ) 
				{
					continue;
				...
				if (n != 0)
				{
					NPC->client->ps.eFlags |= EF_DUAL_WEAPONS;
					NPC->s.eFlags |= EF_DUAL_WEAPONS;
				}
				else
				{
					NPC->client->ps.eFlags &= ~EF_DUAL_WEAPONS;
			...
			if ( !Q_stricmp( token, "forceRegenRate" ) ) 
			{
				...
				NPC->NPC->forcePowerRegenRate = n;
				if ( NPC->NPC->forcePowerRegenRate <= 0 )
					NPC->NPC->forcePowerRegenRate = 1;
			...
			if ( !Q_stricmp( token, "forceRegenAmount" ) ) 
			{
				...
				NPC->NPC->forcePowerRegenAmount = n;
				////rwwFIXMEFIXME: support this?
				continue;
			}
...
#define MAX_NPC_FILE_SIZE 65536	//largement suffisant, pas besoin de 512 ko pour chaque

void NPC_LoadParms( void ) 
{
	int			len = 0, totallen, npcExtFNLen, mainBlockLen, fileCnt, i;
...
#ifdef _XBOX
	npcParseBuffer = (char *) Z_Malloc(MAX_NPC_FILE_SIZE, TAG_TEMP_WORKSPACE, qfalse, 4);
#else
	npcParseBuffer = (char *) malloc(MAX_NPC_FILE_SIZE);
	if (!npcParseBuffer)
	{
		Com_Error( ERR_DROP, S_COLOR_RED"FATAL ERROR : Not enough memory, aborting NPC_LoadParms...\n" );
		return;
	}
#endif
