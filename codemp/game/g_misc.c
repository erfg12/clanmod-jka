...
void TeleportPlayer( gentity_t *player, vec3_t origin, vec3_t angles ) {
	...
	if ( player->client->sess.sessionTeam != TEAM_SPECTATOR ) {
		...
		tent->s.otherEntityNum = player->s.clientNum; //lmo for client id of sound when played
...
void SP_fx_runner( gentity_t *ent )
{
	...
	Com_Printf( "fx_runner %s at %s\n", fxFile, vtos(ent->s.origin) );
...
void SP_emote_model ( gentity_t *ent ) //cm - Jake
{
	char *model;
	G_SpawnString( "model", "", &model );
...
//absolument génial
/*QUAKED misc_npc_generator (1 0 0) (-16 -16 -24) (16 16 32)

NPC Generator
Generator of NPCs.

npctype - type name of the NPC
maxcount - integer number of maximum count of NPCs generated at the same time
spawninterval - interval between 2 spawns ( min )
firstgoal - where the npcs try to go when generated
...
gnpcgen_t *NPCG_New()
{
	gnpcgen_t	*gen = malloc( sizeof(gnpcgen_t) );

#ifdef _DEBUG
	if ( !gen )
	{
		G_Error( S_COLOR_RED"NPCG Fatal Error : Cannot alloc generator!\n" );
	}
#endif
...
#else	//Il faut éviter un max les allocations dynamiques
gnpcgen_t *NPCG_New()
{
...
//en fonction de la team et de la rareté
...
#ifdef _DEBUG
static void NPCG_ListTypes( gnpcgen_t *gen )
{
	int count;
	int i;
	...
	Com_Printf( "------------NPC Generator------------\n" );
	Com_Printf( " NPC Type Count = %d\n", count );
	Com_Printf( " Total Prob = %d\n\n", gen->totalLuck );
	Com_Printf( " Goals Count = %d\n\n", gen->numFirstGoals );
	
	for (i = 0;i < count;i++)
	{
		Com_Printf( " Type %d >> '%s' ( prob : %d )\n", i, gen->npcTypes[i].type, gen->npcTypes[i].luck );
	}
