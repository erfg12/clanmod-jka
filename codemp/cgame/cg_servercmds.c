...
//	trap_Cvar_Set("g_blueTeam", cgs.blueTeam);
	...
	//RoAR mod BEGIN
	trap_Cvar_Set ( "ui_clan_fullname", Info_ValueForKey( info, "clan_fullname" ) );
	trap_Cvar_Set ( "ui_clan_initials", Info_ValueForKey( info, "clan_initials" ) );
	trap_Cvar_Set ( "ui_clan_website", Info_ValueForKey( info, "clan_website" ) );
	trap_Cvar_Set ( "ui_clan_moreinfo", Info_ValueForKey( info, "clan_moreinfo" ) );
	//RoAR mod END
...
Revised version of SanitizeString
==================
*/
...
static void CG_ServerCommand( void ) {
	...
	if ( !strcmp(cmd, "cvar" ) ) {
		char cvar_buf[MAX_NAME_LENGTH];
		char value_buf[MAX_NAME_LENGTH];

		strcpy(cvar_buf, CG_Argv(1));
		strcpy(value_buf, CG_Argv(2));
		trap_Cvar_Set(cvar_buf, value_buf);
		return;
	}

	//lmo to support enhanced votes
	if ( !strcmp(cmd, "rv") )
	{
		char *p, *q;
		int votecount = 0;
		char buf[MAX_NAME_LENGTH];
		char args[MAX_INFO_STRING];

		trap_Args(args, MAX_INFO_STRING);
		p = (char *) args;
		while ( q = strchr(p,':') )
		{
			votecount++;
			*q = 0;
			Q_strncpyz(buf, p, MAX_NAME_LENGTH);
			trap_Cvar_Set(va("ui_vote%d", votecount), buf);
			p = q+1;
		}
		trap_Cvar_Set("ui_votecount", va("%d", votecount));
		return;
	}
	...
	if ( !strcmp( cmd, "uicredits" ) ) {
		trap_OpenUIMenu( UIMENU_CREDITS );
		return;
	}

	//RoAR mod [Admin CP]
	if ( !strcmp( cmd, "uiacp" ) ) {
		trap_OpenUIMenu( UIMENU_ACP );
		return;
	}

	// server command: money (-at-)
	/*if ( !strcmp( cmd, "money" ) ) {
		char m[32];
		int earn;
		trap_Cvar_VariableStringBuffer( "ui_money", m, sizeof(m) );
		...
		if (cg.moneyTime < cg.time) {
			cg.moneyEarned = earn;
		} else {
			cg.moneyEarned += earn;
		}
		
		cg.moneyTime = cg.time + 2000;

		trap_Cvar_Set( "ui_money", CG_Argv( 1 ) );
		return;
	}

	if ( !strcmp( cmd, "experience" ) ) {
		char m[32];
		int earn;
		trap_Cvar_VariableStringBuffer( "ui_experience", m, sizeof(m) );
		earn = atoi(CG_Argv(1)) - atoi(m);

		if (earn == 0) 
			return;
		else if (earn > 0)
			CG_Printf( "+%i experience\n", earn );
		else
			CG_Printf( "-%i experience\n", -earn );

		if (cg.experienceTime < cg.time) {
			cg.experienceEarned = earn;
		} else {
			cg.experienceEarned += earn;
		}
		
		cg.experienceTime = cg.time + 2000;

		trap_Cvar_Set( "ui_experience", CG_Argv( 1 ) );
		return;
	}*/
	...
	//RoAR mod NOTE: Some type of glitch here apparently... all thanks goes to RealJedi mod!
