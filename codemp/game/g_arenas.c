...
void UpdateTournamentInfo( void ) {
	...
	if ( level.clients[playerClientNum].sess.sessionTeam == TEAM_SPECTATOR ) {
		...
		Com_sprintf( msg, sizeof(msg), "postgame %i %i %i %i %i %i %i %i %i %i %i %i %i %i", level.numNonSpectatorClients, playerClientNum, accuracy,
			player->client->ps.persistant[PERS_MONEY], player->client->ps.persistant[PERS_EXPERIENCE],player->client->ps.persistant[PERS_FULLGOD],
			player->client->ps.persistant[PERS_SKILL_EARNED], player->client->ps.persistant[PERS_ICON], player->client->ps.persistant[PERS_SCORE],
			perfect, score1, score2, level.time, player->client->ps.persistant[PERS_REGEN] );
	}
