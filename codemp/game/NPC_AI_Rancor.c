...
void Rancor_Swing( qboolean tryGrab )
{
	...
	for ( i = 0; i < numEnts; i++ )
	{
		...
		//RoAR mod BEGIN!
		if ( (radiusEnt->client->pers.tzone == 1) )
		{//can't hit a protected person
			continue;
		}

		if ( (radiusEnt->client->pers.amdemigod == 1) )
		{//can't hit a protected person
			continue;
		}

		if ( (radiusEnt->client->pers.autopro == 1) )
		{//can't hit a protected person
			continue;
		}

		if ( (radiusEnt->client->pers.ampunish == 1) )
		{//can't hit a protected person
			continue;
		}

		if ( (radiusEnt->client->pers.amsleep == 1) )
		{//can't hit a protected person
			continue;
		}
		//END
