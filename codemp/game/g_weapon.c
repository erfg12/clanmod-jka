...
//cm - Jake 
//NOTE: This file will have some cm_(damage) stuff.

// Bryar Pistol
//--------
#define BRYAR_PISTOL_VEL			cm_BRYAR_PISTOL_VELOCITY.integer //1600
#define BRYAR_PISTOL_DAMAGE			cm_BRYAR_PISTOL_DAMAGE.integer
...
#define BLASTER_VELOCITY			cm_E11_BLASTER_VELOCITY.integer //2300
#define BLASTER_DAMAGE				cm_E11_BLASTER_DAMAGE.integer

// Tenloss Disruptor
//----------
#define DISRUPTOR_MAIN_DAMAGE			cm_DISRUPTOR_MAIN_DAMAGE.integer //40
#define DISRUPTOR_MAIN_DAMAGE_SIEGE		50
#define DISRUPTOR_NPC_MAIN_DAMAGE_CUT	0.25f

#define DISRUPTOR_ALT_DAMAGE			cm_DISRUPTOR_ALT_DAMAGE.integer //125
...
// Wookiee Bowcaster
//----------
#define	BOWCASTER_DAMAGE			cm_BOWCASTER_DAMAGE.integer
#define	BOWCASTER_VELOCITY			cm_BOWCASTER_VELOCITY.integer //1300
...
#define	REPEATER_DAMAGE				cm_REPEATER_DAMAGE.integer
#define	REPEATER_VELOCITY			cm_REPEATER_VELOCITY.integer//1600

#define REPEATER_ALT_SIZE				3	// half of bbox size
#define	REPEATER_ALT_DAMAGE				cm_REPEATER_ALT_DAMAGE.integer //60
#define REPEATER_ALT_SPLASH_DAMAGE		cm_REPEATER_SPLASH_DAMAGE.integer //60
#define REPEATER_ALT_SPLASH_RADIUS		128
#define REPEATER_ALT_SPLASH_RAD_SIEGE	80
#define	REPEATER_ALT_VELOCITY			cm_REPEATER_VELOCITY.integer //1100

// DEMP2
//----------
#define	DEMP2_DAMAGE				cm_DEMP2_DAMAGE.integer
#define	DEMP2_VELOCITY				cm_DEMP2_VELOCITY.integer//1800
#define	DEMP2_SIZE					2		// half of bbox size

#define DEMP2_ALT_DAMAGE			cm_DEMP2_ALT_DAMAGE.integer //12		// does 12, 36, 84 at each of the 3 charge levels.
...
#define FLECHETTE_DAMAGE			cm_FLECHETTE_DAMAGE.integer //15
#define FLECHETTE_VEL				cm_FLECHETTE_VELOCITY.integer //3500
#define FLECHETTE_SIZE				1
#define FLECHETTE_MINE_RADIUS_CHECK	256
#define FLECHETTE_ALT_DAMAGE		cm_FLECHETTE_ALT_DAMAGE.integer
#define FLECHETTE_ALT_SPLASH_DAM	60
#define FLECHETTE_ALT_SPLASH_RAD	128

// Personal Rocket Launcher
//---------
#define	ROCKET_VELOCITY				cm_ROCKET_VELOCITY.integer //900
#define	ROCKET_DAMAGE				cm_ROCKET_DAMAGE.integer
#define	ROCKET_SPLASH_DAMAGE		cm_ROCKET_SPLASH_DAMAGE.integer
...
#define	CONC_VELOCITY				cm_CONCUSION_VELOCITY.integer //3000
#define	CONC_DAMAGE					cm_CONCUSION_DAMAGE.integer //150
#define	CONC_NPC_DAMAGE_EASY		40
#define	CONC_NPC_DAMAGE_NORMAL		80
#define	CONC_NPC_DAMAGE_HARD		100
#define	CONC_SPLASH_DAMAGE			cm_CONCUSION_SPLASH_DAMAGE.integer
#define	CONC_SPLASH_RADIUS			200 //300
//alt
#define CONC_ALT_DAMAGE				cm_CONCUSION_ALT_DAMAGE.integer //100
...
// Stun Baton
//--------------
#define STUN_BATON_DAMAGE			cm_STUN_BATON_DAMAGE.integer
#define STUN_BATON_ALT_DAMAGE		20
#define STUN_BATON_RANGE			8

// Melee
//--------------
#define MELEE_SWING1_DAMAGE			cm_MELEE_SWING1_DAMAGE.integer
#define MELEE_SWING2_DAMAGE			cm_MELEE_SWING2_DAMAGE.integer
...
//---------------------------------------------------------
{
	...
	{//need to loop this in case we hit a Jedi who dodges the shot
		...
		else if (traceEnt && traceEnt->client && traceEnt->client->ps.fd.forcePowerLevel[FP_SABER_DEFENSE] >= FORCE_LEVEL_3)
		{
			...
			{ //broadcast and stop the shot because it was blocked
				...
				te->s.otherEntityNum = tr.entityNum; //lmo hack for sending owner info with sound event entity
	...
	VectorCopy( muzzle, tent->s.origin2 );
	tent->s.eventParm = ent->s.number;
	tent->s.otherEntityNum = ent->s.number; //lmo mark with owner info for duel NoX
	...
	if ( render_impact )
	{
		if ( tr.entityNum < ENTITYNUM_WORLD && traceEnt->takedamage )
		{
			...
			tent->s.otherEntityNum = ent->s.number; //lmo mark with owner info
			if (traceEnt->client)
			{
				tent->s.weapon = 1;
			}
...
//---------------------------------------------------------
{
	...
	for (i = 0; i < traces; i++ )
	{
		...
		/*if (g_gametype.integer == GT_RPG && traceEnt && traceEnt->client && (traceEnt->client->ps.userInt2 == 0 || ent->client->ps.userInt2 == 0) &&
				traceEnt->client->ps.duelInProgress == 0 && traceEnt->s.eType != ET_NPC) {
			skip = tr.entityNum;
			VectorCopy(tr.endpos, start);
			continue;
			}*/
		...
		else if (traceEnt && traceEnt->client && traceEnt->client->ps.fd.forcePowerLevel[FP_SABER_DEFENSE] >= FORCE_LEVEL_3)
		{
			...
			{ //broadcast and stop the shot because it was blocked
				...
				tent->s.eventParm = ent->s.number;
				tent->s.otherEntityNum = ent->s.number; //lmo mark owner info for duel NoX
...
//---------------------------------------------------------
{
	...
	efEnt = G_PlayEffect( EFFECT_EXPLOSION_DEMP2ALT, ent->r.currentOrigin, ent->pos1, ent->s.owner );
...
#define TD_DAMAGE			cm_THERMAL_DETONATOR_DAMAGE.integer //only do 70 on a direct impact
#define TD_SPLASH_RAD		128
#define TD_SPLASH_DAM		90
#define TD_VELOCITY			cm_THERMAL_DETONATOR_VELOCITY.integer //900
#define TD_MIN_CHARGE		0.15f
#define TD_TIME				3000//6000
#define TD_ALT_TIME			3000

#define TD_ALT_DAMAGE		cm_THERMAL_DETONATOR_ALT_DAMAGE.integer//100
...
//---------------------------------------------------------
{
	...
	//lmo mark with owner info for duel NoX
...
======================================================================
*/
#define LT_DAMAGE			cm_TRIP_MINE_DAMAGE.integer
...
void laserTrapExplode( gentity_t *self )
{
	...
	if (self->s.weapon == WP_FLECHETTE)
	{
		G_PlayEffect(EFFECT_EXPLOSION_FLECHETTE, self->r.currentOrigin, v, self->s.owner);
	}
	else
	{
		G_PlayEffect(EFFECT_EXPLOSION_TRIPMINE, self->r.currentOrigin, v, self->s.owner);
	}
...
{ //create a laser trap entity
	...
	//lmo mark with owner info
	laserTrap->s.otherEntityNum = owner->s.number;
...
void charge_stick (gentity_t *self, gentity_t *other, trace_t *trace)
{
	...
	{ //hit an entity that we just want to explode on (probably another projectile or something)
		...
		self->count = -1;
		G_PlayEffect(EFFECT_EXPLOSION_DETPACK, self->r.currentOrigin, v, self->s.owner);
...
void drop_charge (gentity_t *self, vec3_t start, vec3_t dir) 
{
	...
	//lmo mark owner info for duel NoX
	bolt->s.otherEntityNum = self->s.number;
	bolt->damage = cm_DET_PACK_DAMAGE.integer;
	bolt->splashDamage = cm_DET_PACK_SPLASH_DAMAGE.integer;
...
void WP_FireStunBaton( gentity_t *ent, qboolean alt_fire )
{
	...
	{ //see if either party is involved in a duel
		...
		/*if (g_gametype.integer == GT_RPG && tr_ent->client && (tr_ent->client->ps.userInt2 == 0 ||
			ent->client->ps.userInt2 == 0) && ent->client->ps.duelInProgress == 0 &&
				tr_ent->client->ps.duelInProgress == 0 && tr_ent->s.eType != ET_NPC && ent->s.eType != ET_NPC) {
					return;
				}*/
	}
	...
	{
		G_PlayEffect( EFFECT_STUNHIT, tr.endpos, tr.plane.normal, ent->s.owner );
...
void WP_FireMelee( gentity_t *ent, qboolean alt_fire )
{
	...
	{ //hit something
		...
		{ //special duel checks
			...
			/*if (g_gametype.integer == GT_RPG && tr_ent->client && tr_ent->client->ps.userInt2 == 0 &&
				tr_ent->client->ps.duelInProgress == 0 && tr_ent->s.eType != ET_NPC) {
					return;
			}
			if (g_gametype.integer == GT_RPG && ent->client && ent->client->ps.userInt2 == 0 &&
...
//		G_PlayEffect(EFFECT_EXPLOSION_DETPACK, explOrg, puffAngle);
		G_PlayEffect(EFFECT_EXPLOSION_DETPACK, explOrg, /*self->r.currentAngles*/puffAngle, self->s.owner); //lmo mark owner info
	...
	{ //see if we are freshly dead and should be smoking
		...
		{ //is it time yet to spawn another smoke puff?
			...
			G_PlayEffect(EFFECT_SMOKE, smokeOrg, puffAngle, self->s.owner);
