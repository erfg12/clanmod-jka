...
void AddScore( gentity_t *ent, vec3_t origin, int score )
{
	...
	*/
	...
	if ( g_gametype.integer == GT_TEAM && !g_dontPenalizeTeam )
		level.teamScores[ ps->persistant[PERS_TEAM] ] += score;
	CalculateRanks();
}
...
void player_die( gentity_t *self, gentity_t *inflictor, gentity_t *attacker, int damage, int meansOfDeath ) {
	...
	//lmo check to see if they had a grapple out! (lmo)
	if (self->client->hook)
	{ // free it!
	...
	//cm - Jake
	//NOTE: I added this here so you can view deaths on scoreboard.
	if (g_gametype.integer != GT_DUEL && g_gametype.integer != GT_POWERDUEL && self->s.eType != ET_NPC){
...
extern void RunEmplacedWeapon( gentity_t *ent, usercmd_t **ucmd );
	...
	if (g_slowmoDuelEnd.integer && (/*g_gametype.integer == GT_FFA || */g_gametype.integer == GT_DUEL || g_gametype.integer == GT_POWERDUEL) && attacker && attacker->inuse && attacker->client)
	{
		if (!gDoSlowMoDuel)
		{
			gDoSlowMoDuel = qtrue;
	...
	else if (g_slowmoDuelEnd.integer && g_gametype.integer != GT_DUEL && g_gametype.integer != GT_POWERDUEL && attacker && attacker->inuse && attacker->client)
	{
		//RoAR mod NOTE: Any other gametype must be actually IN a duel.
	...
	*/
	...
	if (level.numPlayingClients >= 2) {
		    ...
		    if (attacker && attacker->client && attacker->s.eType != ET_NPC && attacker->client->pers.userID > 0)
            {
				...
				//SendUDP(va("say|%s killed %s by %s", killerName, self->client->pers.netname, obit)); // this might be too much
	...
	if (attacker && attacker->client) {
		...
		if ( attacker == self || OnSameTeam (self, attacker ) ) {
			...
			{
				if ((!self->NPC) || (self->client->playerTeam == attacker->client->enemyTeam))
					AddScore( attacker, self->r.currentOrigin, 1 );
			}
			...
			/*if ( g_gametype.integer == GT_RPG && 200 ) {
				GiveMoney( attacker, 50 ); // give bonus money for a kill (-at-)
				GiveExperience( attacker, 25 ); // give bonus money for a kill (-at-)
			}*/
...
void G_GetDismemberBolt(gentity_t *self, vec3_t boltPoint, int limbType)
{
	...
	{ //Make some saber hit sparks over the severed wrist area
		...
		te->s.otherEntityNum = self->s.number; //lmo hack for sending owner info in sound event entity.
	}
}
...
void G_Dismember( gentity_t *ent, gentity_t *enemy, vec3_t point, int limbType, float limbRollBase, float limbPitchBase, int deathAnim, qboolean postDeath )
{
	...
	*/
	...
	//RoAR mod NOTE: ENSIFORM glitch fix.
...
void CM_Dismemberment(gentity_t *ent)
{
	int sect = G2_MODELPART_HEAD;
	vec3_t boltPoint;
	...
	{
		//G_GetDismemberBolt(self, boltPoint, sect);
		G_Dismember( ent, ent, boltPoint, sect, 90, 0, BOTH_DEATH1, qfalse );
		sect++;
	}
}
...
void G_CheckForDismemberment(gentity_t *ent, gentity_t *enemy, vec3_t point, int damage, int deathAnim, qboolean postDeath)
{
	...
	else if (ent->client && (ent->client->ps.eFlags2 & EF2_MD3_MODEL))
		return;
...
void G_Damage( gentity_t *targ, gentity_t *inflictor, gentity_t *attacker,
	...
	//cm - Jake I removed these because its causing a dedicated server to crash.
	//int oldHealth = 0; //cm - Dom
	//int oldArmour = 0; //cm - Dom

	//cm - Dom 
	//Store current health and armour of the target
	//oldHealth = targ->client->ps.stats[STAT_HEALTH];
	//oldArmour = targ->client->ps.stats[STAT_ARMOR];
	...
	if (targ && targ->client && targ->client->ps.duelInProgress)
	{
		...
		else if (attacker && attacker->client && !(mod == MOD_SABER || mod == MOD_FALLING || dueltypes[attacker->client->ps.clientNum] == 1 || dueltypes[targ->client->ps.clientNum] == 2 || dueltypes[targ->client->ps.clientNum] == 3)) // update for falling while dueling, or full force dueling
		{
			return;
		}
	}
	if (attacker && attacker->client && attacker->client->ps.duelInProgress)
	{
		...
		else if (targ && targ->client && !(mod == MOD_SABER || mod == MOD_FALLING || dueltypes[targ->client->ps.clientNum] == 1 || dueltypes[targ->client->ps.clientNum] == 2 || dueltypes[targ->client->ps.clientNum] == 3)) // update for falling while dueling, or full force dueling
		{
			return;
		}
	}
	...
	/*
	if ((g_gametype.integer == GT_RPG) && attacker && attacker->client && targ && targ->client && 
		(targ->client->ps.userInt2 == 0 || attacker->client->ps.userInt2 == 0)
		&& attacker->client->ps.duelInProgress == 0){
		if (targ->s.eType != ET_NPC && attacker->s.eType != ET_NPC && attacker->client->ps.pm_type != PM_DEAD && targ->client->ps.pm_type != PM_DEAD){
		return;
		}
	}*/
	...
	*/
	...
	if (take) 
	{
		...
		// check g_moneyEnabled and if we have a valid attacker (-at-)
		/*if ((g_gametype.integer == GT_RPG) && (attacker) && (attacker->client) && (targ) && (targ->client) && (targ != attacker))
		{
			int earn = 0;
			int earn2 = 0;
			...
			if (targ->health > maxHealth)
				earn -= targ->health - maxHealth;
			...
			if (targ->health > maxHealth) 
				earn2 -= targ->health - maxHealth;

			// earn money for doing damage (-at-)
			if (earn > 0) {
				// earn money for percent of total health
				earn = (earn * 9) / maxHealth;
					// only 75% of money from saber attacks
					if (mod == MOD_SABER) earn -= (earn >> 2);
					GiveMoney( attacker, earn );
			}
			// earn experience
			if (earn2 > 0) {
				// earn money for percent of total health
				earn = (earn * 21) / maxHealth;
					// only 75% of money from saber attacks
					if (mod == MOD_SABER) earn2 -= (earn2 >> 2);
					GiveExperience( attacker, earn2 );
...
qboolean G_RadiusDamage ( vec3_t origin, gentity_t *attacker, float damage, float radius,
	...
	*/
	...
	for ( e = 0 ; e < numListedEntities ; e++ ) {
		...
		/*if( (g_gametype.integer == GT_RPG) && ent != attacker && ent->client && 
			(ent->client->ps.userInt2 == 0 || attacker->client->ps.userInt2 == 0)
			&& attacker->client && attacker->client->ps.userInt2 == 0 && 
			attacker->s.eType != ET_NPC && ent->s.eType != ET_NPC && 
			attacker->client->ps.pm_type != PM_DEAD && ent->client->ps.pm_type != PM_DEAD)
		{ //both dont have PvP flags
			continue;
		}*/
