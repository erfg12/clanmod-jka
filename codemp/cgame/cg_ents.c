...
//LASTHOPE GRAPPLEMOD FIXME: originate fx_local.h
...
void CG_Special( centity_t *cent ) {
	...
	{	// The portable shield should go through a different rendering function.
		//lmo check owner info if duel NoX
...
static void CG_General( centity_t *cent ) {
	...
	if (cent->currentState.weapon == WP_STUN_BATON)
	{
		...
		// lerp the tag
		trap_R_LerpTag( &lerped, cg_weapons[WP_STUN_BATON].missileModel, 0, 0,
			1.0, "tag_extrem" );
		...
		trap_G2API_GetBoltMatrix(parent->ghoul2, 0, 0, &mat, parent->turAngles, parent->lerpOrigin, cg.time, cgs.gameModels, parent->modelScale);
		BG_GiveMeVectorFromMatrix(&mat, ORIGIN, start);

		//RoAR mod NOTE: Grapple string line (SHOOT)
		/*trap_FX_AddLine( start, end, 0.1f, 3.0f, 0.0f, 
							1.0f, 0.0f, 0.0f,
							WHITE, WHITE, 0.0f,
							20, 0, 
							0 );*/
		CG_TestLine(start, end, 1, color, 1);
	}
	//END

	if (cg.snap->ps.duelInProgress &&
		( (cent->currentState.weapon == WP_SABER && cent->currentState.otherEntityNum != cg.snap->ps.clientNum && cent->currentState.otherEntityNum != cg.snap->ps.duelIndex) ||
			cent->currentState.eType & ET_BODY || cent->currentState.weapon == G2_MODEL_PART))
...
//	int	col;
	...
	if (s1->weapon == WP_STUN_BATON)
	{
		...
		// lerp the tag
		trap_R_LerpTag( &lerped, weapon->missileModel, 0, 0,
			1.0, "tag_extrem" );
		...
		trap_G2API_GetBoltMatrix(parent->ghoul2, 1, 0, &mat, parent->turAngles, parent->lerpOrigin, cg.time, cgs.gameModels, parent->modelScale);
		BG_GiveMeVectorFromMatrix(&mat, ORIGIN, start);

		//RoAR mod NOTE: Grapple string line (PULL)
		/*trap_FX_AddLine( start, end, 0.1f, 3.0f, 0.0f, 
							1.0f, 0.0f, 0.0f,
							WHITE, WHITE, 0.0f,
							20, 0, 
