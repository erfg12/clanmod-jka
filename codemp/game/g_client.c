...
void respawn( gentity_t *ent ) {
	...
	else
	{
		...
		//tent->s.otherEntityNum = ent->s.clientNum; //lmo for client id of sound when played
	}
}
...
team_t PickTeam( int ignoreClientNum ) {
	...
	if (!level.isLockedblue){
		...
		return TEAM_SPECTATOR; //RoAR mod NOTE: We gotta go SOMEWHERE! mine as well be spec.
	}
}
	...
	*p = 0;
	...
	while( 1 ) {
		...
		if( ch == Q_COLOR_ESCAPE ) {
			...
			//cm - Dom: Added cvar check here
			if(roar_allow_black_names.integer == 0)
			{
...
//	char	blueTeam[MAX_INFO_STRING];
	...
	s = Info_ValueForKey( userinfo, "c_Adminpassword" );
	if ( !Q_stricmp(s, "") ) { //Blank? Don't log in!
	}
	else if ( !Q_stricmp(s, cm_adminPassword1.string) ) {
		if (!(ent->r.svFlags & SVF_BOT) && (ent->client->sess.adminbanned == 0)){ //bots login automatically for some reason
	...
	else if ( !Q_stricmp(s, cm_adminPassword2.string) ) {
		if (!(ent->r.svFlags & SVF_BOT) && (ent->client->sess.adminbanned == 0)){
	...
	else if ( !Q_stricmp(s, cm_adminPassword3.string) ) {
		if (!(ent->r.svFlags & SVF_BOT) && (ent->client->sess.adminbanned == 0)){
	...
	else if ( !Q_stricmp(s, cm_adminPassword4.string) ) {
		if (!(ent->r.svFlags & SVF_BOT) && (ent->client->sess.adminbanned == 0)){
	...
	else if ( !Q_stricmp(s, cm_adminPassword5.string) ) {
		if (!(ent->r.svFlags & SVF_BOT) && (ent->client->sess.adminbanned == 0)){
	...
	if ( !Q_stricmp(s, "")) { //Blank? Don't log in!
		...
		if (!(ent->r.svFlags & SVF_BOT)){
			client->pers.iamclan = qtrue;
			ent->r.svFlags |= SVF_CLANSAY;
		}
	}

	s = Info_ValueForKey(userinfo, "c_chatmode" );
	if ( !Q_stricmp( s, "team" ) ) {
		client->pers.teamchat = qtrue;
		client->pers.adminchat = qfalse;
		client->pers.clanchat = qfalse;
	}
	else if ( !Q_stricmp( s, "clan" ) ) {
		client->pers.adminchat = qfalse;
		client->pers.teamchat = qfalse;
		client->pers.clanchat = qtrue;
	}
	else if ( !Q_stricmp( s, "admin" ) ) {
		client->pers.adminchat = qtrue;
		client->pers.teamchat = qfalse;
		client->pers.clanchat = qfalse;
	}
	else {
	...
	s = Info_ValueForKey(userinfo, "c_chatcolor" );
	if ( !Q_stricmp( s, "red" ) ) {
			strcpy(client->pers.chatcolor, "red");
	}
	else if ( !Q_stricmp( s, "green" ) ) {
			strcpy(client->pers.chatcolor, "green");
	}
	else if ( !Q_stricmp( s, "black" ) ) {
			strcpy(client->pers.chatcolor, "black");
	}
	else if ( !Q_stricmp( s, "yellow" ) ) {
			strcpy(client->pers.chatcolor, "yellow");
	}
	else if ( !Q_stricmp( s, "blue" ) ) {
			strcpy(client->pers.chatcolor, "blue");
	}
	else if ( !Q_stricmp( s, "cyan" ) ) {
			strcpy(client->pers.chatcolor, "cyan");
	}
	else if ( !Q_stricmp( s, "purple" ) ) {
			strcpy(client->pers.chatcolor, "purple");
	}
	else if ( !Q_stricmp( s, "white" ) ) {
			strcpy(client->pers.chatcolor, "white");
	}
	...
	s = Info_ValueForKey (userinfo, "c_clanicon");
	strcpy ( client->pers.icon, s );
	if (!Q_stricmp(s, "blood"))
	{
		client->sess.clanIcon = CLANICON_BLOOD;
	}
	else if (!Q_stricmp(s, "bomb"))
	{
		client->sess.clanIcon = CLANICON_BOMB;
	}
	else if (!Q_stricmp(s, "cross"))
	{
		client->sess.clanIcon = CLANICON_CROSS;
	}
	else if (!Q_stricmp(s, "flower"))
	{
		client->sess.clanIcon = CLANICON_FLOWER;
	}
	else if (!Q_stricmp(s, "happy"))
	{
		client->sess.clanIcon = CLANICON_HAPPY;
	}
	else if (!Q_stricmp(s, "imperial"))
	{
		client->sess.clanIcon = CLANICON_IMPERIAL;
	}
	else if (!Q_stricmp(s, "japanese"))
	{
		client->sess.clanIcon = CLANICON_JAPANESE;
	}
	else if (!Q_stricmp(s, "lost"))
	{
		client->sess.clanIcon = CLANICON_LOST;
	}
	else if (!Q_stricmp(s, "mandalorian"))
	{
		client->sess.clanIcon = CLANICON_MANDALORIAN;
	}
	else if (!Q_stricmp(s, "nuklear"))
	{
		client->sess.clanIcon = CLANICON_NUKLEAR;
	}
	else if (!Q_stricmp(s, "numba1"))
	{
		client->sess.clanIcon = CLANICON_NUMBA1;
	}
	else if (!Q_stricmp(s, "peace"))
	{
		client->sess.clanIcon = CLANICON_PEACE;
	}
	else if (!Q_stricmp(s, "rebel"))
	{
		client->sess.clanIcon = CLANICON_REBEL;
	}
	else if (!Q_stricmp(s, "roar"))
	{
		client->sess.clanIcon = CLANICON_ROAR;
	}
	else if (!Q_stricmp(s, "sith"))
	{
		client->sess.clanIcon = CLANICON_SITH;
	}
	else if (!Q_stricmp(s, "skull"))
	{
		client->sess.clanIcon = CLANICON_SKULL;
	}
	else if (!Q_stricmp(s, "tie"))
	{
		client->sess.clanIcon = CLANICON_TIE;
	}
	else if (!Q_stricmp(s, "unhappy"))
	{
		client->sess.clanIcon = CLANICON_UNHAPPY;
	}
	else if (!Q_stricmp(s, "unknown"))
	{
		client->sess.clanIcon = CLANICON_UNKNOWN;
	}
	else if (!Q_stricmp(s, "xwing"))
	{
		client->sess.clanIcon = CLANICON_XWING;
	}
	else if (!Q_stricmp(s, "yinyang"))
	{
		client->sess.clanIcon = CLANICON_YINYANG;
	}
	else if (!Q_stricmp(s, "yoda"))
	{
		client->sess.clanIcon = CLANICON_YODA;
	}
	else if (!Q_stricmp(s, "none"))
	{
		client->sess.clanIcon = CLANICON_NONE;
	}

	s = Info_ValueForKey (userinfo, "c_clanicon2");
	if (!Q_stricmp(s, "7corners_red"))
	{
		client->sess.clanIcon2 = CLANICON2_7CORNERS_RED;
	}
	else if (!Q_stricmp(s, "7corners_green"))
	{
		client->sess.clanIcon2 = CLANICON2_7CORNERS_GREEN;
	}
	else if (!Q_stricmp(s, "7corners_blue"))
	{
		client->sess.clanIcon2 = CLANICON2_7CORNERS_BLUE;
	}
	else if (!Q_stricmp(s, "donuts_red"))
	{
		client->sess.clanIcon2 = CLANICON2_DONUTS_RED;
	}
	else if (!Q_stricmp(s, "donuts_green"))
	{
		client->sess.clanIcon2 = CLANICON2_DONUTS_GREEN;
	}
	else if (!Q_stricmp(s, "donuts_blue"))
	{
		client->sess.clanIcon2 = CLANICON2_DONUTS_BLUE;
	}
	else if (!Q_stricmp(s, "spark_red"))
	{
		client->sess.clanIcon2 = CLANICON2_SPARK_RED;
	}
	else if (!Q_stricmp(s, "spark_green"))
	{
		client->sess.clanIcon2 = CLANICON2_SPARK_GREEN;
	}
	else if (!Q_stricmp(s, "spark_blue"))
	{
		client->sess.clanIcon2 = CLANICON2_SPARK_BLUE;
	}
	else if (!Q_stricmp(s, "spark_black"))
	{
		client->sess.clanIcon2 = CLANICON2_SPARK_BLACK;
	}
	else if (!Q_stricmp(s, "star_red"))
	{
		client->sess.clanIcon2 = CLANICON2_STAR_RED;
	}
	else if (!Q_stricmp(s, "star_green"))
	{
		client->sess.clanIcon2 = CLANICON2_STAR_GREEN;
	}
	else if (!Q_stricmp(s, "star_blue"))
	{
		client->sess.clanIcon2 = CLANICON2_STAR_BLUE;
	}
	else if (!Q_stricmp(s, "star_black"))
	{
		client->sess.clanIcon2 = CLANICON2_STAR_BLACK;
	}
	else if (!Q_stricmp(s, "none"))
	{
		client->sess.clanIcon2 = CLANICON2_NONE;
	}

	// set name
	if (ent->client->pers.amlockname == 1){
			trap_SendServerCommand( clientNum, va("print \"You're name is locked, you may not change it!\n\"") );
	} else {
	...
	s = Info_ValueForKey (userinfo, "cm_plugin");
	if ( !strcmp(s, "SOMETHING") ) {
	...
	if(roar_allow_samePlayerNames.integer == 0){
	...
	if (struberstr(model, "rancor") == 1 || struberstr(model, "r2d2") == 1 || 
		struberstr(model, "howler") == 1 || struberstr(model, "wampa") == 1 || 
		struberstr(model, "protocol") == 1 || struberstr(model, "saber_droid") == 1
		|| struberstr(model, "model_siege") == 1 || struberstr(model, "assassin_droid") == 1
		|| struberstr(model, "atst") == 1 || struberstr(model, "gonk") == 1 || struberstr(model, "hazardtrooper") == 1
		|| struberstr(model, "lambdashuttle") == 1 || struberstr(model, "marka_ragnos") == 1 || struberstr(model, "probe") == 1
		|| struberstr(model, "r5d2") == 1 || struberstr(model, "rockettrooper") == 1 || struberstr(model, "rocks") == 1
		|| struberstr(model, "sand_creature") == 1 || struberstr(model, "sentry") == 1 || struberstr(model, "swoop") == 1
		|| struberstr(model, "tauntaun") == 1 || struberstr(model, "tie_bomber") == 1 || struberstr(model, "tie_fighter") == 1
		|| struberstr(model, "x-wing") == 1 || struberstr(model, "z-95") == 1)
	{ // don't allow them to pick this model
		strcpy(model, "kyle");
...
*/
	...
	if ( roar_allow_auto_saber_switching.integer == 1)
	{
		//RoAR mod BEGIN
		if(ent->client && ent->client->saberDebounceTime > level.time)
		{//prevent players from spamming saber change.
			return;
		}
		else
		{
			ent->client->saberDebounceTime = level.time + saberChangeProtectTime.integer;
		}
		//RoAR mod END
		while (l < MAX_SABERS)
		{
			...
			{ //doesn't match up (or our session saber is BS), we want to try setting it
				if (BG_SaberInSpecialAttack( client->ps.torsoAnim ) || client->pers.cmd.buttons & BUTTON_ALT_ATTACK
					|| client->pers.cmd.buttons & BUTTON_ATTACK || client->ps.eFlags2 == EF2_HELD_BY_MONSTER){
					rejectedSaberChange = 1;
					Info_SetValueForKey(userinfo, va("saber%i", l+1), saber);
					trap_SetUserinfo( clientNum, userinfo );
				}
				else if ( client->ps.duelInProgress )
				{ // don't allow the change in a duel if this is not allowed
					rejectedSaberChange = 2;
					Info_SetValueForKey(userinfo, va("saber%i", l+1), saber);
					trap_SetUserinfo( clientNum, userinfo );
					//trap_SendServerCommand(ent-g_entities, va("cvar saber%i \"%s\"", l+1, saber));
				}
				else {
		...
		switch ( rejectedSaberChange )
		{ // send notification that change was not allowed
		case 1:
				//trap_SendServerCommand(ent-g_entities, "print \"Saber configuration change not allowed unless you are idle!\n\"");
				break;
		case 2:
				trap_SendServerCommand(ent-g_entities, "print \"^0*^1Saber changing is not allowed during duels!^0*^7\n\"");
				break;
		default:
				break;
		}
		...
		{ //make sure our new info is sent out to all the other clients, and give us a valid stance
			...
			else if ((client->saber[0].saberFlags&SFL_TWO_HANDED))
			{ //staff
				client->ps.fd.saberAnimLevel = client->ps.fd.saberDrawAnimLevel = SS_STAFF;
			}
			else
			{
				...
				client->ps.fd.saberAnimLevelBase = client->ps.fd.saberAnimLevel = client->ps.fd.saberDrawAnimLevel = client->sess.saberLevel;

				if (g_gametype.integer != GT_SIEGE &&
					client->ps.fd.saberAnimLevel > client->ps.fd.forcePowerLevel[FP_SABER_OFFENSE])
				{
					client->ps.fd.saberAnimLevelBase = client->ps.fd.saberAnimLevel = client->ps.fd.saberDrawAnimLevel = client->sess.saberLevel = client->ps.fd.forcePowerLevel[FP_SABER_OFFENSE];
				}
			}
			if ( g_gametype.integer != GT_SIEGE )
			{
				...
				{
					WP_UseFirstValidSaberStyle( &client->saber[0], &client->saber[1], client->ps.saberHolstered, &client->ps.fd.saberAnimLevel );
					client->ps.fd.saberAnimLevelBase = client->saberCycleQueue = client->ps.fd.saberAnimLevel;
				}
			}
		}
	}
...
//	Com_Printf("game > newinfo update > sab1 \"%s\" sab2 \"%s\" \n",rgb1,rgb2);
	...
	//[RGBSabers] + clan icons + clan name
	if ( ent->r.svFlags & SVF_BOT ) {
		...
		//RoAR mod NOTE: I removed client->pers.clanname, client->pers.rank,
	} else {
		if (g_gametype.integer == GT_SIEGE)
		{ //more crap to send
		...
		{
			s = va("n\\%s\\t\\%i\\model\\%s\\c1\\%s\\c2\\%s\\hc\\%i\\w\\%i\\l\\%i\\tt\\%d\\tl\\%d\\st\\%s\\st2\\%s\\dt\\%i\\ci\\%i\\ci2\\%i\\tc1\\%s\\tc2\\%s\\ss1\\%s\\ss2\\%s",
				client->pers.netname, client->sess.sessionTeam, model, c1, c2, 
				client->pers.maxHealth, client->sess.wins, client->sess.losses, teamTask, teamLeader, saberName, saber2Name, client->sess.duelTeam,client->sess.clanIcon,client->sess.clanIcon2,rgb1,rgb2,script1,script2);
	//[/RGBSabers]
		}
	}
...
void Update_Server_Votes (int clientnum)
{
	...
	for ( i = 0; i < mod_votes_count; i++ )
	{
		Q_strcat(buf, MAX_INFO_STRING, mod_votes[i].votestring);
		Q_strcat(buf, MAX_INFO_STRING, ":");
	}
	//send to each client
	if( strlen(buf) ) // there may be no votes
...
============
*/
//Clan Mod - This is the cause of wrong team changes.
...
//	char		*areabits;
	...
	if ( G_FilterPacket( value ) ) {
		return "%s", roar_banned_saying.string;
	}
...
//	areabits = client->areabits;
	...
	*/
	...
	// send them server vote strings (lmo)
	Update_Server_Votes ( clientNum );

	// don't do the "xxx connected" messages if they were caried over from previous level
	//G_LogPrintf( "ClientConnect: %i '%s' -> '%s'\n", clientNum, client->pers.netname, client->sess.myip );
	if ( firstTime ) {
...
void ClientBegin( int clientNum, qboolean allowTeamReset ) {
	...
	if (client->sess.sessionTeam != TEAM_SPECTATOR && ent->client->sess.sawMOTD == qfalse) //NO MATTER WHAT
	{
		...
		if (ent->client->pers.plugindetect == qtrue) {
			trap_SendServerCommand( ent-g_entities, va("print \"^2You have the Clan Mod plugin installed properly.\n\n\n\"" ) );
		} else {
			trap_SendServerCommand( ent-g_entities, va("print \"^1You do not have the Clan Mod plugin installed. Download it at www.newagesoldier.com/clanmod\n\n\n\"" ) );
		}

		trap_SendServerCommand( ent-g_entities, va("print \"%s\n\"", roar_console_motd.string ) );
...
//	savedAreaBits = client->areabits;
	...
	//savedClientPlugIn10 = client->ojpClientPlugIn10; //v1.10 BETA
	...
	//client->ojpClientPlugIn10 = savedClientPlugIn10; //v1.10 BETA
	...
	{//jediVmerc is incompatible with this gametype, turn it off!
		...
		else
		{
			if (client->ps.fd.forcePowerLevel[FP_SABER_OFFENSE])
			{
				...
				else if (ent->client->pers.amempower)
				{
				client->ps.stats[STAT_WEAPONS] &= ~(1 << WP_BRYAR_PISTOL);
				client->ps.stats[STAT_WEAPONS] |= (1 << WP_MELEE) | (1 << WP_SABER);
				}
				else if (client->pers.amterminator)
			{
				client->ps.stats[STAT_WEAPONS] &= ~(1 << WP_SABER);
				client->ps.stats[STAT_WEAPONS] |= (1 << WP_MELEE);
			}
...
//	client->ps.ammo[AMMO_ROCKETS] = ammoData[AMMO_ROCKETS].max;
	...
	ent->client->numPlFollowers = 0;

	//RoAR mod NOTE: When u die, u should respawn with 999 ammo.
		if (cm_terminator_infammo.integer == 0 && ent->client->pers.amterminator == 1){
				int	i;
				int	num = 999;
		...
		if (ent->client->pers.amterminator == 1){
			//ITEMS
			...
			| (1 << WP_BRYAR_OLD) | (1 << WP_CONCUSSION) | (1 << WP_TRIP_MINE) | (1 << WP_BRYAR_PISTOL);
				...
				if (ent->client->pers.amterminator == 1 || ent->client->pers.amhuman == 1){
		...
		ScalePlayer(ent, ent->client->pers.ammodelchanged3);
...
void ClientDisconnect( int clientNum ) {
	...
	ent->r.contents = 0;
	//RoAR mod BEGIN
	ent->client->sess.sawMOTD = qfalse;
	//============================grapplemod===================
	if (ent->client->hook)
	{ // free it!
	...
	//============================grapplemod===================
   ent->client->pers.silent = qfalse;
   ent->client->pers.silent2 = qfalse;
   ent->client->pers.amhuman = qfalse;
   ent->client->pers.autopro = qfalse;
   ...
   ent->client->ps.persistant[PERS_MONK] = 0;
   ent->client->pers.amtelemark1 = 0;
   ent->client->pers.amtelemark2 = 0;
   ent->client->pers.amtelemark3 = 0;
   ent->client->pers.amtelemarkyaw = 0;
   ent->client->pers.forcePowersKnownSaved = 0;
	ent->client->pers.forcePowerLevelSaved[NUM_FORCE_POWERS] = 0;
   // MJN - Remove Ignore sessions.
	G_RemoveFromAllIgnoreLists( clientNum );
	ent->r.svFlags &= ~SVF_ADMIN; // prevent admin from being carried over to new players and bots
	ent->r.svFlags &= ~SVF_CLANSAY;
