...
/*
===================
G_spawngentityfromstring

Given to me by the TreeRPG mod creator himself...
===================
*/
/*
int G_SpawnGEntityFromString( char *TheString, CustSpawn_t SpawnThing ) { 
   int EndPos = -1; //endpos stores the location of the next comma 
   char *StrResult; //the set of text between the commas 
   char Key[MAX_STRING_CHARS], Value[MAX_STRING_CHARS]; //the key and value vars 
   gentity_t *ent; //the entity we will be creating 
   level.spawning = qtrue; //if we dont set this, we will get an error from the error-checking 
   //   of the G_Spawn function 
   ent = G_Spawn(); //get a new entity 

   //let the rest of the mod know that this was a custom spawned entity and 
   //   not part of the original map 
   ent->IsCustSpawn = qtrue; 

   //this way, the CustomSpawn var and the entities CustomSpawn var are exactly the same thing, so 
   //   they bolth can be referanced 
   ent->CustSpawn = &SpawnThing; 
   if(!TheString) {//err, we didnt get a spawn string... ah well, just quit 
      G_FreeEntity( ent ); //delete the entity, its not needed 
      return -1; 
   } 
   while(1){ //loop forever (untill it hits a break statement) 
      //_strnicmp 
      //StartPos = EndPos + 1; 
      //delete characters from the string untill we get to a comma 
      TheString = _strninc(TheString, EndPos + 1); 
      StrResult = strstr(TheString, ","); 

      //find the difference between the 2, so we can get only the text within the commas 
      EndPos = (int)(StrResult - TheString); 

      //Com_Printf("%s", strstr(TheString, ",")); 
      if(!StrResult || !TheString) break; //if nothing was found, then spawn the thing 
      if(EndPos > strlen(TheString) || EndPos <= 0) break; //same as above, only for a different type of error 
      //copy the text found into the Key var 
      strncpy_s(Key, sizeof(Key), TheString, EndPos); 

      //memcpy(Key, TheString, EndPos); 
      //repeat to get the Value var 
      TheString = _strninc(TheString, EndPos + 1); 
      StrResult = strstr(TheString, ","); 
      EndPos = (int)(StrResult - TheString); 
      //if nothing was found, assume the end point is the last character of the string 
      if(EndPos > strlen(TheString) || EndPos <= 0) EndPos = strlen(TheString); 
      strncpy_s(Value, sizeof(Value), TheString, EndPos); 
      //debug thing 
      //assert(strcmp(Key, "modelscale") != 0); 
      //use the pre-existing function to convert the key and value into an Entity-> setting 
      BG_ParseField(fields, Key, Value, ent); 
   } 
   if(!ent->classname){ //no classname? odd... just quit 
      G_FreeEntity( ent ); 
      return -1; 
   } 
   // move editor origin to the position to spawn the entity 
   VectorCopy( ent->s.origin, ent->s.pos.trBase ); 
   VectorCopy( ent->s.origin, ent->r.currentOrigin ); 

   // if it didnt spawn properly, then delete the entity 
   if ( !G_CallSpawn( ent ) ) { 
      G_FreeEntity( ent ); 
      return -1; 
   } 

   //check to see if the scripting system wants to do anything to the entity 
   if ( trap_ICARUS_ValidEnt( ent ) ) 
   { 
      trap_ICARUS_InitEnt( ent ); 
      ...
      { 
         if ( Q_strncmp( "NPC_", ent->classname, 4 ) != 0 ) 
         {//if its an npc, then set up npc specific stuff (for singleplayer npcs only) 
            G_ActivateBehavior( ent, BSET_SPAWN ); 
         } 
      } 
   } 
   //Com_Printf("entity number: %i\n", ent->s.number); 
   //ent->CustSpawn = &SpawnThing; 
   return ent->s.number; //return the entity number 
}
*/
