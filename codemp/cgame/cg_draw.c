...
static void CG_DrawSaberStyle( centity_t *cent, menuDef_t *menuHUD)
{
	...
	{
	//SaberMOD FEATURE : Plus de variété pour les icônes de stances
...
//	}
	...
	//Com_Printf("%d / %d\n",cg.snap->ps.fd.forcePower,maxForcePower);
...
#endif
//ClassMOD FIXME : c'est ici qu'on pourra changer les icônes des pouvoirs
...
//ClassMod FIXME : encore un truc intéressant pour la customisation des classes
/*
=================
CG_DrawCrosshair
=================
			...
			else if (crossEnt->currentState.shouldtarget || crossEnt->currentState.eType == ET_NPC)
			{
				...
				if (crossEnt->currentState.eType == ET_NPC)
				{
					...
					else
					{
						...
						//plTeam = cg_entities[cg.snap->ps.clientNum].currentState.teamowner;
					}
					...
					else if ( crossEnt->currentState.teamowner == -1 )
					{ //not on a team
						if (crossEnt->currentState.teamowner == -1 ||
...
static void CG_ScanForCrosshairEntity( void ) {
	...
	//RoAR mod NOTE: 1.19e (THIS CAUSED THE CROSSHAIR PROBLEM!!!)
	if ( cg_dynamicCrosshair.integer && cg_dynamicCrosshairPrecision.integer )
	{ //then do a trace with ghoul2 models in mind
		// lmo special crosshair trace to see tzone players in crosshair but not duel nonx stuff
		Crosshair_CG_G2Trace( &trace, start, vec3_origin, vec3_origin, end, 
...
//			ignore, CONTENTS_SOLID|CONTENTS_BODY );
		...
		// lmo special crosshair trace to see tzone players in crosshair but not duel nonx stuff
			Crosshair_CG_G2Trace( &extraTrace, cg.refdef.vieworg, vec3_origin, vec3_origin, extraEnd, 
				ignore, CONTENTS_SOLID|CONTENTS_BODY );
...
//				ignore, CONTENTS_SOLID|CONTENTS_BODY );
	...
	{
		// lmo special crosshair trace to see tzone players in crosshair but not duel nonx stuff
		Crosshair_CG_Trace( &trace, start, vec3_origin, vec3_origin, end, 
			ignore, CONTENTS_SOLID|CONTENTS_BODY );
	...
	else
	{
		...
		else
		{
			if ( cgs.gametype != GT_FFA )
				...
				baseColor = CT_WHITE; // lmo use full colour in names for FFA games
		}
	}
...
//lmo don't grey out names of duelers
...
// lmo don't fade!	tcolor[3] = color[3]*0.5f;
	tcolor[3] = color[3];

	if (cgs.gametype != GT_FFA )
		CG_SanitizeString(name, sanitized);
	else
		Q_strncpyz(sanitized, name, sizeof(sanitized)); // lmo don't sanitize name (strip colour) for FFA gametype

	if (cg_entities[cg.crosshairClientNum].currentState.bolt1 &&
			(cg_entities[cg.crosshairClientNum].currentState.bolt1 - 1) != cg.snap->ps.clientNum)
	{ //display name and duel opponent
			UI_DrawProportionalString(320, 170,
				va("%s", cgs.clientinfo[ cg.crosshairClientNum ].name),
				UI_CENTER, tcolor);
	}		
	else if (isVeh)
	{
...
//JetpackMod FIXME : pour la custo
//MOD FIXME : c pas bo, faudrait se faire un dégradé
