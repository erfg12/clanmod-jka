...
void NPC_Pain(gentity_t *self, gentity_t *attacker, int damage)
{
	...
	{//hit by a teammate
		...
		{//we weren't already enemies
			...
			{//dammit, stop that!
				...
				else if ( !( self->NPC->scriptFlags & SCF_DONT_FIRE ) )
				{//okay, we're going to turn on our ally, we need to set and lock our enemy and put ourselves in a bstate that lets us attack him (and clear any flags that would stop us)
					...
					self->NPC->scriptFlags &= ~(SCF_CROUCHED|SCF_WALKING|SCF_NO_COMBAT_TALK|SCF_FORCED_MARCH);
		...
		NPC->client->leader->client->ps.powerups[PW_CLOAKED] )
	{
		if ( !NPC->enemy )
		{
			...
			if ( NPC->enemy )
			{
				...
				if ( IsMeleeWeapon( NPC->client->ps.weapon ) && DistanceSquared( NPC->r.currentOrigin, NPC->enemy->r.currentOrigin ) > (128*128) )
				{
					G_ClearEnemy( NPC );
					//Jedi_Cloak( NPC );
				}
				...
				{
					TIMER_Set( NPC, "cloakRafale", Q_irand(3000,6000) );
				}
			}
		}
	...
	if ( other->client && other->client->ps.powerups[PW_CLOAKED] )
	{
		if ( !NPC->enemy || NPC->enemy->health <= 0 )
		{
			G_ClearEnemy( NPC );
			if ( NPC_TargetVisible( other ) )
			{
				other->flags &= ~FL_NOTARGET;
				G_SetEnemy( NPC, other );
				other->flags |= FL_NOTARGET;
			...
			else
			{
				...
				//on lui fait face
				CalcEntitySpot( NPC, SPOT_HEAD_LEAN, muzzle );
				CalcEntitySpot( other, SPOT_WEAPON, position );

				GetAnglesForDirection( muzzle, position, angles );
				angles[YAW] += flrand( -35.0f, 35.0f );

				NPCInfo->desiredYaw		= AngleNormalize360( angles[YAW] );
				NPCInfo->desiredPitch	= AngleNormalize360( angles[PITCH] );

				NPC_UpdateAngles( qtrue, qtrue );
				NPCInfo->investigateDebounceTime = level.time + Q_irand( 3000, 8000 );
				NPCInfo->behaviorState = BS_SEARCH;
			}
		}
	}
	...
	if ( NPCInfo->ignorePain == qfalse )
	{
		...
		if ( NPC->enemy != other && NPC != other && (!other->client||!other->client->ps.powerups[PW_CLOAKED]) )
...
void NPC_Touch(gentity_t *self, gentity_t *other, trace_t *trace) 
{
	...
	{//FIXME:  if pushing against another bot, both ucmd.rightmove = 127???
		...
		{
			if ( self->client->enemyTeam && self->client->enemyTeam != NPCTEAM_NEUTRAL )
			{//See if we bumped into an enemy
				...
				{//bumped into an enemy
					...
					{//MCG - Begin: checking specific BS mode here, this is bad, a HACK
						...
						if ( NPC->enemy )
						{
							if ( NPC->enemy->health < 1 || !NPC_TargetVisible2( NPC->enemy ) )
								G_ClearEnemy( NPC );
						}
						if ( !NPC->enemy )
						{//not already mad at them
		...
		if ( NPC_ValidEnemy( other ) )
		{
			if ( NPC->enemy )
			{
				if ( !NPC_ValidEnemy( NPC->enemy ) || !NPC_TargetVisible2( NPC->enemy ) )
					G_ClearEnemy( NPC );
			}
...
void NPC_Use( gentity_t *self, gentity_t *other, gentity_t *activator ) 
{
	...
	//{
		...
		//RoAR mod NOTE: Lets test this on GONKS first.
...
//		}
	...
	if ( self->client->NPC_class == CLASS_JAWA && !self->client->ps.stats[STAT_WEAPONS] )
	{
		goto endUse;
	}
