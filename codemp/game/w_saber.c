...
void WP_ActivateSaber( gentity_t *self )
{
	...
	if (self->client->ps.fd.forceGripCripple && roar_grip_open_saber.integer == 0)
	{ //can't activate saber while being gripped / RoAR mod NOTE: YOU CAN NOW!
		return;
	}
...
static int G_PowerLevelForSaberAnim( gentity_t *ent, int saberNum, qboolean mySaberHit )
{
	...
	else
	{
		...
		case LS_KICK_R_AIR:
		case LS_KICK_L_AIR:
			return FORCE_LEVEL_0; //RoAR mod NOTE: This stops staff + jump + kick glitch
			break;
		case BOTH_VS_ATR_S:
		case BOTH_VS_ATL_S:
		case BOTH_VT_ATR_S:
		...
		*/
		...
		case BOTH_ALORA_SPIN_SLASH:
			...
			return FORCE_LEVEL_3;*/
			return FORCE_LEVEL_0; //RoAR mod NOTE: people are abusing this emote
			break;
		case BOTH_A6_FB:
			if ( mySaberHit )
			{//someone else hit my saber, not asking for damage level, but defense strength
				...
				self->client->ps.fd.saberAnimLevel == SS_DUAL)
			{
				if (saberInSpecial)
				{
					...
					{ //these attacks are long and have the potential to hit a lot so they will do less damage.
						if (cm_customSaberDmgs.integer == 1){
