...
void NPC_SetMiscDefaultData( gentity_t *ent )
{
	...
	//NPCMod FIX : étend ça à tous les NPCs de classe WAMPA
	if ( ent->client->NPC_class == CLASS_WAMPA )
	{
		Wampa_SetBolts( ent );
		ent->s.g2radius = 80;//???
		ent->mass = 300;//???
	...
	//NPCMod FIX : Flame Thrower de Boba
	if ( ent->client->NPC_class == CLASS_BOBAFETT )
	{
		Boba_SetBolts( ent );
	}

	/*if ( ent->client->NPC_class == CLASS_SABOTEUR )
	{
		ent->NPC->special |= NSP_CLOAKING;
	}*/

	/*if ( ent->NPC->weaponFlags & NWF_JANGO_BLASTER )
	{
		ent->s.eFlags |= EF_JANGO_TWIN;
		ent->client->ps.eFlags |= EF_JANGO_TWIN;
		ent->NPC->jstate = JNB_LEFT;			//Comme ds EpII
	}*/
	
	//NPCMod FIX : euh bof Yoda en a fait une petite démonstration dans EPII,
 	//si vous vous en êtes pas aperçu changez de marque de shit
	/*
...
//	if ( ent->client->ps.saber[0].type != SABER_NONE )
	...
	{		
		...
		//NPCMod NOTE : interesting, ça c pour Maw
		ent->client->ps.eFlags2 |= EF2_FLYING;
		ent->count = 30; // SEEKER shot ammo count
	}
	
	//NPCMod FIX : encore une confusion datant de JO sur les classes/noms de NPCs, now
	//tous les npcs de classe CLASS_GONK sont usables
	...
	//case NPCTEAM_PLAYER:
		...
		{//good jedi
			//NPCMod FIX : nop les jedis peuvent être parfois assez hargneux dans les npcs packs
		...
		//NPCMod FIX : dans la version de Raven tous les npcs utilisant un Blaster ou des Thermals
		//utilisaient la fonction ST_ClearTimers destinée aux stormtroopers, ce fix rétablit le tout
	...
	// nest pas un humanoide
	if ( ent->localAnimIndex > 0 )
	{
		ent->flags |= FL_NO_KNOCKBACK;
	}

	if ( ent->client->NPC_class == CLASS_JAWA && !ent->client->ps.stats[STAT_WEAPONS] )
	{
		ent->NPC->defaultBehavior = ent->NPC->behaviorState = BS_FLEE;
	}
	
	if ( ent->client->NPC_class == CLASS_ROCKETTROOPER )
	{
		ent->s.eFlags2 |= EF2_FLYING;
		ent->client->ps.eFlags2 |= EF2_FLYING;
	}

	if ( ent->client->NPC_class == CLASS_ROCKETTROOPER && ent->localAnimIndex != 0 )
	{
		//ent->client->ps.pm_flags |= PMF_ROCKET_TROOPER;
		ent->client->ps.eFlags2 |= EF2_FLYING;
	}
}
...
//NPCMod FIX : là c la cata, un vieux bout de code de JO qui empêche toutes les
//customisations de JA
//NPCMod NOTE : appelé par NPC_SetWeapons
/*
...
//NPCMod : c la fête pour les NPCs de JA, plus de choix d'arme in-game et selon leur nom, on s'en remet uniquement aux .npc
/*
...
-------------------------
*/
//extern const char *spawnEffectTable[];
void NPC_SpawnEffect (gentity_t *ent)
{
	...
	orig[2] += ent->r.mins[2] - 8;

//#ifdef _DEBUG
//	Com_Printf( S_COLOR_CYAN"DEBUG : ent->r.mins[2] = %f\n", ent->r.mins[2] );
//#endif

	//en fonction de genericValue4 - cf g_misc et NPC_Spawn_Do
	//fxID = G_EffectIndex( spawnEffectTable[ent->genericValue4-1] );

	//G_PlayEffectID( fxID, orig, dir );
}
...
//	} else
	...
	*/
	...
	//NPCMod FIX : WP_NONE est enfin utilisable et les armes ne sont plus définies selon leur nom
	//NPCMod BUGFIX : il semble que ent->client->ps.weapon est à tout les coup WP_NONE ici, en gros c bof
...
//	ent->NPC->side = 1;
	...
	//ent->client->ps.ping = mod_NPCPing.integer;
	...
	{
		renderInfo_t *ri = &client->renderInfo;
		ri->headBolt = trap_G2API_AddBolt( ent->ghoul2, 0, "*head_eyes" );
	}
...
//==NPC_Connect( newent, net_name );===================================
	...
	if ( ent->NPC_type )
	{
		//NPCMod FIX : Remplacement de "kyle" par "player", ça a l'air d'un vieux bout de code de JO
...
//==New stuff=====================================================================
	...
	else if(ent->s.teamowner != TEAM_FREE || g_gametype.integer != GT_SINGLE_PLAYER)
	{
		newent->s.teamowner = ent->s.teamowner;
	}
	else
	{
		switch( newent->s.teamowner )
		{
		case NPCTEAM_PLAYER:
			newent->client->sess.sessionTeam = TEAM_BLUE;
			break;
		case NPCTEAM_ENEMY:
			newent->client->sess.sessionTeam = TEAM_RED;
			break;
		case NPCTEAM_NEUTRAL:
		case NPCTEAM_FREE:
		default:
...
finish:
	...
	if ( !(newent->spawnflags & SFB_NOTSOLID) )
	{
		if (NPC_SpotWouldTelefrag(newent))
		{
			newent->think = G_FreeEntity;
			newent->nextthink = level.time + 100;
			G_FreeEntity( newent );
			return NULL;
		}
	}
...
void NPC_ShySpawn( gentity_t *ent )
{
	...
	while( i < MAX_CLIENTS )
	{
		...
		if ( DistanceSquared( self->r.currentOrigin, ent->r.currentOrigin ) <= SHY_SPAWN_DISTANCE_SQR )
			continue;

		if ( (InFOV( ent, self, 80, 64 )) ) // FIXME: hardcoded fov
			if ( (NPC_ClearLOS2( self, ent->r.currentOrigin )) )
				continue;
...
void SP_NPC_Jedi( gentity_t *self)
{
	if(!self->NPC_type)
	{
		...
		{
			self->NPC_type = BG_StringAlloc( RandomJedi() );
		}
		else 
		{
			/*
...
/*void SP_NPC_Rosh_Penin( gentity_t *self)
{
	...
	/*if ( self->spawnflags & 2 )
		self->NPC->scriptFlags |= SCF_NO_FORCE;*/
//}
...
gentity_t *NPC_SpawnType( gentity_t *ent, char *npc_type, char *targetname, qboolean isVehicle ) 
{
	...
	for ( i = 0; i < 4; i++ )
	{
		...
		VectorMA(ent->r.currentOrigin, dist, forward, end);
...
stringID_table_t TeamTable2[] =
{
	"free",NPCTEAM_FREE,			// caution, some code checks a team_t via "if (!team_t_varname)" so I guess this should stay as entry 0, great or what? -slc
	"player",NPCTEAM_PLAYER,
	"enemy",NPCTEAM_ENEMY,
	"neutral",NPCTEAM_NEUTRAL,	// most droids are team_neutral, there are some exceptions like Probe,Seeker,Interrogator
	"",	-1
};
...
void NPC_Kill_f( void ) 
{
	...
	if ( Q_stricmp( "team", name ) == 0 )
	{
		...
		{
			killTeam = (npcteam_t)GetIDForString( TeamTable2, name );
...
void Cmd_NPC_f( gentity_t *ent ) 
{
	...
	{
		G_LogPrintf("NPC admin command executed by %s.\n", ent->client->pers.netname);
		NPC_Spawn_f( ent );
	}
	else if ( Q_stricmp( cmd, "kill" ) == 0 ) 
	{
			G_LogPrintf("NPC admin command executed by %s. (KILL ALL)\n", ent->client->pers.netname);
			NPC_Kill_f();
	}
