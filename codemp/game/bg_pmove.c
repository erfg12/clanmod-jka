...
#ifdef QAGAME
	if (!( g_dmflags.integer & DF_SLIDEONHEADS ))
#else
	if (!( cgs.dmflags & DF_SLIDEONHEADS ))
#endif
	{
		...
		BG_CanUseFPNow(pm->gametype, pm->ps, pm->cmd.serverTime, FP_LEVITATION) )
	{
		...
		{//on the ground
			...
			if ( anim != -1 )
			{
				...
				{
//#ifdef QAGAME
					//if (cm_forcekickflip.integer == 0){
					if (pm->ps->userInt2 == 0){
						pm->trace( &trace, pm->ps->origin, mins, maxs, traceto, pm->ps->clientNum, MASK_SOLID );
					} else /*if (cm_forcekickflip.integer >= 1)*/{
...
//#endif
		...
		{//in the air
			...
			{//run up wall, flip backwards
				...
				//if (cm_forcekickflip.integer == 1){
				if (pm->ps->userInt2 == 1 && pm->ps->velocity[2] > 200 && !BG_InSpecialJump(pm->ps->legsAnim) ){
				if ( trace.fraction < 1.0f && trace.entityNum < MAX_CLIENTS )
				{
					int parts = SETANIM_LEGS;
...
static void PM_GrappleMove( void ) {
	...
	{
		if ( vlen > pm->ps->userFloat1 && !(BG_InSpecialJump(pm->ps->torsoAnim)||(pm->ps->pm_flags&PMF_STUCK_TO_WALL)) )
		{	
...
static void PM_CrashLandEffect( void )
{
	...
	if ( delta >= 30 ) 
	{
		...
		if ( effectID != -1 )
		{
			G_PlayEffect( effectID, bottom, pml.groundTrace.plane.normal,ENTITYNUM_NONE );
		}
	}
}
#endif
...
static void PM_Footsteps( void ) {
	...
	else if ( pm->ps->pm_flags & PMF_DUCKED )
	{
...
#ifdef QAGAME
		if (!(g_dmflags.integer & DF_EASYROLL)){
		if ( ( (PM_RunningAnim( pm->ps->legsAnim )&&VectorLengthSquared(pm->ps->velocity)>=40000) || PM_CanRollFromSoulCal( pm->ps ) ) &&
		...
		if ( ( (PM_RunningAnim( pm->ps->legsAnim )&&VectorLengthSquared(pm->ps->velocity)>=100) || PM_CanRollFromSoulCal( pm->ps ) ) &&
			!BG_InRoll(pm->ps, pm->ps->legsAnim) )
		{
		{//roll!
			rolled = PM_TryRoll();
...
static void PM_WaterEvents( void ) {		// FIXME?
	...
	if ( impact_splash )
	{
		...
		if ( tr.fraction < 1.0f )
		{
			...
			{
				G_PlayEffect( EFFECT_LAVA_SPLASH, tr.endpos, tr.plane.normal, pm->ps->clientNum );
			}
			else if ( (tr.contents&CONTENTS_SLIME) )
			{
				G_PlayEffect( EFFECT_ACID_SPLASH, tr.endpos, tr.plane.normal, pm->ps->clientNum );
			}
			else //must be water
			{
				G_PlayEffect( EFFECT_WATER_SPLASH, tr.endpos, tr.plane.normal, pm->ps->clientNum );
			}
		}
	}
#endif
			...
			{ //ok, grapple time
				...
				if (icandoit)
				{
					...
					{ //providing the anim set succeeded..
						...
						pm->invulnerableTimer = 0;
						pm->ps->tzone = 0;
						pm->takedamage = qtrue;
						pm->ps->saberCanThrow = qtrue;
...
void BG_CmdForRoll( playerState_t *ps, int anim, usercmd_t *pCmd )
{
	...
	case BOTH_ROLL_F:
#ifdef QAGAME
		if (g_dmflags.integer & DF_JK2ROLL)
#else
...
void BG_AdjustClientSpeed(playerState_t *ps, usercmd_t *cmd, int svTime)
{
	...
	{
		ps->speed *= roar_FPGRIPspeed.integer;
	}
	#endif
	...
	{ //can't roll unless you're able to move normally
		...
		{
			ps->speed = ps->legsTimer/1.5;//450; //2.0
		}
		if (ps->speed > 600)
		{
			ps->speed = 600;
...
void PmoveSingle (pmove_t *pmove) {
...
#ifdef QAGAME
		if (!(g_dmflags.integer & DF_JK2_YELLOWDFA))
#else
		if (!(cgs.dmflags & DF_JK2_YELLOWDFA))
#endif
		{
		PM_SetPMViewAngle(pm->ps, pm->ps->viewangles, &pm->cmd);
		}
		...
		&& BG_FullBodyTauntAnim( pm->ps->torsoAnim ) )
	{
		...
		{//stop the anim
			...
			} else if ( pm->ps->legsAnim == BOTH_SLEEP6START
				&& pm->ps->torsoAnim == BOTH_SLEEP6START )
			{
				PM_SetAnim( SETANIM_BOTH, BOTH_SLEEP6START, SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD, 0 );
			}
			else
			{
				pm->ps->legsTimer = pm->ps->torsoTimer = 0;
		...
		else 
		{
			...
			//RoAR mod NOTE: Dont freeze client in taunt animation
			...
			if ( pm->ps->legsAnim == BOTH_SLEEP6START )
			{
				if ( pm->ps->legsTimer < 100 )
				{
					pm->ps->legsTimer = 100;
			...
			if ( pm->ps->torsoAnim == BOTH_SLEEP6START )
			{
				if ( pm->ps->torsoTimer < 100 )
				{
					pm->ps->legsTimer = 100;
