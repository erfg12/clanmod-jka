...
qboolean PlaceShield(gentity_t *playerent)
{
	...
	{//room in front
		...
		if ( !tr.startsolid && !tr.allsolid )
		{
			...
			shield->s.otherEntityNum = playerent->s.number; //lmo mark owner info for duel NoX
...
//------------------------------------------------------------------------------------------------------------
{
	...
	G_PlayEffect(EFFECT_EXPLOSION_PAS, self->s.pos.trBase, self->s.angles, self->s.otherEntityNum); //lmo mark effect owner
...
//------------------------------------------------------------------------
{
	...
	sentry->s.otherEntityNum = ent->s.number; // lmo mark owner info for duel NoX
	sentry->s.weapon = WP_TURRET; //lmo so I can identify the entity as sentry gun client side
...
void ItemUse_Jetpack( gentity_t *ent )
{
	...
	if (ent->client->pers.amfreeze == qtrue || ent->client->emote_freeze)
	{ // can't use jetpack when frozen
		return;
	}
...
void EWebDie(gentity_t *self, gentity_t *inflictor, gentity_t *attacker, int damage, int mod)
{
	...
	G_PlayEffect(EFFECT_EXPLOSION_DETPACK, self->r.currentOrigin, fxDir, self->s.number);
