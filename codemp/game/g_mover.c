...
qboolean G_MoverPush( gentity_t *pusher, vec3_t move, vec3_t amove, gentity_t **obstacle ) {
	...
	*obstacle = NULL;
	...
	for ( e = 0 ; e < listedEntities ; e++ ) {
		...
		//RoAR mod NOTE: Elevators kill in 1 hit?! ITS HORRIBLE ON MP/FFA3 MAP!
		if (roar_elevator_1_hit_kill.integer == 0)
		{
		...
		if ( pusher->s.pos.trType == TR_SINE || pusher->s.apos.trType == TR_SINE ) {
			G_Damage( check, pusher, pusher, NULL, NULL, 999999, 0, MOD_CRUSH );
			continue;
		}
		}
		else if (roar_elevator_1_hit_kill.integer == 1)
		{
		if (check->s.eType == ET_BODY ||
			(check->s.eType == ET_PLAYER))
		{ //whatever, just crush it
			G_Damage( check, pusher, pusher, NULL, NULL, 0, 0, MOD_CRUSH );
			continue;
		}
		// the move was blocked an entity
		// bobbing entities are instant-kill and never get blocked
