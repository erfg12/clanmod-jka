...
extern qboolean BG_AttackAnim( int anim );
...
void NPC_BSFollowLeader (void)
{
	...
	NPC_CheckEnemy( NPCInfo->confusionTime<level.time, qfalse, qtrue );
	...
	{//no enemy, find one
		...
		if ( !NPC->enemy )
		{
			if ( NPC->client->leader 
				...
				&& NPC_TargetVisible2( NPC->client->leader->enemy ) )
			{
	...
	if ( NPC->enemy && NPC->client->ps.weapon && NPC->enemy->health > 0 )
	{//If have an enemy, face him and fire
		...
		else
		{
			...
			{//face
				...
				CalcEntitySpot( NPC->enemy, SPOT_CHEST/*Q_irand( SPOT_ORIGIN,SPOT_LEGS )*//*SPOT_HEAD*/, enemy_org );
				...
				if ( enemyVisibility >= VIS_SHOOT
					&& ( !IsMeleeWeapon( NPC->client->ps.weapon )||DistanceSquared(enemy_org,muzzle)<2500 )
					)
				{//shoot
					NPC_AimAdjust( 2 );
					if ( ( NPC_GetHFOVPercentage( NPC->enemy->r.currentOrigin, NPC->r.currentOrigin, NPC->client->ps.viewangles, NPCInfo->stats.hfov ) > 0.6f 
						...
						|| ( NPC->client->ps.weapon == WP_MELEE && (NPCInfo->scriptFlags&SCF_ALT_FIRE) ) )
	...
	if ( curAnim != BOTH_ATTACK1
		...
		&& curAnim != BOTH_MELEE1
		&& curAnim != BOTH_MELEE2
		&& (NPC->client->ps.saberMove == LS_READY||NPC->client->ps.saberMove == LS_NONE||NPC->client->ps.saberMove == -1||PM_SaberInReflect(NPC->client->ps.saberMove)) )
	//if ( !( BG_AttackAnim( curAnim ) ) )
	{//Don't move toward leader if we're in a full-body attack anim
		...
		float	followDist = (NPC->enemy&&NPC->enemy->health>0)?128.0f:96.0f;//FIXME:  If there are enmies, make this larger?
