...
void NPC_ApplyScriptFlags ( int scriptFlags )
{
	if ( scriptFlags & SCF_CROUCHED )
	{
	...
	if(scriptFlags & SCF_RUNNING)
	{
		ucmd.buttons &= ~BUTTON_WALKING;
	}
	else if(scriptFlags & SCF_WALKING)
	{
		if ( NPCInfo->charmedTime > level.time && (ucmd.forwardmove || ucmd.rightmove) )
...
/*
	if(scriptFlags & SCF_CAREFUL)
	{
		ucmd.buttons |= BUTTON_CAREFUL;
	}
*/
	if(scriptFlags & SCF_LEAN_RIGHT)
	{
	...
	else if(scriptFlags & SCF_LEAN_LEFT)
	{
		ucmd.buttons |= BUTTON_USE;
		ucmd.rightmove = -127;
		ucmd.forwardmove = 0;
	...
	if ( (scriptFlags & SCF_ALT_FIRE) && (ucmd.buttons & BUTTON_ATTACK) && (NPC->client->ps.weapon != WP_DISRUPTOR) )
	{//Use altfire instead
...
void NPC_ExecuteBState ( gentity_t *self)//, int msec ) 
{
	...
	{
		if ( NPC->client->ps.fallingToDeath )
		{
			NPC_SetAnim( NPC, SETANIM_BOTH, BOTH_FALLDEATH1INAIR, SETANIM_FLAG_HOLD );
			memset( &ucmd, 0, sizeof( usercmd_t ) );
			...
			(NPC->client->NPC_class != CLASS_BOBAFETT && NPC->client->NPC_class != CLASS_ROCKETTROOPER) )
		{
			...
			{
				NPCInfo->lastAltitud = NPC->r.currentOrigin[2];
			}
			else if ( ( NPC->client->ps.velocity[2] < 0 ) && ( NPCInfo->lastAltitud ) )
			{
				trace_t		tr;
				vec3_t		end, traceMins = {-128, -128, -128}, traceMaxs = {128, 128, 128};

				VectorMA( NPC->r.currentOrigin, 65536, NPC->client->ps.velocity, end );
				trap_Trace( &tr, NPC->r.currentOrigin, traceMins, traceMaxs, end, NPC->s.number, MASK_SOLID );

				if ( ( (NPCInfo->lastAltitud - tr.endpos[2]) > 256 ) &&
					( G_GroundDistance( NPC ) > 394 ) &&
					( sqrt( (NPC->client->ps.velocity[0]*NPC->client->ps.velocity[0]) + (NPC->client->ps.velocity[1]*NPC->client->ps.velocity[1]) ) < abs(NPC->client->ps.velocity[2]) ) &&
					( ( (NPC->client->ps.lastOnGround + 800) < level.time ) || !( WP_ForcePowerAvailable( NPC, FP_LEVITATION, qfalse ) && WP_ForcePowerUsable( NPC, FP_LEVITATION) ) ) )
				{
					...
					NPCInfo->defaultBehavior = NPCInfo->behaviorState = NPCInfo->tempBehavior = BS_FLEE;
...
//	CheckSelf();
	...
	if ( NPC->enemy )
	{
		...
		{
			if ( NPC_TargetVisible2 ( NPC->enemy ) )
			{
				NPC->client->enemyLastSeenTime2 = level.time;
			}
			else if ( (level.time - NPC->client->enemyLastSeenTime2) > (((NPCInfo->stats.aggression*2)-NPCInfo->stats.intelligence)*1000) )
			{
				G_ClearEnemy( NPC );
			}
		}
	...
	if ( NPC->enemy )
	{
		...
		else if ( NPC->enemy->NPC && (NPC->enemy->NPC->surrenderTime > level.time || (NPC->enemy->NPC->scriptFlags&SCF_FORCED_MARCH)) )
	...
	NPC_CheckAttackHold();
	NPC_ApplyScriptFlags( NPCInfo->scriptFlags );
...
{	// FIXME : once torsoAnim and legsAnim are in the same structure for NCP and Players
	...
	if ( ent->client && (ent->client->ps.eFlags2&EF2_MD3_MODEL) )
	{
		return;
	}
