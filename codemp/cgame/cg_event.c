...
void CG_EntityEvent( centity_t *cent, vec3_t position ) {
	...
	case EV_PRIVATE_DUEL:
		...
		{ //starting the duel
			//if (es->eventParm == 5)
			if (es->eventParm == 2) //RoAR mod NOTE: OH! Found it.
			{
				CG_CenterPrint( CG_GetStringEdString("MP_SVGAME", "BEGIN_DUEL"), 120, GIANTCHAR_WIDTH*2 );				
				trap_S_StartLocalSound( cgs.media.countFightSound, CHAN_ANNOUNCER );
			}
			...
			{ // signalling duel type with parameter number. Also, for clients in duel, start duel music
				cg_dueltypes[es->number] = es->eventParm - 1; // set dueltype for partner #1
...
//		trap_S_StartSound (NULL, es->number, CHAN_AUTO, cgs.media.noAmmoSound );
	...
	case EV_SABER_HIT:
		DEBUGNAME("EV_SABER_HIT");
		{
			...
			if (doit) {
				if ( es->otherEntityNum >= 0					//lmo reversed Num and Num2 for consitancy with my nox code (see w_saber.c for where ev_hits are generated
					&& es->otherEntityNum < ENTITYNUM_NONE )
...
//					&& es->otherEntityNum2 < ENTITYNUM_NONE )
				{//we have a specific person who is causing this effect, see if we should override it with any custom saber effects/sounds
					clientInfo_t *client = NULL;
					if ( cg_entities[es->otherEntityNum].currentState.eType == ET_NPC )
//					if ( cg_entities[es->otherEntityNum2].currentState.eType == ET_NPC )
					{
						client = cg_entities[es->otherEntityNum].npcClient;
...
//						client = &cgs.clientinfo[es->otherEntityNum2];
				...
				if (es->otherEntityNum != ENTITYNUM_NONE && es->otherEntityNum2 != ENTITYNUM_NONE)
				{
					...
					saberOwner = &cg_entities[es->otherEntityNum];
//					saberOwner = &cg_entities[es->otherEntityNum2];

					saberOwner->serverSaberHitIndex = es->otherEntityNum2;
