...
//#ifndef FINAL_BUILD
	...
	//CG_Printf( "Unknown custom sound: %s\n", lSoundName );
	//[RACC]
//#endif
	return 0;
}
...
//[/RGBSabers]
	...
	v = Info_ValueForKey(configstring, "cln");
	Q_strncpyz( newInfo.clan, v, sizeof( newInfo.clan ) );

	v = Info_ValueForKey(configstring, "rnk");
	Q_strncpyz( newInfo.rank, v, sizeof( newInfo.rank ) );
...
//	Q_strncpyz(newInfo.blueTeam, v, MAX_TEAMNAME);
	...
	//clan icon
	v = Info_ValueForKey( configstring, "ci" );

	if (v)
	{
		newInfo.clanIcon = atoi(v);
	}
	else
	{
		newInfo.clanIcon = 0;
	...
	v = Info_ValueForKey( configstring, "ci2" );

	if (v)
	{
		newInfo.clanIcon2 = atoi(v);
	}
	else
	{
		newInfo.clanIcon2 = 0;
...
static void CG_ForcePushBodyBlur( centity_t *cent )
{
	...
	if ( cent->currentState.eFlags2 & EF2_MD3_MODEL )
		return;
...
//[RGBSabers]
	if(((ojp_teamrgbsabers.integer == 1 && cg.snap && cg.snap->ps.clientNum != cent->currentState.number)) &&
					...
					if ( client->saber[saberNum].blade[bladeNum].trail.haveOldPos[i] )
					{
						...
						{//only put marks on architecture
							...
							{//ugh, need to have a real sound debouncer... or do this game-side
								...
								trap_S_StartSound ( trace.endpos, cent->currentState.number, CHAN_WEAPON, trap_S_RegisterSound( va("sound/weapons/saber/saberhitwall%i", Q_irand(1, 3)) ) );
							}
						}
					}
					else
...
void CG_Player( centity_t *cent ) {
	...
	//RoAR mod NOTE: I assume this is part of Duel Mod.
	...
	//NPCMod FIX : Boba the come back!!!
	if ((cent->currentState.NPC_class == CLASS_BOBAFETT || cent->currentState.NPC_class == CLASS_ROCKETTROOPER) && (cent->currentState.eFlags2 & EF2_FLYING) && !(cent->currentState.eFlags & EF_DEAD))
	{
		mdxaBone_t	mat;
		vec3_t	flamePos, flameDir;
		qboolean	wj = qfalse;
		fxHandle_t	flameEffect = (cent->currentState.NPC_class == CLASS_BOBAFETT)?cgs.effects.mBobaJet:cgs.effects.mBobaJet;
		
		if (cent->bobaInit == qfalse)
		{
			cent->bolt_jet1 = trap_G2API_AddBolt(cent->ghoul2, 0, "*jet1");
			cent->bolt_jet2 = trap_G2API_AddBolt(cent->ghoul2, 0, "*jet2");

			if ((cent->bolt_jet1 == -1) || (cent->bolt_jet2 == -1))
			{
				cent->bolt_jet1 = trap_G2API_AddBolt(cent->ghoul2, 0, "*hip_br");
				cent->bolt_jet2 = trap_G2API_AddBolt(cent->ghoul2, 0, "*hip_bl");
		...
		{
  			//1st flamme
  			trap_G2API_GetBoltMatrix(cent->ghoul2, 0, cent->bolt_jet1, &mat, cent->turAngles, cent->lerpOrigin, cg.time, cgs.gameModels, cent->modelScale);
			BG_GiveMeVectorFromMatrix(&mat, ORIGIN, flamePos);
			...
			//NPCMod FIXME : ne tient pas compte de son mouvement, s'il monte la flamme doit être différente(voir pour les joueurs)
			trap_FX_PlayEffectID(flameEffect, flamePos, flameDir, -1, -1);
			
			//2nde flamme
			trap_G2API_GetBoltMatrix(cent->ghoul2, 0, cent->bolt_jet2, &mat, cent->turAngles, cent->lerpOrigin, cg.time, cgs.gameModels, cent->modelScale);
			BG_GiveMeVectorFromMatrix(&mat, ORIGIN, flamePos);
...
// minimal_add:
	...
	if (cgs.gametype == GT_TEAM){
	if (cg.snap->ps.userInt1 == 1 && cent->currentState.number != cg.snap->ps.clientNum &&
			cent->currentState.number < MAX_CLIENTS && (cg.snap->ps.persistant[PERS_TEAM] == TEAM_SPECTATOR || cg.snap->ps.persistant[PERS_TEAM] == team) &&
			!(cent->currentState.eFlags & EF_DEAD)){
				if (ci->clanIcon == CLANICON_NONE) { 
				CG_PlayerFloatSprite( cent, trap_R_RegisterShaderNoMip ( va("clanicons/none" )));
				}
				else if (ci->clanIcon == CLANICON_BLOOD) { 
				CG_PlayerFloatSprite( cent, trap_R_RegisterShaderNoMip ( va("clanicons/blood" )));
				}
				else if (ci->clanIcon == CLANICON_BOMB) { 
				CG_PlayerFloatSprite( cent, trap_R_RegisterShaderNoMip ( va("clanicons/bomb" )));
				}
				else if (ci->clanIcon == CLANICON_CROSS) { 
				CG_PlayerFloatSprite( cent, trap_R_RegisterShaderNoMip ( va("clanicons/cross" )));
				}
				else if (ci->clanIcon == CLANICON_FLOWER) { 
				CG_PlayerFloatSprite( cent, trap_R_RegisterShaderNoMip ( va("clanicons/flower" )));
				}
				else if (ci->clanIcon == CLANICON_HAPPY) { 
				CG_PlayerFloatSprite( cent, trap_R_RegisterShaderNoMip ( va("clanicons/happy" )));
				}
				else if (ci->clanIcon == CLANICON_IMPERIAL) { 
				CG_PlayerFloatSprite( cent, trap_R_RegisterShaderNoMip ( va("clanicons/imperial" )));
				}
				else if (ci->clanIcon == CLANICON_JAPANESE) { 
				CG_PlayerFloatSprite( cent, trap_R_RegisterShaderNoMip ( va("clanicons/japanese" )));
				}
				else if (ci->clanIcon == CLANICON_LOST) { 
				CG_PlayerFloatSprite( cent, trap_R_RegisterShaderNoMip ( va("clanicons/lost" )));
				}
				else if (ci->clanIcon == CLANICON_MANDALORIAN) { 
				CG_PlayerFloatSprite( cent, trap_R_RegisterShaderNoMip ( va("clanicons/mandalorian" )));
				}
				else if (ci->clanIcon == CLANICON_NUKLEAR) { 
				CG_PlayerFloatSprite( cent, trap_R_RegisterShaderNoMip ( va("clanicons/nuklear" )));
				}
				else if (ci->clanIcon == CLANICON_NUMBA1) { 
				CG_PlayerFloatSprite( cent, trap_R_RegisterShaderNoMip ( va("clanicons/numba1" )));
				}
				else if (ci->clanIcon == CLANICON_PEACE) { 
				CG_PlayerFloatSprite( cent, trap_R_RegisterShaderNoMip ( va("clanicons/peace" )));
				}
				else if (ci->clanIcon == CLANICON_REBEL) { 
				CG_PlayerFloatSprite( cent, trap_R_RegisterShaderNoMip ( va("clanicons/rebel" )));
				}
				else if (ci->clanIcon == CLANICON_ROAR) { 
				CG_PlayerFloatSprite( cent, trap_R_RegisterShaderNoMip ( va("clanicons/roar" )));
				}
				else if (ci->clanIcon == CLANICON_SITH) { 
				CG_PlayerFloatSprite( cent, trap_R_RegisterShaderNoMip ( va("clanicons/sith" )));
				}
				else if (ci->clanIcon == CLANICON_SKULL) { 
				CG_PlayerFloatSprite( cent, trap_R_RegisterShaderNoMip ( va("clanicons/skull" )));
				}
				else if (ci->clanIcon == CLANICON_TIE) { 
				CG_PlayerFloatSprite( cent, trap_R_RegisterShaderNoMip ( va("clanicons/tie" )));
				}
				else if (ci->clanIcon == CLANICON_UNHAPPY) { 
				CG_PlayerFloatSprite( cent, trap_R_RegisterShaderNoMip ( va("clanicons/unhappy" )));
				}
				else if (ci->clanIcon == CLANICON_UNKNOWN) { 
				CG_PlayerFloatSprite( cent, trap_R_RegisterShaderNoMip ( va("clanicons/unknown" )));
				}
				else if (ci->clanIcon == CLANICON_XWING) { 
				CG_PlayerFloatSprite( cent, trap_R_RegisterShaderNoMip ( va("clanicons/xwing" )));
				}
				else if (ci->clanIcon == CLANICON_YINYANG) { 
				CG_PlayerFloatSprite( cent, trap_R_RegisterShaderNoMip ( va("clanicons/yinyang" )));
				}
				else if (ci->clanIcon == CLANICON_YODA) { 
				CG_PlayerFloatSprite( cent, trap_R_RegisterShaderNoMip ( va("clanicons/yoda" )));
				}
				if (ci->clanIcon2 == CLANICON2_NONE) { 
				CG_PlayerFloatSprite( cent, trap_R_RegisterShaderNoMip ( va("clanicons/none" )));
				}
				else if (ci->clanIcon2 == CLANICON2_7CORNERS_RED) { 
				CG_PlayerFloatSprite( cent, trap_R_RegisterShaderNoMip ( va("clanicons/layer2_7corners_red" )));
				}
				else if (ci->clanIcon2 == CLANICON2_7CORNERS_GREEN) { 
				CG_PlayerFloatSprite( cent, trap_R_RegisterShaderNoMip ( va("clanicons/layer2_7corners_green" )));
				}
				else if (ci->clanIcon2 == CLANICON2_7CORNERS_BLUE) { 
				CG_PlayerFloatSprite( cent, trap_R_RegisterShaderNoMip ( va("clanicons/layer2_7corners_blue" )));
				}
				else if (ci->clanIcon2 == CLANICON2_DONUTS_RED) { 
				CG_PlayerFloatSprite( cent, trap_R_RegisterShaderNoMip ( va("clanicons/layer2_donut_red" )));
				}
				else if (ci->clanIcon2 == CLANICON2_DONUTS_GREEN) { 
				CG_PlayerFloatSprite( cent, trap_R_RegisterShaderNoMip ( va("clanicons/layer2_donut_green" )));
				}
				else if (ci->clanIcon2 == CLANICON2_DONUTS_BLUE) { 
				CG_PlayerFloatSprite( cent, trap_R_RegisterShaderNoMip ( va("clanicons/layer2_donut_blue" )));
				}
				else if (ci->clanIcon2 == CLANICON2_SPARK_RED) { 
				CG_PlayerFloatSprite( cent, trap_R_RegisterShaderNoMip ( va("clanicons/layer2_spark_red" )));
				}
				else if (ci->clanIcon2 == CLANICON2_SPARK_GREEN) { 
				CG_PlayerFloatSprite( cent, trap_R_RegisterShaderNoMip ( va("clanicons/layer2_spark_green" )));
				}
				else if (ci->clanIcon2 == CLANICON2_SPARK_BLUE) { 
				CG_PlayerFloatSprite( cent, trap_R_RegisterShaderNoMip ( va("clanicons/layer2_spark_blue" )));
				}
				else if (ci->clanIcon2 == CLANICON2_SPARK_BLACK) { 
				CG_PlayerFloatSprite( cent, trap_R_RegisterShaderNoMip ( va("clanicons/layer2_spark_black" )));
				}
				else if (ci->clanIcon2 == CLANICON2_STAR_RED) { 
				CG_PlayerFloatSprite( cent, trap_R_RegisterShaderNoMip ( va("clanicons/layer2_star_red" )));
				}
				else if (ci->clanIcon2 == CLANICON2_STAR_GREEN) { 
				CG_PlayerFloatSprite( cent, trap_R_RegisterShaderNoMip ( va("clanicons/layer2_star_green" )));
				}
				else if (ci->clanIcon2 == CLANICON2_STAR_BLUE) { 
				CG_PlayerFloatSprite( cent, trap_R_RegisterShaderNoMip ( va("clanicons/layer2_star_blue" )));
				}
				else if (ci->clanIcon2 == CLANICON2_STAR_BLACK) { 
				CG_PlayerFloatSprite( cent, trap_R_RegisterShaderNoMip ( va("clanicons/layer2_star_black" )));
				}
			}	
	}
	if (cgs.gametype >= GT_TEAM && cg.snap->ps.userInt1 == 0 && cg_drawFriend.integer &&
	...
	/*else if (cgs.gametype == GT_RPG && cent->currentState.number == cg.snap->ps.clientNum &&
			cent->currentState.number < MAX_CLIENTS &&
			!(cent->currentState.eFlags & EF_DEAD) && cg.snap->ps.userInt2 == 1){
		CG_PlayerFloatSprite( cent, trap_R_RegisterShaderNoMip ( "clanicons/skull" ));
	}
	else if (cgs.gametype == GT_RPG && cent->currentState.number < MAX_CLIENTS && 
		!(cent->currentState.eFlags & EF_DEAD) && cg.snap->ps.userInt2 == 1 && cent->currentState.eType != ET_NPC){
		CG_PlayerFloatSprite( cent, trap_R_RegisterShaderNoMip ( "clanicons/skull" ));
	}*/
...
stillDoSaber:
	...
	//RoAR mod NOTE: DUEL EFFECTS HERE!
	/*if (cg.snap->ps.duelInProgress)
	...
	//RoAR mod NOTE: Thanks LastHopeMod!
	/*if (cg_entities[cent->currentState.number].amterminator )
	{ // blue electricity
		legs.renderfx &= ~RF_FORCE_ENT_ALPHA;
		legs.renderfx &= ~RF_MINLIGHT;
	...
	if (cg_entities[cent->currentState.number].amfreeze )
	{ //aborb light blue
		refEntity_t prot;
		...
		prot.shaderRGBA[0] = 0;
		prot.shaderRGBA[1] = 0;
		prot.shaderRGBA[2] = 100;
		prot.shaderRGBA[3] = 100;
	...
	if (cg_entities[cent->currentState.number].amdemigod )
	{ // gold
		legs.shaderRGBA[0] = 128;
		legs.shaderRGBA[1] = 127;
		legs.shaderRGBA[2] = 0;
		legs.shaderRGBA[3] = 200;
	...
	if (cg_entities[cent->currentState.number].amhuman )
	{ //aborb light red
		refEntity_t prot;
		
		memcpy(&prot, &legs, sizeof(prot));

		prot.shaderRGBA[0] = 25;
		prot.shaderRGBA[1] = 0;
		prot.shaderRGBA[2] = 0;
