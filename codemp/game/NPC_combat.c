...
void NPC_ChooseWeapon()
{
	if (!TIMER_Done(NPC,"chooseWeap") || !(NPC->enemy))
		return;

	if (!NPC->client->ps.stats[STAT_WEAPONS])
	{
		//NPC_ChangeWeapon(WP_NONE);
		NPC_CheckGetNewWeapon();
		TIMER_Set(NPC,"chooseWeap",Q_irand(3000,6000));
		return;
	}

	switch(NPC->client->NPC_class)
	{
	case CLASS_BOBAFETT:	//déjà implémenté dans NPC_AI_Jedi
	...
	else
	{
		...
		{
		case CLASS_ROCKETTROOPER:
			TIMER_Set(NPC,"chooseWeap",Q_irand(10000,20000));
			break;
		case CLASS_KYLE:
			TIMER_Set(NPC,"chooseWeap",Q_irand(3000,6000));
			break;
		default:
			TIMER_Set(NPC,"chooseWeap",Q_irand(4500,11000));
			break;
		}
	}
}
...
{//FIXME: Probably a better way to do this, is a linked list of your teammates already available?
	...
	return Q_irand(0,40)?qfalse:qtrue;
}
	...
	{//delay their attack based on how far away they're facing from enemy
		...
		*/
		...
		case WP_BLASTER:
			...
			break;
		case WP_MELEE:
			//attDelay -= ( self->NPC->scriptFlags & SCF_ALT_FIRE )?weaponData[WP_MELEE].altFireTime:weaponData[WP_MELEE].fireTime;
			//attDelay -= Q_irand( 0, 500 );
			break;
		case WP_BOWCASTER:
			attDelay += Q_irand( 0, 500 );
...
//			break;
		...
		*/
		...
		//if ( self->client->playerTeam == NPCTEAM_PLAYER )
		if ( self->client->NPC_class == CLASS_REBEL || self->NPC->rank >= RANK_COMMANDER )
		{//clamp it
			if ( attDelay > 2000 )
			{
				attDelay = 2000;
...
void G_SetEnemy( gentity_t *self, gentity_t *enemy )
{
	...
	{
#ifdef _DEBUG
		Com_Printf( S_COLOR_YELLOW"WARNING : Trying to set a NULL enemy\n" );
#endif
		return;
	}
	...
	{
#ifdef _DEBUG
		Com_Printf( S_COLOR_YELLOW"WARNING : Trying to set himself as enemy\n" );
#endif
		return;
	}
	...
	{
#ifdef _DEBUG
		Com_Printf( S_COLOR_YELLOW"WARNING : Trying to set a inuse enemy\n" );
#endif
		return;
	}

	if ( enemy->s.number == ENTITYNUM_WORLD )
	{
#ifdef _DEBUG
		Com_Printf( S_COLOR_YELLOW"WARNING : Trying to set ENTITYNUM_WORLD as enemy\n" );
#endif
		return;
	}
...
//#endif
	...
	{
		self->enemy = enemy;
		//self->client->enemyLastSeenTime2 = level.time;
		return;
	}
...
//	}
	...
	if ( self->enemy == NULL )
	{
		...
		{//Hmm, how about sniper and bowcaster?
			...
			//if ( self->client->playerTeam == NPCTEAM_PLAYER )
			if ( self->client->NPC_class == CLASS_REBEL )
			{
...
void ChangeWeapon( gentity_t *ent, int newWeapon ) 
{
	...
	//NPCMod FEATURE : nouveau système pour les shoots des NPCs, on utilise l'agression pour moduler la valeur
	switch ( newWeapon ) 
	{
	case WP_BRYAR_PISTOL://prifle
		/*if ( !ent->NPC->weaponFlags & NWF_JANGO_BLASTER )
		{
			ent->NPC->aiFlags &= ~NPCAI_BURST_WEAPON;
			ent->NPC->burstSpacing = 1000 / ent->NPC->stats.aggression;//attackdebounce

			if (!(ent->client->ps.eFlags & EF_DUAL_WEAPONS))
				ent->NPC->burstSpacing *= 2;
		}
		else
		{*/
			...
			ent->NPC->burstSpacing = 3000 / ent->NPC->stats.aggression;//attackdebounce
		//}
	...
	case WP_DISRUPTOR:
		...
		if ( ent->NPC->scriptFlags & SCF_ALT_FIRE )
		{
			...
			//ent->NPC->aiFlags |= NPCAI_BURST_WEAPON;
			//ent->NPC->burstMin = 8;
			//ent->NPC->burstMean = Q_irand(8,20);
			//ent->NPC->burstMax = 20;
			ent->NPC->burstMean = 0;
			ent->NPC->burstCount = 0;
			...
			{
			case 0:
				ent->NPC->burstSpacing = 10000;//attackdebounce
				break;
			case 1:
				ent->NPC->burstSpacing = 9000;//attackdebounce
				break;
			case 2:
				ent->NPC->burstSpacing = 8000;//attackdebounce
				break;
			}

			ent->NPC->burstSpacing /= ent->NPC->stats.aggression;
		}
		else
		{
			//ent->NPC->burstSpacing = 1000;//attackdebounce
	...
	//	ent->NPC->burstSpacing = 1000;//attackdebounce
		if ( g_spskill.integer == 0 )
			ent->NPC->burstSpacing = 4000;//attack debounce
		else if ( g_spskill.integer == 1 )
	...
	case WP_REPEATER:
		...
		{
			ent->NPC->aiFlags |= NPCAI_BURST_WEAPON;
			ent->NPC->burstMin = 3 + ent->NPC->stats.aggression;
			ent->NPC->burstMax = 10 + ( 2 * ent->NPC->stats.aggression );
			ent->NPC->burstMean = Q_irand(ent->NPC->burstMin,ent->NPC->burstMax);
			if ( g_spskill.integer == 0 )
				ent->NPC->burstSpacing = 5000;//attack debounce
			else if ( g_spskill.integer == 1 )
				ent->NPC->burstSpacing = 4500;//attack debounce
			else 
				ent->NPC->burstSpacing = 4250;//attack debounce
			ent->NPC->burstSpacing /= ent->NPC->stats.aggression;
		}
		break;
	...
	case WP_DEMP2:
		ent->NPC->aiFlags &= ~NPCAI_BURST_WEAPON;
		if ( ent->NPC->scriptFlags & SCF_ALT_FIRE )
			ent->NPC->burstSpacing = 5000;//attackdebounce
		else
			ent->NPC->burstSpacing = 2500;//attackdebounce
		ent->NPC->burstSpacing /= ent->NPC->stats.aggression;
		break;
	...
	case WP_FLECHETTE:
		...
		if ( ent->NPC->scriptFlags & SCF_ALT_FIRE )
		{
			ent->NPC->burstSpacing = 4500;//attackdebounce
		}
		else
		{
			ent->NPC->burstSpacing = 3500;//attackdebounce
		}
		ent->NPC->burstSpacing /= ent->NPC->stats.aggression;
		break;
	...
	case WP_ROCKET_LAUNCHER:
		...
		{
			if ( g_spskill.integer == 0 )
				ent->NPC->burstSpacing = 10000;//attack debounce
			else if ( g_spskill.integer == 1 )
				ent->NPC->burstSpacing = 8000;//attack debounce
			else 
				ent->NPC->burstSpacing = 7000;//attack debounce
		}
		else
		{
			if ( g_spskill.integer == 0 )
				...
				ent->NPC->burstSpacing = 6000;//attack debounce
			else 
				ent->NPC->burstSpacing = 5500;//attack debounce
		}
		ent->NPC->burstSpacing /= ent->NPC->stats.aggression;
		break;
	...
	*/
	...
	case WP_BLASTER:
		if ( ent->NPC->scriptFlags & SCF_ALT_FIRE )
		{
			ent->NPC->aiFlags |= NPCAI_BURST_WEAPON;
			ent->NPC->burstMin = 3 * ent->NPC->stats.aggression;
			ent->NPC->burstMean = Q_irand(3,6) * ent->NPC->stats.aggression;
			ent->NPC->burstMax = 6 * ent->NPC->stats.aggression;
			/*if (ent->client->ps.eFlags & EF_DUAL_WEAPONS)
			{
				ent->NPC->burstMin = Q_irand(5,8) * ent->NPC->stats.aggression;
				ent->NPC->burstMean = Q_irand(5,8) * ent->NPC->stats.aggression;
				ent->NPC->burstMax = Q_irand(5,8) * ent->NPC->stats.aggression;
				if (ent->NPC->burstMax < ent->NPC->burstMin)
					ent->NPC->burstMax = ent->NPC->burstMin;

				if ((ent->NPC->burstMean < ent->NPC->burstMin) || (ent->NPC->burstMean > ent->NPC->burstMax))
				{
					if (Q_irand(0,1))
						ent->NPC->burstMean = ent->NPC->burstMin;
					else
						ent->NPC->burstMean = ent->NPC->burstMax;
				}
			}*/
			...
			if ( g_spskill.integer == 0 )
				...
				ent->NPC->burstSpacing = 3250;//attack debounce
		}
		else
		{
			ent->NPC->aiFlags &= ~NPCAI_BURST_WEAPON;
			if ( g_spskill.integer == 0 )
				...
				ent->NPC->burstSpacing = 1750;//attack debounce
			else 
	...
	case WP_STUN_BATON:
		ent->NPC->aiFlags &= ~NPCAI_BURST_WEAPON;
		ent->NPC->burstSpacing = Q_irand(1500,3000);//attackdebounce
		ent->NPC->burstSpacing /= ent->NPC->stats.aggression;
		break;
...
{
	////NPCMod FIX : Optimise à donf
	//if ( NPC->client->ps.weapon == newWeapon )
	//	return;

	//NPCMod FIX : nottament pour Boba,évite de changer d'arme à chaque frame
	if ( NPC->client->ps.weapon != newWeapon )
...
*/
	...
	/*if ( NPC->client->ps.weapon == WP_BRYAR_PISTOL &&
		(NPCInfo->weaponFlags&NWF_JANGO_BLASTER) )
	{
		NPCInfo->scriptFlags &= ~SCF_ALT_FIRE;
		if ( !NPCInfo->burstMean )
		{
			NPCInfo->burstMean = Q_irand(3,9);
			NPCInfo->jstate = Q_irand(JNB_LEFT,JNB_BOTH);
		}

		switch(NPCInfo->jstate)
		{
		case JNB_LEFT:
			break;
		case JNB_RIGHT:
...
//#ifdef _DEBUG
//		Com_Printf( "NPC->client->ps.zoomMode = %d\n", NPC->client->ps.zoomMode );
//		Com_Printf( "NPC->client->ps.zoomLocked = %d\n", NPC->client->ps.zoomLocked );
//		Com_Printf( "NPC->client->ps.zoomLockTime = %d\n", NPC->client->ps.zoomLockTime );
//		Com_Printf( "NPC->client->ps.zoomFov = %d\n", NPC->client->ps.zoomFov );
//		Com_Printf( "NPC->client->ps.zoomTime = %d\n\n", NPC->client->ps.zoomTime );
//#endif

		if ( !NPC->client->ps.zoomMode )
		{
			NPCInfo->burstMean = level.time + ( Q_irand(200,400) * (6-NPCInfo->stats.aim) );
			ucmd.buttons &= ~BUTTON_ATTACK;
			ucmd.buttons |= BUTTON_ALT_ATTACK;

//#ifdef _DEBUG
//			Com_Printf( "Phase 1 : !NPC->client->ps.zoomMode\n" );
//#endif
		}
		else if ( !NPC->client->ps.zoomLocked )
		{
			if ( NPCInfo->burstMean > level.time )
			{
				ucmd.buttons &= ~BUTTON_ATTACK;
				ucmd.buttons |= BUTTON_ALT_ATTACK;

//#ifdef _DEBUG
//				Com_Printf( "Phase 2 : !NPC->client->ps.zoomLocked && NPCInfo->burstMean > level.time\n" );
//#endif
			}
			else
			{
				...
				NPCInfo->burstCount = 0;
				NPCInfo->burstMean = level.time + ( Q_irand(100,120) * (7-NPCInfo->stats.aggression) );

//#ifdef _DEBUG
//				Com_Printf( "Phase 3 : !NPC->client->ps.zoomLocked && NPCInfo->burstMean <= level.time\n" );
//#endif
			}
		}
		else
		{
			if ( !NPCInfo->burstCount )
			{
				...
				{
					NPCInfo->burstCount++;
					NPCInfo->burstMean = level.time + ( Q_irand(100,200) * (NPCInfo->stats.intelligence+6-NPCInfo->stats.aim) );

//#ifdef _DEBUG
//					Com_Printf( "Phase 4 : !NPCInfo->burstCount && NPCInfo->burstMean <= level.time\n" );
//#endif
				}
			}
			else if ( NPCInfo->burstCount == 1 )
			{
				if ( NPCInfo->burstMean <= level.time )
				{
					//ucmd |= BUT_ATTACK; quedal à faire donc
//#ifdef _DEBUG
//					Com_Printf( "Phase 5 : NPCInfo->burstCount && NPCInfo->burstMean <= level.time\n" );
//#endif
				}
				else
				{
					...
					NPCInfo->burstMean = 0;
//#ifdef _DEBUG
//					Com_Printf( "Phase 6 : SHOOTT!!!\n" );
//#endif
				}
			}
			else
			{
				//On remet manuellement, sinon ça cause qqes probs avec la suppression de atackHoldTime
				ucmd.buttons &= ~(BUTTON_ATTACK|BUTTON_ALT_ATTACK);
				NPCInfo->burstCount = 0;
				NPC->client->ps.zoomMode = 0;
				NPC->client->ps.zoomFov = 0;
				NPC->client->ps.zoomLocked = qfalse;
				NPC->client->ps.zoomLockTime = 0;
	...
	{
//#ifdef _DEBUG
//		Com_Printf( "NPC->client->ps.weaponChargeTime = %d\n", NPC->client->ps.weaponChargeTime );
//		Com_Printf( "level.time = %d\n", level.time );
//		Com_Printf( "NPCInfo->burstCount = %d\n", NPCInfo->burstCount );
//		Com_Printf( "NPCInfo->burstMean = %d\n\n", NPCInfo->burstMean );
//#endif

		delay = 0;

		if ( NPC->client->ps.weaponChargeTime > (level.time - NPCInfo->burstCount) )
		{
			//quedal
//#ifdef _DEBUG
//			Com_Printf( "PHASE 2\n");
//#endif
		...
		else if ( !NPCInfo->burstMean )
		{
			NPCInfo->burstMean = 1;
			NPCInfo->burstCount = Q_irand(1500,4000);
//#ifdef _DEBUG
//			Com_Printf( "PHASE 1\n");
//#endif
		}
		...
		{
			ucmd.buttons &= ~BUTTON_ATTACK;		//on lache et on fait tt péter
			delay = NPCInfo->burstSpacing;
			NPCInfo->burstMean = 0;
//#ifdef _DEBUG
//			Com_Printf( "PHASE 3\n\n");
//#endif
	...
	//NPC->client->ps.weaponTime += delay;
...
void WeaponThink( qboolean inCombat ) 
{
	...
	{
		NPCInfo->jediAggression = Q_irand(6,8);
		//Jedi_Decloak( NPC );
	}
...
//NPCMod FEATURE / FIX : pour Boba nottament
...
qboolean CanShoot ( gentity_t *ent, gentity_t *shooter ) 
{
	...
	CalcEntitySpot( ent, SPOT_CHEST, spot );		//FIXME preferred target locations for some weapons (feet for R/L)
...
//MCG - End
	...
	if ( traceEnt->client && ( traceEnt->client->playerTeam == shooter->client->playerTeam ) && traceEnt != shooter->enemy ) 
	{
		return qfalse;
	}
...
qboolean ValidEnemy(gentity_t *ent)
{
	...
	//			if( ( entTeam == NPCTEAM_FREE && ( ent->enemy == NPC || ( ent->enemy && ent->enemy->client && ent->enemy->client->playerTeam == NPC->client->playerTeam ) ) )
	//				|| ( NPC->client->enemyTeam == NPCTEAM_FREE && ent->client->NPC_class != NPC->client->NPC_class )
...
gentity_t *NPC_CheckEnemy( qboolean findNew, qboolean tooFarOk, qboolean setEnemy )
{
	...
	if ( NPC->r.svFlags & SVF_IGNORE_ENEMIES )
	...
	if ( NPC->enemy 
		&& ( NPC->enemy->health <= 0 
			//|| DistanceSquared( NPC->r.currentOrigin, NPC->enemy->r.currentOrigin ) > ( NPCInfo->stats.visrange * NPCInfo->stats.visrange * 8 )
			/*|| (NPC->enemy->client && (NPC->enemy->client->ps.pm_flags & PM_DEAD ) )*/
			)
		)
	{
		G_ClearEnemy( NPC );
	...
	newEnemy = NPC_PickEnemyExt( qtrue );
...
//			newEnemy = NPC_PickEnemy( closestTo, NPC->client->enemyTeam, qtrue, Q_irand( qfalse, qtrue ), qtrue );//3rd parm was (NPC->enemyTeam == TEAM_STARFLEET)
//			if ( newEnemy )
//			{
//				foundenemy = qtrue;
//				if ( setEnemy )
...
int NPC_ShotEntity( gentity_t *ent, vec3_t impactPos )
{
	...
	CalcEntitySpot( ent, SPOT_CHEST/*CHEST*/, targ );
...
//	qboolean	duck_ok = qfalse;
	...
	//Yaw to enemy
	CalcEntitySpot( NPC->enemy, SPOT_CHEST/*CHEST*//*HEAD*/, enemy_org );
