	...
	if (bot_honorableduelacceptance.integer)
	{
			...
			bs->frame_Enemy_Len < 400)
		{
			...
			{ //Our enemy has his saber holstered and has challenged us to a duel, so challenge him back
				...
				//RoAR mod NOTE: We can't allow bots to be lamed...
				bs->doAttack = 0;
				bs->doAltAttack = 0;
				bs->botChallengingTime = level.time + 100;
				//bs->beStill = 1000; <---CHASE HIM DOOWWWN!
			}
		}
	}
...
int BotAISetup( int restart ) {
	...
	//RoAR mod BEGIN
	//trap_Cvar_Register(&bot_nolaming, "bot_nolaming", "0", 0);
	//RoAR mod END
#ifndef FINAL_BUILD
